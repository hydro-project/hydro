...
async fn main() {
    ...
    let config = vec![
        r#"profile.dev.lto="off""#.to_string(),
        r#"profile.dev.strip="debuginfo""#.to_string(),
        r#"profile.dev.opt-level=3"#.to_string(),
        r#"profile.dev.panic="unwind""#.to_string(),
    ...
    let nodes = builder
        .with_process(
            &p1,
            deployment.add_localhost_docker("First".to_string(), None, config.clone()),
        )
        .with_process(
            &p2,
            deployment.add_localhost_docker("Second".to_string(), None, config.clone()),
        )
        .with_process(
            &p3,
            deployment.add_localhost_docker("Third".to_string(), None, config.clone()),
        )
