...
async fn main() {
    ...
    use hydro_test::cluster::compute_pi::{compute_pi, Worker, Leader};
    ...
    let clusters = vec![(cluster.id().raw_id(), std::any::type_name::<Worker>().to_string(), 8)];
    let processes = vec![(leader.id().raw_id(), std::any::type_name::<Leader>().to_string())];
