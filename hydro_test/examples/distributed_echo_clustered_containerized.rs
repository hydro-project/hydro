...
use hydro_test::distributed::distributed_echo::distributed_clustered_echo;
...
async fn main() {
    ...
    let (external_sink, external_stream) = distributed_clustered_echo(&external, &p1, &c2, &p3);
    ...
    external_sink.send("external".to_owned()).await.unwrap();
