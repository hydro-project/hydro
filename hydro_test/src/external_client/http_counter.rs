...
pub fn http_counter_server<'a, P>(
    ...
    // Clone and filter into increment and get streams
    ...
    let lookup_tick = process.tick();
    let batch_to_get = get_stream
        .batch(&lookup_tick, nondet!(/** batch get requests */))
        .entries()
        .map(q!(|(cid, key)| (key, cid)))
        .into_keyed();
    let lookup_result = counters
        .snapshot(&lookup_tick, nondet!(/** snapshot for join */))
        .join(batch_to_get.clone());
    let get_responses = lookup_result
        ...
        .chain(
            ...
            batch_to_get
                .entries()
                .anti_join(lookup_result.entries().map(q!(|(k, _)| k)))
                .map(q!(|(key, cid)| (
                    cid,
                    format!(
                        "HTTP/1.1 200 OK\r\n\
    ...
    let increment_responses = increment_stream
        .batch(
            &lookup_tick,
            nondet!(/** wait for the increment to process */),
        )
        .map(q!(|key| {
            format!(
                "HTTP/1.1 200 OK\r\n\
