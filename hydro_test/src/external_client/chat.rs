...
pub fn chat_server<'a, P>(
    ...
    sliced!(|
        use(in_stream, nondet_user_arrival_broadcast) as msg_batch,
        use(track_membership(membership), nondet_user_arrival_broadcast) as members
    | {
        members.filter(q!(|b| *b))
            .keys()
            .cross_product(msg_batch.entries())
            .into_keyed()
    })
