...
mod tests {
    ...
    async fn broadcast_3a_maelstrom() {
        ...
        output_handle.complete(broadcast_server(input).assume_ordering(nondet!(/** test */)));

        let mut deployment = MaelstromDeployment::new(1)
    ...
    async fn broadcast_3b_maelstrom() {
        ...
        let mut deployment = MaelstromDeployment::new(5)
            .maelstrom_path(&PathBuf::from_str(&std::env::var("MAELSTROM_PATH").unwrap()).unwrap())
            .workload("broadcast")
            .time_limit(20);
