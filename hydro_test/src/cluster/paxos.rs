...
fn p_ballot_calc<'a>(
    ...
    let (p_ballot, p_has_largest_ballot) = sliced! {
        let p_received_max_ballot = use::atomic(max_ballot_atomic, nondet!(/** up to date with tick input */));
...
pub fn index_payloads<'a, L: Location<'a> + NoTick, P: PaxosPayload>(
    ...
    let latest_max_slot = p_max_slot.latest_atomic();
    let all_payloads = c_to_proposers.all_ticks_atomic();

    sliced! {
        let mut next_slot = use::state(|l| l.singleton(q!(0)));
        let updated_max_slot = use::atomic(latest_max_slot, nondet!(/** up to date with tick input */));
        let payload_batch = use::atomic(all_payloads, nondet!(/** up to date with tick input */));
