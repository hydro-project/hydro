...
pub fn paxos_core<'a, P: PaxosPayload>(
    ...
    a_log_complete_cycle.complete(a_log.latest_tick(local_nondet("
...
pub fn leader_election<'a, L: Clone + Debug + Serialize + DeserializeOwned>(
    ...
    let (p_ballot, p_has_largest_ballot) = p_ballot_calc(
        proposer_tick,
        p_received_max_ballot.latest_tick(
            proposer_tick,
            partial_exposed_nondet("
...
fn p_leader_heartbeat<'a>(
    ...
    let p_leader_expired = p_to_proposers_i_am_leader
        ...
        .latest_tick(proposer_tick, local_nondet("absorbed into timeout"))
    ...
    let p_trigger_election = p_leader_expired.continue_if(
        proposers
            ...
            .batch(proposer_tick, local_nondet("absorbed into interval"))
            .first(),
    );
...
fn p_p1b<'a, P: Clone + Serialize + DeserializeOwned>(
    ...
    let p_received_quorum_of_p1bs = quorums
        ...
        .snapshot(local_nondet("see above"))
        .entries()
        .max_by_key(q!(|t| t.0))
        .zip(p_ballot.clone())
        .filter_map(q!(
...
fn sequence_payload<'a, P: PaxosPayload>(
    ...
    let indexed_payloads = index_payloads(proposer_tick, p_max_slot, c_to_proposers
            .batch(
                proposer_tick,
                partial_exposed_nondet("
    ...
    let p_to_replicas = join_responses(
        proposer_tick,
        quorums.map(q!(|k| (k, ()))),
        payloads_to_send.batch(local_nondet(
            "
...
pub fn acceptor_p2<'a, P: PaxosPayload, S: Clone>(
    ...
    let a_checkpoint = a_checkpoint.latest_tick(
        acceptor_tick,
        local_nondet(
            "
