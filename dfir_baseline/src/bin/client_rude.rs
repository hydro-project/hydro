/// Rude Open-Loop Client for Admission Control Testing
/// 
/// Unlike client_quiet, this client treats rejection responses as failures
/// and retries them. This tests whether admission control can prevent
/// metastability even with aggressive "rude" clients.
...
async fn main() -> anyhow::Result<()> {
    ...
    // Receiver task - RUDE: treats rejections as failures requiring retry
    let pending_for_receiver = pending_requests.clone();
    let metrics_for_receiver = metrics_writer.clone();
    let retry_tx_for_receiver = retry_tx.clone();
    let receiver_handle = tokio::spawn(async move {
        let mut read_half = read_half;

        loop {
            // Check termination BEFORE attempting to read
            ...
            match tokio::time::timeout(Duration::from_millis(100), read_half.read_exact(&mut buf)).await {
                Ok(Ok(_)) => {
                    ...
                    match response.status {
                        dfir_baseline::ResponseStatus::Success => {
                            // Success - remove from pending and record
                        ...
                        dfir_baseline::ResponseStatus::Rejected => {
                            // RUDE BEHAVIOR: Treat rejection as failure, retry if possible
                            let ts = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs_f64() * 1000.0;
                            let _ = metrics_for_receiver.lock().await.write_event(
                                MetricEvent::RequestRejected { timestamp: ts, req_id: response.id }
                            );
                            
                            if let Some(state) = pending.get_mut(&response.id) {
                                if state.retry_count < max_retries {
                                    state.retry_count += 1;
                                    let _ = retry_tx_for_receiver.send(response.id);
                                } else {
                                    // Max retries exceeded
                                    pending.remove(&response.id);
                                    let _ = metrics_for_receiver.lock().await.write_event(
                                        MetricEvent::RequestFailed { timestamp: ts, req_id: response.id }
                                    );
                                }
                            }
                        }
                ...
                Ok(Err(_)) => break,
                Err(_) => continue, // Timeout on read, just loop and check termination
            }
        }
    });
