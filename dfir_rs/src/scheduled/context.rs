...
/// A wrapper around an inline-codegen tick closure that provides `run()` and `run_tick()` methods.
///
/// `Tick` is the async closure type generated by `dfir_syntax_inline!` / `as_code_inline`.
#[doc(hidden)]
...
impl<Tick: AsyncFnMut()> InlineFlow<Tick> {
