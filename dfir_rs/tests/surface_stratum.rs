#![allow(
    ...
    reason = "imports and qualifications used by commented-out tests"
)]
...
pub fn test_tick_loop_1() {
    ...
    // Without `defer_tick_lazy()` this would be "unsafe" although legal.
...
pub fn test_tick_loop_2() {
    ...
    let mut df = dfir_syntax! {
        a = union() -> tee();
        source_iter([1, 3]) -> [0]a;
        a[0] -> defer_tick_lazy() -> defer_tick_lazy() -> map(|x| 2 * x) -> [1]a;
        a[1] -> for_each(|x| output_inner.borrow_mut().push(x));
    };
    assert_graphvis_snapshots!(df);
...
pub fn test_tick_loop_3() {
    ...
    let mut df = dfir_syntax! {
        a = union() -> tee();
        source_iter([1, 3]) -> [0]a;
        a[0] -> defer_tick_lazy() -> defer_tick_lazy() -> defer_tick_lazy() -> map(|x| 2 * x) -> [1]a;
        a[1] -> for_each(|x| output_inner.borrow_mut().push(x));
    };
    assert_graphvis_snapshots!(df);
