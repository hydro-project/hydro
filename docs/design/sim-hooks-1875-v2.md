...
independent, and a picked observation always makes a nontrivial decision (staying silent
...
`is_honorable`, `is_nontrivial`, `apply`) — registered in a per-instance registry
...
it when the predicate is satisfied; `auto_pause()` just makes installation leave the bool
set. None of these touch the scheduler.

**The rest of `Scripted<H>` behind the existing `RuntimeHook` trait** (the erased trait
the scheduler drives all runtime hook objects through):
...
- The **boundary scan**: at the existing point where the async dataflows report no
  ...
  on any `forgotten()` one. No entropy consumed.
...
- **Keyed hooks** (`KeyedBatchHook`, `KeyedSnapshotHook`, and keyed ordering): decisions
  take a key as their first argument (`keyed_batch.release(&key, 2)`), with per-key
