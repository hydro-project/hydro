    ...
    try {
      ...
      const layoutResult: LayoutResult = {
        ...
        containers: containers.map(container => {
          ...
          const baseWidth = layoutedNode?.width || layoutedNode?.dimensions?.width || container.expandedDimensions?.width || 400;
          const baseHeight = layoutedNode?.height || layoutedNode?.dimensions?.height || container.expandedDimensions?.height || 300;
          
          // Add padding to height for title area at bottom to prevent occlusion
          return {
            ...container,
