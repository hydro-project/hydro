...
import { parseHydroGraphJSON, validateHydroGraphJSON } from '../dist/JSONParser.js';
...
class InvariantChecker {
  ...
  checkNodeVisibilityInvariant(context) {
    ...
    for (const node of allNodes) {
      ...
      assert.strictEqual(
        isVisible,
        shouldBeVisible,
        `${context}: Node ${node.id} visibility invariant violated. Hidden: ${node.hidden}, InCollapsed: ${this.isNodeInCollapsedContainer(node.id)}, Visible: ${isVisible}`
      );
    }
  }
  ...
  checkEdgeVisibilityInvariant(context) {
    ...
    for (const edge of allEdges) {
      ...
      assert.strictEqual(
        isVisible,
        shouldBeVisible,
        `${context}: Edge ${edge.id} (${edge.source} -> ${edge.target}) visibility invariant violated. Source visible: ${sourceVisible}, Target visible: ${targetVisible}, Edge visible: ${isVisible}`
      );
    }
  }
    ...
    for (const container of allContainers) {
      ...
      for (const childId of children) {
        if (this.state.graphNodes.has(childId)) {
          // It's a node
          assert.strictEqual(
            this.state.getNodeContainer(childId),
            container.id,
  ...
  checkHyperEdgeConsistency(context) {
    ...
    for (const hyperEdge of hyperEdges) {
      assert(
        !hiddenContainerIds.has(hyperEdge.source) && !hiddenContainerIds.has(hyperEdge.target),
        `${context}: HyperEdge ${hyperEdge.id} should not connect to hidden containers. Source: ${hyperEdge.source}, Target: ${hyperEdge.target}`
      );
    }
    ...
    for (const hyperEdge of hyperEdges) {
      assert(
        !visibleExpandedContainerIds.has(hyperEdge.source) && !visibleExpandedContainerIds.has(hyperEdge.target),
        `${context}: HyperEdge ${hyperEdge.id} should not connect to visible but expanded containers. Source: ${hyperEdge.source}, Target: ${hyperEdge.target}`
      );
    }
    ...
    for (const hyperEdge of hyperEdges) {
      ...
      assert(
        sourceIsVisibleCollapsed || targetIsVisibleCollapsed,
        `${context}: HyperEdge ${hyperEdge.id} should connect to at least one visible, collapsed container. Source: ${hyperEdge.source}, Target: ${hyperEdge.target}`
      );
      ...
      if (!sourceIsVisibleCollapsed) {
        ...
        assert(
          ...
          `${context}: HyperEdge source ${hyperEdge.source} should be a visible node when not a collapsed container`
        );
      }
      ...
      if (!targetIsVisibleCollapsed) {
        ...
        assert(
          ...
          `${context}: HyperEdge target ${hyperEdge.target} should be a visible node when not a collapsed container`
        );
      }
    }
  ...
  _checkContainerHyperEdgeConstraints(context, hiddenContainerIds, visibleExpandedContainerIds) {
    ...
    for (const containerId of hiddenContainerIds) {
      const adjacentHyperEdges = hyperEdges.filter(he => 
        he.source === containerId || he.target === containerId
      );
      
      assert(
        adjacentHyperEdges.length === 0,
        `${context}: Hidden container ${containerId} should have no adjacent hyperEdges, but found: ${adjacentHyperEdges.map(he => he.id).join(', ')}`
      );
    }
    ...
    for (const containerId of visibleExpandedContainerIds) {
      ...
      assert(
        adjacentHyperEdges.length === 0,
        `${context}: Visible but expanded container ${containerId} should have no adjacent hyperEdges, but found: ${adjacentHyperEdges.map(he => he.id).join(', ')}`
      );
    }
  }
  ...
  checkCollectionConsistency(context) {
    ...
    assert.strictEqual(
      visibleNodes.length,
      expectedVisibleNodes.length,
      `${context}: Visible nodes collection size mismatch. Expected: ${expectedVisibleNodes.length}, Actual: ${visibleNodes.length}`
    );
    ...
    assert.strictEqual(
      visibleEdges.length,
      expectedVisibleEdges.length,
      `${context}: Visible edges collection size mismatch. Expected: ${expectedVisibleEdges.length}, Actual: ${visibleEdges.length}`
    );
    ...
    assert.strictEqual(
      ...
      expectedExpandedContainers.length,
      `${context}: Expanded containers collection size mismatch. Expected: ${expectedExpandedContainers.length}, Actual: ${expandedContainers.length}`
    );
  }
  ...
  checkNodeToEdgeMappingInvariant(context) {
    ...
    for (const [nodeId, expectedEdgeIds] of expectedMapping) {
      ...
      assert.strictEqual(
        ...
        `${context}: Node ${nodeId} edge mapping size mismatch. Expected: ${expectedEdgeIds.size}, Actual: ${actualEdgeIds.size}`
      );
      
      for (const edgeId of expectedEdgeIds) {
        assert(
          actualEdgeIds.has(edgeId),
          `${context}: Node ${nodeId} should be connected to edge ${edgeId}`
        );
      }
    }
    ...
    for (const [nodeId, actualEdgeIds] of this.state.nodeToEdges) {
      ...
      assert.strictEqual(
        actualEdgeIds.size,
        expectedEdgeIds.size,
        `${context}: Node ${nodeId} has unexpected edge mappings`
      );
    }
  }
