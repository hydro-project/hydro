...
function validateNodeWithinContainer(
  childNode: ReactFlowNode, 
  containerNode: ReactFlowNode
): { isWithin: boolean; violations: string[] } {
...
function validateAllBoundaries(reactFlowData: ReactFlowData): { 
  allValid: boolean; 
  violations: Array<{ childId: string; containerId: string; issues: string[] }> 
} {
  const violations: Array<{ childId: string; containerId: string; issues: string[] }> = [];
...
function testBoundaryCheckingFunction(): void {
  ...
  // Valid child node
  const validChild: ReactFlowNode = {
    id: 'child1',
    type: 'standard',
    position: { x: 10, y: 10 },
  ...
  const invalidChild: ReactFlowNode = {
    id: 'child2',
    type: 'standard',
...
async function testSimpleLayoutBoundaries(): Promise<void> {
...
async function testMultiContainerBoundaries(): Promise<void> {
...
function testCoordinateTranslation(): void {
...
function testTopLevelNodeTranslation(): void {
...
export async function runLayoutBoundaryTests(): Promise<void> {
