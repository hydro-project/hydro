...
class InvariantChecker {
    ...
    checkNodeVisibilityInvariant(context) {
        ...
        for (const node of visibleNodes) {
            ...
            assert.strictEqual(node.hidden, false, `${context}: Visible node ${node.id} should not be hidden`);
            const container = this.state.getNodeContainer(node.id);
            if (container) {
                const containerData = this.state.getContainer(container);
                assert.strictEqual(containerData?.collapsed, false, `${context}: Visible node ${node.id} should not be in collapsed container ${container}`);
            }
        }
    }
    /**
    ...
    checkEdgeVisibilityInvariant(context) {
        ...
        for (const edge of visibleEdges) {
            ...
            assert(visibleNodeIds.has(edge.source), `${context}: Edge ${edge.id} source ${edge.source} should be visible`);
            assert(visibleNodeIds.has(edge.target), `${context}: Edge ${edge.id} target ${edge.target} should be visible`);
            // Edge should not be hidden
            assert.strictEqual(edge.hidden, false, `${context}: Visible edge ${edge.id} should not be hidden`);
        }
    }
    /**
    ...
    checkContainerHierarchyInvariant(context) {
        ...
        for (const container of allContainers) {
            ...
            for (const childId of children) {
                ...
                if (nodeContainer) {
                    assert.strictEqual(nodeContainer, container.id, `${context}: Node ${childId} should have container ${container.id} as parent`);
                }
            }
        }
    }
    ...
    checkHyperEdgeConsistency(context) {
        ...
        for (const edge of visibleEdges) {
            ...
            assert(!edge.id?.startsWith('hyper_'), `${context}: Found hyperedge ${edge.id} in visibleEdges - hyperedges should be encapsulated!`);
        }
    }
    /**
    ...
    checkCollectionConsistency(context) {
        ...
        assert.strictEqual(expandedContainers.length, expectedExpandedCount, `${context}: Expanded containers collection size mismatch. Expected: ${expectedExpandedCount}, Actual: ${expandedContainers.length}`);
        // Check that all expanded containers are indeed not collapsed
        for (const container of expandedContainers) {
            assert.strictEqual(container.collapsed, false, `${context}: Container ${container.id} in expandedContainers should not be collapsed`);
        }
    }
}
/**
