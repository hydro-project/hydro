...
export function FlowGraph({ visualizationState, config = DEFAULT_RENDER_CONFIG, layoutConfig, eventHandlers, className, style }) {
    ...
    const baseReactFlowDataRef = useRef(null);
    ...
    const applyManualPositions = useCallback((baseData, manualPosMap) => {
        if (manualPosMap.size === 0)
            return baseData;
        return {
            ...baseData,
    ...
    return (_jsxs("div", { className: className, style: { height: '400px', ...style }, children: [_jsxs(ReactFlow, { nodes: reactFlowData?.nodes || [], edges: reactFlowData?.edges || [], nodeTypes: nodeTypes, edgeTypes: edgeTypes, onNodeClick: onNodeClick, onEdgeClick: onEdgeClick, onNodeDrag: onNodeDrag, onNodeDragStart: onNodeDragStart, onNodeDragStop: onNodeDragStop, onNodesChange: onNodesChange, fitView: config.fitView !== false, fitViewOptions: { padding: 0.1, maxZoom: 1.2 }, attributionPosition: "bottom-left", nodesDraggable: config.nodesDraggable !== false, nodesConnectable: config.nodesConnectable !== false, elementsSelectable: config.elementsSelectable !== false, panOnDrag: config.enablePan !== false, zoomOnScroll: config.enableZoom !== false, minZoom: 0.1, maxZoom: 2, children: [_jsx(Background, { color: "#ccc" }), config.enableControls !== false && _jsx(Controls, {}), config.enableMiniMap !== false && (_jsx(MiniMap, { nodeColor: "#666", nodeStrokeWidth: 2, position: "bottom-right" }))] }), loading && (_jsx("div", { style: {
                    position: 'absolute',
                    top: '10px',
                    right: '10px',
