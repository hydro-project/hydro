    ...
    static convert(layoutResult) {
        ...
        layoutResult.containers.forEach(container => {
            if (!isValidELKContainer(container)) {
                console.warn(`[ReactFlowConverter] ⚠️ Invalid container ${container.id}: missing required dimensions`);
                return;
            }
        ...
        layoutResult.nodes.forEach(node => {
            ...
            const standardNodeData = {
                ...
                ...Object.fromEntries(Object.entries(node).filter(([key]) => !['id', 'label', 'style', 'x', 'y', 'width', 'height', 'hidden'].includes(key)))
            };
            const standardNode = {
                id: node.id,
                type: 'standard',
