...
// ============ ELK Bridge Tests ============
function testVisStateToELKExtraction() {
    ...
    const elkGraph = bridge.visStateToELK(mockVisState);
...
function testExtractVisibleNodes() {
    ...
    const visibleNodes = bridge.extractVisibleNodes(mockVisState);
...
function testExtractAllEdges() {
    console.log('  Testing edge extraction (regular + hyperedges)...');
    const bridge = new ELKBridge();
    const mockVisState = createMockVisState();
    const allEdges = bridge.extractAllEdges(mockVisState);
...
function testBuildELKGraph() {
    console.log('  Testing ELK graph construction...');
    const bridge = new ELKBridge();
    const mockVisState = createMockVisState();
    const nodes = bridge.extractVisibleNodes(mockVisState);
    const containers = bridge.extractVisibleContainers(mockVisState);
    const edges = bridge.extractAllEdges(mockVisState);
    const elkGraph = bridge.buildELKGraph(nodes, containers, edges);
...
function testELKResultApplication() {
    ...
    // Apply ELK results
    bridge.elkToVisState(elkResult, mockVisState);
...
function testContainerHierarchy() {
    ...
    const isInContainer = bridge.isNodeInContainer('node1', 'container1', container);
    const isNotInContainer = bridge.isNodeInContainer('node2', 'container1', container);
...
// ============ Run All Tests ============
export function runELKBridgeTests() {
...
//# sourceMappingURL=ELKBridge.test.js.map