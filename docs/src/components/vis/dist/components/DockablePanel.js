...
import { PANEL_POSITIONS } from './types.js';
import { COMPONENT_COLORS, SIZES, SHADOWS } from '../shared/config.js';
export function DockablePanel({ id, title, children, defaultPosition = PANEL_POSITIONS.TOP_RIGHT, defaultDocked = true, defaultCollapsed = false, onPositionChange, onDockChange, onCollapseChange, className = '', minWidth = 200, minHeight = 100, maxWidth = 400, maxHeight = 600, style }) {
    ...
    const handleDragMove = useCallback((e) => {
        ...
        let newPosition = PANEL_POSITIONS.FLOATING;
    ...
    return (_jsxs("div", { ref: panelRef, className: `dockable-panel ${className}`, style: { ...getPanelStyles(), ...style }, children: [_jsxs("div", { ref: headerRef, onMouseDown: handleDragStart, onTouchStart: handleDragStart, style: {
                    display: 'flex',
                    alignItems: 'center',
                    ...
                    color: COMPONENT_COLORS.TEXT_PRIMARY,
                }, children: [_jsx("span", { children: title }), _jsxs("div", { style: { display: 'flex', gap: '4px' }, children: [_jsx("button", { onClick: handleCollapseToggle, style: {
                                    background: 'none',
                                    border: 'none',
                                    cursor: 'pointer',
                                    fontSize: '10px',
                                ...
                                }, title: isCollapsed ? 'Expand panel' : 'Collapse panel', children: isCollapsed ? '▲' : '▼' }), isDocked && (_jsx("span", { style: {
                                    fontSize: '8px',
                                    color: COMPONENT_COLORS.TEXT_DISABLED,
                                    padding: '2px',
                                }, title: "Panel is docked", children: "\uD83D\uDCCC" }))] })] }), !isCollapsed && (_jsx("div", { style: {
                    padding: '12px',
                    fontSize: '10px',
