...
describe('CoordinateTranslator', () => {
  describe('elkToReactFlow', () => {
    ...
    it('should convert child element coordinates to relative positions', () => {
      ...
      expect(result.y).toBe(150); // 225 - 75 = 150
    });
  });
  ...
  describe('round-trip conversion', () => {
    ...
    it('should preserve coordinates for child elements', () => {
      ...
      expect(backToChildELK.x).toBeCloseTo(originalChildELK.x, 4);
      expect(backToChildELK.y).toBeCloseTo(originalChildELK.y, 4);
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('should handle negative relative coordinates', () => {
      ...
      expect(negativeReactFlow.x).toBe(-75); // 25 - 100 = -75
      expect(negativeReactFlow.y).toBe(-50); // 50 - 100 = -50
    });
  });
});
