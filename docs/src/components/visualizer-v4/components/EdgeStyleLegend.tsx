...
interface EdgeStyleLegendProps {
  ...
  propertyMappings?: Record<string, string | { reactFlowType?: string; style?: React.CSSProperties; animated?: boolean; label?: string; styleTag?: string } >;
  };
  compact?: boolean;
  className?: string;
...
// Shared Styles type for helpers
type LegendStyles = {
  pairBoxStyle: React.CSSProperties;
  numberStyle: React.CSSProperties;
  contentStyle: React.CSSProperties;
  edgeRowStyle: React.CSSProperties;
  sampleStyle: React.CSSProperties;
  labelStyle: React.CSSProperties;
};

// Helper function to generate visual sample from style settings
function generateVisualSample(styleSettings: Record<string, string | number>) {
...
function renderSemanticMappingBoxes(edgeStyleConfig: EdgeStyleLegendProps['edgeStyleConfig'], styles: LegendStyles) {
  ...
  Object.entries(edgeStyleConfig.semanticMappings).forEach(([groupName, group]) => {
    ...
    Object.entries(group).forEach(([optionName, styleSettings], optionIndex) => {
      const sample = generateVisualSample(styleSettings as Record<string, string | number>);
      const box = (
...
function renderBooleanPairBoxes(edgeStyleConfig: EdgeStyleLegendProps['edgeStyleConfig'], styles: LegendStyles) {
...
function renderSinglePropertyBoxes(edgeStyleConfig: EdgeStyleLegendProps['edgeStyleConfig'], styles: LegendStyles) {
  if (!edgeStyleConfig?.singlePropertyMappings) return null;
  return Object.entries(edgeStyleConfig.singlePropertyMappings).map(([property, styleTag]) => {
...
function renderLegacyBoxes(edgeStyleConfig: EdgeStyleLegendProps['edgeStyleConfig'], styles: LegendStyles) {
...
function EdgeStyleLegendInner({
  ...
  const semanticBoxes = useMemo(() => renderSemanticMappingBoxes(edgeStyleConfig, styles), [edgeStyleConfig, styles]);
  const booleanPairs = useMemo(() => renderBooleanPairBoxes(edgeStyleConfig, styles), [edgeStyleConfig, styles]);
  const singleProps = useMemo(() => renderSinglePropertyBoxes(edgeStyleConfig, styles), [edgeStyleConfig, styles]);
  const legacyBoxes = useMemo(() => renderLegacyBoxes(edgeStyleConfig, styles), [edgeStyleConfig, styles]);
