...
interface OriginalEndpoint {
  source: string; // Original leaf source
  target: string; // Original leaf target
}
...
export class ContainerCollapseExpandEngine {
   ...
   */
  private _findCurrentVisibleAncestor(nodeId: string): string {
    let current = nodeId;
  ...
  private _expandChildContainers(containerId: string): void {
    const children = this.state.getContainerChildren(containerId);
    for (const childId of children) {
      const childContainer = this.state.getContainer(childId);
      if (childContainer) {
  ...
  private _showContainerChildren(containerId: string): void {
    const children = this.state.getContainerChildren(containerId);
    for (const childId of children) {
      const node = this.state.getGraphNode(childId);
      if (node) {
  ...
  private _hideCrossingEdges(crossingEdges: any[]): void {
   ...
   */
  private _findCrossingEdges(containerId: string): any[] {
    const children = this.state.getContainerChildren(containerId);
    ...
    for (const [edgeId, edge] of this.state.graphEdges) {
      if (edge.hidden) continue; // Skip already hidden edges
   ...
   */
  private _groupEdgesByCurrentExternalEndpoint(crossingEdges: any[], containerId: string): Map<string, { incoming: any[], outgoing: any[] }> {
    const children = this.state.getContainerChildren(containerId);
    const groups = new Map<string, { incoming: any[], outgoing: any[] }>();
  ...
  private _createHyperedgesFromGroups(containerId: string, edgeGroups: Map<string, { incoming: any[], outgoing: any[] }>): void {
  ...
  private _createHyperedge(sourceId: string, targetId: string, edges: any[]): void {
    ...
    const originalEndpoints = new Map<string, OriginalEndpoint>();
  ...
  private _isContainerEndpoint(endpointId: string): boolean {
    return this.state.getContainer(endpointId) !== undefined;
  }

  // ============ Hyperedge Grounding ============

  private _findContainerHyperedges(containerId: string): any[] {
    const containerHyperedges: any[] = [];
  ...
  private _areBothVisibleLeafNodes(sourceId: string, targetId: string): boolean {
  ...
  private _shouldCreateIntermediateHyperedge(sourceId: string, targetId: string): boolean {
