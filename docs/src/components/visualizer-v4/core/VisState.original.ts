...
export class VisualizationState implements ContainerHierarchyView {
   ...
   * Used by FlowGraph for applying user-dragged positions during relayout
   */
  getAllManualPositions(): Map<string, { x: number; y: number }> {
    return new Map(this._collections.manualPositions);
  }

  /**
   * Check if there are any manual positions
   * Used by FlowGraph to determine if manual position application is needed
   */
  hasAnyManualPositions(): boolean {
    return this._collections.manualPositions.size > 0;
  }

  /**
   * Set manual position for a node or container
   * Used by FlowGraph when user drags elements
   */
  ...
  private _handleContainerCollapse(containerId: string): void {
    ...
    for (const childId of children) {
      const childContainer = this._collections.containers.get(childId);
      if (childContainer) {
        this._handleContainerCollapse(childId);
      }
    }
    ...
    // 6. CRITICAL FIX: Update existing hyperEdges that now have invalid endpoints
  ...
  private _hideChild(childId: string): void {
