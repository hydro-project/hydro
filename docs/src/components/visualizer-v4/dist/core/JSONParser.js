 ...
 * ```
 */
export function parseGraphJSON(jsonData, selectedGrouping) {
...
export function createGraphParser(options = {}) {
    const { validateData = true, strictMode = false, defaultNodeStyle = NODE_STYLES.DEFAULT, defaultEdgeStyle = EDGE_STYLES.DEFAULT } = options;
    return {
        parse: (data, grouping) => {
            if (validateData) {
 ...
 * ```
 */
export function validateGraphJSON(jsonData) {
    try {
        const data = typeof jsonData === 'string' ? JSON.parse(jsonData) : jsonData;
        const errors = [];
        const warnings = [];
...
function parseNodes(nodes, state) {
    for (const rawNode of nodes) {
        try {
            ...
            const style = mapStyleConstant(rawNode.style, NODE_STYLES, NODE_STYLES.DEFAULT);
    ...
    for (const rawEdge of edges) {
        try {
            ...
            if (typeof rawEdge.style === 'string') {
                style = mapStyleConstant(rawEdge.style, EDGE_STYLES, EDGE_STYLES.DEFAULT);
            }
            else if (rawEdge.style && typeof rawEdge.style === 'object') {
                style = detectEdgeStyleFromObject(rawEdge.style);
            }
            else {
 ...
 */
function detectEdgeStyleFromObject(styleObj) {
...
function parseHierarchy(data, groupingId, state) {
    let containerCount = 0;
    // Find the requested hierarchy - check both new and old formats
    let hierarchyChoice = null;
    let groupsData = null;
    ...
    if (groupsData) {
        ...
        function createContainersFromHierarchy(hierarchyItems, parentId) {
...
function mapStyleConstant(rawStyle, styleConstants, defaultStyle) {
