...
export class VisualizationState {
     ...
     */
    setContainer(id, { expandedDimensions = { width: 0, height: 0 }, collapsed = false, hidden = false, children = [], ...otherProps }) {
        const container = {
            id,
            expandedDimensions,
            collapsed,
    ...
    _checkHyperEdgeExistence() {
        ...
        for (const hyperEdge of this.hyperEdges.values()) {
            const hasCollapsedContainer = visibleCollapsedContainers.has(hyperEdge.source) ||
