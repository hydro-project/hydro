...
export function Visualizer({ graphData, onControlsReady }) {
            ...
            if (groupNodes.length > 0) {
                ...
                const doInit = async () => {
                    performLayoutOperation(async (opId) => {
                        ...
                        const collapsedResult = await reactFlowStateManager.initializeReactFlow(initState, fullLayoutResult.nodes, edges, currentLayout, 'auto-collapse');
    ...
    const handleLayoutChange = useCallback(async (newLayout) => {
        ...
        if (visualState && nodes.length > 0) {
            performLayoutOperation(async (opId) => {
                const result = await reactFlowStateManager.updateLayout(visualState, nodes, edges, newLayout, 'layout-change');
    ...
    const handleCollapseAll = useCallback(async () => {
        ...
        return performLayoutOperation(async (opId) => {
            ...
            const result = await reactFlowStateManager.updateContainerStates(newState, nodes, edges, currentLayout, 'collapse-all');
    ...
    const handleExpandAll = useCallback(async () => {
        ...
        return performLayoutOperation(async (opId) => {
            ...
            // Apply layout with new state via ReactFlow state manager
            const result = await reactFlowStateManager.updateContainerStates(newState, nodes, edges, currentLayout, 'expand-all');
            setVisualState(newState); // Update state
            // Deterministic viewport fitting after layout is complete
    ...
    const handleNodeClick = useCallback(async (event, node) => {
        if (node.type === 'group' || node.type === 'collapsedContainer') {
            ...
            return performLayoutOperation(async (opId) => {
                ...
                const result = await reactFlowStateManager.updateContainerStates(newState, nodes, edges, currentLayout, `toggle-container-${node.id}`);
    ...
    useEffect(() => {
        if (onControlsReady) {
            const controls = (_jsx(LayoutControls, { currentLayout: currentLayout, onLayoutChange: handleLayoutChange, colorPalette: colorPalette, onPaletteChange: setColorPalette, hasCollapsedContainers: hasCollapsedContainers, onCollapseAll: handleCollapseAll, onExpandAll: handleExpandAll, autoFit: autoFit, onAutoFitToggle: handleAutoFitToggle, onFitView: handleFitView }));
    ...
    useEffect(() => {
        ...
        const processGraph = async () => {
            ...
            try {
                ...
                if (isCancelled)
                    return; // Don't update state if component unmounted or effect cancelled
    ...
    if (isLoading) {
        return _jsx("div", { className: styles.loading, children: "Laying out graph..." });
    }
    return (_jsxs("div", { className: styles.visualizationWrapper, children: [_jsx(InfoPanel, { colorPalette: colorPalette, graphData: graphData, nodes: nodes, collapsedContainers: collapsedContainers, onToggleContainer: handleHierarchyToggle, childNodesByParent: childNodesByParent, onPositionChange: (position, docked) => handlePanelPositionChange('info', position, docked), hierarchyChoices: hierarchyChoices, currentGrouping: currentGrouping, onGroupingChange: handleGroupingChange }), _jsx(ReactFlowInner, { nodes: displayNodes, edges: displayEdges, onNodesChange: onNodesChange, onEdgesChange: onEdgesChange, onNodeClick: handleNodeClick, colorPalette: colorPalette }), isInitializing && (_jsx("div", { className: styles.initializationOverlay, children: _jsxs("div", { className: styles.initializationContent, children: [_jsx("div", { className: styles.spinner }), _jsx("span", { children: "Preparing graph..." })] }) })), initFailed && (_jsx("div", { className: styles.initializationOverlay, children: _jsxs("div", { className: styles.initializationContent, children: [_jsx("div", { className: styles.spinner }), _jsx("span", { style: { color: COMPONENT_COLORS.STATUS_ERROR }, children: "Graph failed to initialize after several attempts." }), _jsx("button", { className: styles.clearButton, style: { marginTop: 16 }, onClick: () => {
                                setInitFailed(false);
                                setInitAttempts(0);
                                setHasAutoCollapsed(false);
                                setIsInitializing(false);
