 ...
 */
import { getELKConfig, getContainerELKConfig, createFixedPositionOptions, createFreePositionOptions } from './elkConfig.js';
...
export function createELKStateManager() {
    ...
    async function calculateVisualLayout(allNodes, allEdges, visualState, layoutType = 'mrtree', dimensionsCache) {
        ...
        const containerNodes = visibleNodes.filter(node => node.type === 'group' || node.type === 'collapsedContainer');
    ...
    async function calculateContainerRepositioning(displayNodes, displayEdges, layoutType = 'mrtree', changedContainerId = null) {
        ...
        const containerNodes = displayNodes.filter(node => node.type === 'group' || node.type === 'collapsedContainer');
        if (containerNodes.length === 0) {
        ...
        const elkContainers = containerNodes.map(container => {
            ...
            if (changedContainerId && container.id !== changedContainerId) {
                elkContainer.layoutOptions = createFixedPositionOptions(container.position.x, container.position.y);
            }
            else {
