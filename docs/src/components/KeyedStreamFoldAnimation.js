...
import HydroColors from './hydro-colors';
...
const KeyedStreamFoldAnimation = () => {
    const setupAnimation = (world) => {
        ...
        const inputKeyGroups = {
            "alice": world.createKeyGroup('inputAlice', new Point(inputContainer.getContentCenter().x, inputKeyGroupPositions[0]), 140, 50, 'Key "alice"', HydroColors.getKeyColor(0)),
            "bob": world.createKeyGroup('inputBob', new Point(inputContainer.getContentCenter().x, inputKeyGroupPositions[1]), 140, 50, 'Key "bob"', HydroColors.getKeyColor(1))
        };

        const outputKeyGroupPositions = outputContainer.verticalChildPositions(50, 2);
        const outputKeyGroups = {
            "alice": world.createKeyGroup('outputAlice', new Point(outputContainer.getContentCenter().x, outputKeyGroupPositions[0]), 140, 50, 'Key "alice"', HydroColors.getKeyColor(0)),
            "bob": world.createKeyGroup('outputBob', new Point(outputContainer.getContentCenter().x, outputKeyGroupPositions[1]), 140, 50, 'Key "bob"', HydroColors.getKeyColor(1))
        };
