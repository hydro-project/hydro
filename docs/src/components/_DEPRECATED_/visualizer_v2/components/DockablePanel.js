...
export function DockablePanel({
  ...
  const getPositionClasses = () => {
    ...
    if (isDragging) classes.push(styles.dockablePanelDragging);
    if (isCollapsed) classes.push(styles.dockablePanelCollapsed);
  ...
  return (
    ...
    >
      <div
        ref={headerRef}
        className={styles.dockablePanelHeader}
        onMouseDown={handleDragStart}
        onTouchStart={handleDragStart}
      >
        <h4 className={styles.dockablePanelTitle}>{title}</h4>
        <div className={styles.dockablePanelControls}>
          <button
            className={styles.dockablePanelToggle}
      ...
      {!isCollapsed && (
        <div className={styles.dockablePanelContent}>
          {children}
        </div>
      )}
      
      {/* Dock zone indicators when dragging */}
      {isDragging && (
        <div className={styles.dockZoneOverlay}>
          <div className={`${styles.dockZone} ${styles.dockZoneTopLeft}`} />
          <div className={`${styles.dockZone} ${styles.dockZoneTopRight}`} />
          <div className={`${styles.dockZone} ${styles.dockZoneBottomLeft}`} />
          <div className={`${styles.dockZone} ${styles.dockZoneBottomRight}`} />
        </div>
      )}
    </div>
  );
