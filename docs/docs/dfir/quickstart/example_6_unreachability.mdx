...
import exampleCode from '!!raw-loader!../../../../dfir_rs/examples/example_6_unreachability.rs.disabled';
import exampleOutput from '!!raw-loader!../../../../dfir_rs/examples/snapshots/example_6_unreachability.snap';
import { getLines, extractOutput, extractMermaid } from '../../../src/util';

# Graph Un-Reachability

:::warning
TODO(inline): This example uses intra-tick cycles and strata which are not yet supported in the inline DFIR codegen path. The example code is preserved but currently disabled.
:::
...
> * An example of how a [cyclic dataflow](../concepts/cyclic_flows.mdx) in one stratum executes to completion before starting the next stratum.
...
discussed in the section on [In-Out Trees](../architecture/in-out_trees.md). We need not concern ourselves with this detail other than to look carefully at the `stratum` labels on the grey boxes in our Mermaid diagrams.
...
In this Mermaid graph, note that stratum 0 has a recursive loop back through `my_join`, and `tee`s off output to the `difference` operator in stratum 1 via the handoff and the blocking `neg` input. This means that DFIR will first run the loop of stratum 0 repeatedly until all the transitive reached vertices are passed to the handoff (a [fixpoint](../concepts/cyclic_flows.mdx)), before moving on to compute the unreached vertices via stratum 1.
