...
import exampleCode from '!!raw-loader!../../../../dfir_rs/examples/example_5_reachability.rs.disabled';
...
:::warning
TODO(inline): This example uses intra-tick cycles which are not yet supported in the inline DFIR codegen path. The example code is preserved but currently disabled.
:::
...
> * A first example of a [cyclic dataflow](../concepts/cyclic_flows.mdx).
...
There is also an auto-generated `handoff` operator that enforces the rule that a push producer and a pull consumer must be separated by a `handoff` (see the [Architecture section](../architecture/handoffs.md)).
