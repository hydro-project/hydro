...
import exampleOutput from '!!raw-loader!../../../../dfir_rs/examples/snapshots/example_1_simplest.snap';
...
[_surface syntax_](../syntax/index.mdx).
...
until we run it.
We can run this flow from within Rust via the [`run_available_sync()` method](rust:dfir_rs::runtime::context::Dfir::run_available_sync).

<CodeBlock language="rust">{getLines(exampleCode, 'run')}</CodeBlock>

Note that `run_available_sync()` runs the DFIR graph until no more work is immediately
...
In server applications that use network ingress for inputs, new work can appear at any time. In these applications
we may need a different method than `run_available_sync()`, e.g. the [`run()`](rust:dfir_rs::runtime::context::Dfir::run) method, check [the networking examples](./example_7_networking.mdx) for more.
