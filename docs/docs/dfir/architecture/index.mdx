...
import exampleOutput from '!!raw-loader!../../../../dfir_rs/examples/snapshots/example_5_reachability.snap';
import { extractMermaid } from '../../../src/util';

# Architecture

:::warning
TODO(inline): This page references the scheduled DFIR architecture (subgraphs, handoffs, strata) which is being replaced by the inline codegen path. Content is out of date.
:::

DFIR graphs are divided into two layers: the outer _runtime layer_ and
inner _compiled layer_.

In DFIR, operators are partitioned into groups called _subgraphs_, visible in the mermaid diagrams as the outermost gray
boxes, and are connected to other subgraphs via _handoff_ buffers. The operators within a subgraph are compiled together,
forming the compiled layer, while the runtime layer encompasses the execution of each subgraph in sequence as well as all
runtime APIs (like `Context`). Here is the graph from the
...
graphs that get parsed and compiled into a scheduled graph of one or more compiled subgraphs. Please see the [Surface Syntax](../syntax/index.mdx) docs for more information.
...
The Core API lives in [`dfir_rs::runtime`](rust:dfir_rs::runtime),
mainly in methods on the [`Dfir` struct](rust:dfir_rs::runtime::context::Dfir).
Compiled push-based iterators live in [`dfir_rs::compiled`](rust:dfir_rs::compiled).
