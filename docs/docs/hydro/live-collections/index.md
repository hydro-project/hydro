...
- **[Streams](./streams.md)**: a stream of items arriving over time (API requests, events, etc.)
- **[Singletons / Optionals](./singletons-optionals.md)**: single value (or no value), changing (mutating) over time (local state, aggregation results)
- **[Keyed Streams](./keyed-streams.mdx)**: stream of items grouped by a key, with independent ordering for each group (API requests across several clients, "GROUP BY")
- **Keyed Singletons**: single value (or no value) for each key, changing (mutating) over time (per-client state, grouped aggregations)
