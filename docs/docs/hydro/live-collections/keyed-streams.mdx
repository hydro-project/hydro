...
// Count events per server (commutative and idempotent)
# let counts =
events.fold_commutative_idempotent(
    q!(|| 0), 
    q!(|acc, _| *acc += 1)
)
# ;
# unsafe { counts.snapshot(&process.tick()) }.entries().all_ticks().map(q!(|(k, v)| (k.to_string(), v)))
# }, |mut stream| async move {
// { "server1": 2, "server2": 1 }
# let mut results = vec![];
# for _ in 0..2 {
...
# }
# results.sort();
# assert_eq!(results, vec![("server1".to_string(), 2), ("server2".to_string(), 1)]);
# }));
```
