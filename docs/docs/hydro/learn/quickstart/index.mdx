...
To create locations, you must first create a `FlowBuilder`, which will track all the locations used across your distributed system. Then, you can create a process using `flow.process()`, which returns a `Process` value. In a simulation test, all inputs and outputs will originate on an `External`, which identifies an external client. To create an external, you use `flow.external::<()>()`, where `()` is a placeholder tag.

<CodeBlock language="rust" title={
  <Link href="https://github.com/hydro-project/hydro/tree/main/template/hydro/src/lib.rs">src/lib.rs</Link>
} showLineNumbers={19}>{getLines(highlightLines(libRsSrc, []), 19, 21)}</CodeBlock>

Now, you can create the echo server! You start by receiving inputs from the test client using `source_external_bincode`. This returns a tuple with two values: a port handle that the client can use to send requests, and a `Stream` of the received requests. You can then pass this stream into `echo_capitalize`, which returns a stream of responses. To send these back to the external client, you use `send_bincode_external`. This function returns a port handle that the client can use to receive responses.

<CodeBlock language="rust" title={
  <Link href="https://github.com/hydro-project/hydro/tree/main/template/hydro/src/lib.rs">src/lib.rs</Link>
} showLineNumbers={23}>{getLines(highlightLines(libRsSrc, []), 23, 26)}</CodeBlock>
...
<CodeBlock language="rust" title={
  <Link href="https://github.com/hydro-project/hydro/tree/main/template/hydro/src/lib.rs">src/lib.rs</Link>
} showLineNumbers={27}>{getLines(highlightLines(libRsSrc, []), 27, 37)}</CodeBlock>
...
<CodeBlock language="rust" showLineNumbers={45}>{getLines(echoGcpSrc, 45, 47)}</CodeBlock>

To run the cloud deployment:
<Tabs>
<TabItem value="gcp" label="GCP">
```bash
#shell-command-next-line
