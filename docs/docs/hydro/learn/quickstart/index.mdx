...
The second type parameter identifies the [**`Location`**](../../reference/locations/index.md) for the live collection. Hydro provides two types of locations: [`Process`](../../reference/locations/processes.md) (a single machine) or [`Cluster`](../../reference/locations/clusters.md) (a set of machines). By writing code that involves several locations, you can create a distributed system!

Locations can be **tagged** with a type to distinguish them at compile-time. The template defines `struct EchoServer`, which is used as a tag for the echo server. The input and output streams are both located on `Process<'a, EchoServer>`, which means that the requests will be received on that server and responses will be generated on the same server. All locations in a Hydro program have the same lifetime `'a`, which is used to enforce borrowing constraints.
...
To process incoming requests, you use the APIs available on live collections. This example uses `map`, which transforms each element of the incoming requests into a element of the outgoing responses. In Hydro, all values and closures passed to such APIs must be **quoted**. Quoting _captures_ the Rust code so that it can be run on a distributed node. To quote an expression, you wrap it with the `q!` macro.
...
The `bind_single_client` method creates a server that receives messages from a single external client. It returns a port handle, an input stream, and a sink for output. The method takes a codec (`LinesCodec`) that encodes and decodes the raw bytes (in this case, treating each line of text as a message). The `NetworkHint::TcpPort(Some(4000))` specifies that the server should listen on port 4000.
...
### Cloud Deployment
The `echo_gcp.rs` example shows how to deploy to Google Cloud Platform. The main differences are in how you specify the deployment host and handle networking:

<CodeBlock language="rust" title={
  <Link href="https://github.com/hydro-project/hydro/tree/main/template/hydro/examples/echo_gcp.rs">examples/echo_gcp.rs</Link>
} showLineNumbers={27}>{getLines(echoGcpSrc, 27, 39)}</CodeBlock>

Instead of `deployment.Localhost()`, you use `TrybuildHost::new()` with a GCP configuration. The `TrybuildHost` handles compiling your Rust code for the target platform and deploying it. Notice that the external client still runs on localhost—you're deploying the server to the cloud while connecting from your local machine.

When deploying to the cloud, the port is assigned automatically. You can retrieve it after deployment:

<CodeBlock language="rust" title={
  <Link href="https://github.com/hydro-project/hydro/tree/main/template/hydro/examples/echo_gcp.rs">examples/echo_gcp.rs</Link>
} showLineNumbers={45}>{getLines(echoGcpSrc, 45, 47)}</CodeBlock>

To run the GCP deployment:
```bash
#shell-command-next-line
...
For GCP deployment, you'll need to set up Google Cloud credentials and have the `gcloud` CLI installed. See the [Hydro Deploy documentation](../../reference/deploy/index.mdx) for more details.

:::

## Next Steps

Congratulations! You've now written, tested, and deployed a simple Hydro program. In the next sections, you will expand your code into a stateful counter service with support for multiple clients. Along the way, you will learn how Hydro helps you avoid distributed bugs at compile-time with determinism checks and how to scale your application across clusters of machines.
