...
Let's write a test to verify that the counter works correctly. First, set up the test infrastructure. Because the simulator only supports `Stream` as inputs and outputs, we use `.into_keyed()` to convert the inputs into a `KeyedStream` and `.entries()` to convert the outputs back to a regular `Stream`.
...
Because each group in a keyed stream has an independent order of elements, calling `.entries()` results in an unordered stream of tuples. Therefore, we cannot make assertions about the order of outputs, and must use the `assert_yields_unordered` method instead:
...
Instead of counting the original stream, you now count using the atomic stream. This produces an atomic singleton that will allow you to take consistent snapshots later. Note that the APIs on an atomic live collection are the same as on a regular live collection—`count()` works identically:

<CodeBlock language="rust" title={
  <Link href="https://github.com/hydro-project/hydro/tree/main/hydro_test/src/tutorials/single_counter.rs">src/single_counter.rs</Link>
} showLineNumbers={15}>{getLines(singleCounterSrc, 15, 15)}</CodeBlock>

Now, let's focus on the outputs of the counter service. We need to release the acknowledgements and process get requests, while ensuring that a read-after-write produces a consistent response:
