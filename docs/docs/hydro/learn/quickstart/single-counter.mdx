...
The standard pattern in Hydro is to take in a separate parameter each request type and return a separate stream for each output. Your counter service will take in two `KeyedStream` parameters for increment and get requests and return a tuple of two `KeyedStream`s for the responses:
...
The `sliced!` macro takes multiple `use` statements, using syntax inspired by React hooks. Each specified a live collection to slice along with a non-determinism explanation, and returns the current slice. In this case:
