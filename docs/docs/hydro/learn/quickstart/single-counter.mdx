...
Let's write a test to verify that the counter works correctly. First, set up the test infrastructure:
...
## Consistent Snapshots with Atomic Streams
...
Run the test again with `cargo test`, and it should pass! By marking the stream as atomic, you enable `use::atomic` to take snapshots that are consistent with respect to what's released via `end_atomic()`.