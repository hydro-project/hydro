...
Beyond automatic exploration, tests can take manual control of individual decision points with [simulator hooks](./scripting.mdx), pinning a specific scenario while everything else continues to be explored. A test that scripts every decision can run in [deterministic mode](./deterministic.mdx), which executes exactly one interleaving with no exploration at all.

The simulator can also run [multiple versions of a program](./multi-version.mdx) together. This lets you test rolling deployments and catch compatibility bugs that only appear while old and new cluster members are communicating.
...
## Code Coverage
Simulation tests execute your Hydro code inside a dynamic library that is compiled on the fly when the test runs, rather than inside the test binary itself. The simulator compiles that library with exactly the rustflags your crate was compiled with — including `-C instrument-coverage`, whether it came from `RUSTFLAGS`, cargo config, or `--config` — so lines exercised only through the simulator are attributed to your source files like any other test coverage.
