...
- On a **`NoOrder`** stream (`RetriesHook<T, NoOrder>`), each `release(value, copies)` decision names any buffered element, releases `copies >= 1` total deliveries, and consumes it. `copies` is the *total* — `release(v, 1)` observes `v` exactly once with no duplicate, and zero copies is rejected (that would be a *loss*, which at-least-once does not permit). The output is still `NoOrder`, so where the copies land is unobservable: the cardinality is the entire decision, and downstream ordering points shuffle the copies like any other elements. Elements are released one decision at a time because timing matters on an unordered stream — a released element can cycle back through feedback and be chosen ahead of still-buffered input.
- On a **`TotalOrder`** stream (`RetriesHook<T, TotalOrder>`), a decision supplies the released sequence for a *prefix* of the buffered queue: the values in arrival order, where a value repeated back-to-back simulates retries. One-at-a-time releases buy nothing here — ordered arrivals cannot race past pending input, and downstream batching is independent of how releases are grouped — so a single decision may cover many buffered elements (and may cover only some, leaving the rest pending; the decision fires only once it can be honored in full). Finality of the *trailing* value is explicit: `release(values)` keeps its copies open so later decisions may repeat it, while `release_final(values)` consumes it (`release_final([])` retires an element whose copies were all released earlier); an element left open or unreleased is pending input, subject to the ordinary forgotten-hook check. Delayed redeliveries (`ABAB`) are deliberately inexpressible: an at-least-once collection `[A, B]` denotes the physical realizations `A⁺B⁺` — each element stutters *in place* — and a delayed redelivery is a *different collection* (`[A, B, A, B]`), which can only be minted where the *order* dimension is observed. A positional `release_next(copies)` releases the front element's copies without asserting its value (analogous to the merge hooks' `advance_first`); prefer the value-naming forms, which fail loudly when the script falls out of sync with the program.
