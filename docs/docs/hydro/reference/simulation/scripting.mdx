...
At the top level, each `release(value, copies)` decision releases `copies >= 1` copies of one buffered element and consumes it. `copies` is the *total* number of deliveries — `release(v, 1)` observes `v` exactly once with no duplicate, and zero copies is rejected (that would be a *loss*, which at-least-once does not permit):
...
- On a **`TotalOrder`** stream (`RetriesHook<T, TotalOrder>`), the named value must be the **front** of the buffered queue, and the copies are released adjacently in place. This is deliberate: an at-least-once collection `[A, B]` denotes the physical realizations `A⁺B⁺` — each element stutters *in place*. A delayed redelivery like `ABAB` is a *different collection* (`[A, B, A, B]`), which can only be minted where the *order* dimension is observed. A positional `release_next(copies)` expands the front without asserting its value (analogous to the merge hooks' `advance_first`); prefer the value-naming `release`, which fails loudly when the script falls out of sync with the program.
