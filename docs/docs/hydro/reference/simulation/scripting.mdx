...
- **Inside a tick** (`Bounded` handles), a single `order(values)` decision supplies the complete result for that tick's input: a per-key ordering for `KeyedOrderingHook`, an interleaving preserving each key's order for `PartialOrderingHook`, and an interleaving preserving each input's order for `MergeOrderedHook` / `KeyedMergeOrderedHook`. As with in-tick orderings, an operator that faces a genuine choice with no scripted decision fails the tick.

`KeyedBatchHook` mirrors `BatchHook`, with `release_values` naming `(key, value)` entries: for streams ordered within each key the entries must match each key's buffered prefix in order, and for unordered streams they are matched per key as multisets. `KeyedSnapshotHook::reveal(entries)` names the version each key observes next, scanning forward through that key's buffered versions; keys that are not named observe their previously revealed version again.
