...
When building stateful services, you often need to ensure **consistency** between different outputs. For example, in a counter service:
- When a client sends an increment request and receives an acknowledgement
- A subsequent get request from that client should observe the incremented count
...
Hydro provides **atomic streams** to establish consistency relationships between outputs and state snapshots. By marking a stream as atomic, you create a synchronization point that downstream operations can reference.
...
The key changes are:
1. **`atomic(&tick)`**: Marks the stream as atomic, associating it with a tick
...
Atomic streams work by establishing a **happens-before** relationship:

1. When you call `atomic(&tick)` on a stream, all downstream computations are associated with that tick
2. When you call `end_atomic()`, the outputs are held until all computations in the tick complete
3. When you use `use::atomic` in a `sliced!` block, the snapshot is guaranteed to reflect all updates from completed atomic blocks
...
Here's a complete counter service with atomic consistency:
...
// Increment requests
let increment_requests = process.source_iter(q!(vec![(), (), ()]));

// Mark as atomic and compute count
let increment_request_processing = increment_requests.atomic(&atomic_tick);
let current_count = increment_request_processing.clone().count();

// Release acknowledgements after count is updated
let increment_ack = increment_request_processing.end_atomic();

// Get requests observe consistent snapshots
let get_requests = process.source_iter(q!(vec![()]));
let get_response = sliced! {
    let request_batch = use(get_requests, nondet!(/** test */));
    let count_snapshot = use::atomic(current_count, nondet!(/** test */));
    request_batch.cross_singleton(count_snapshot).map(q!(|(_, count)| count))
};

# get_response
# }, |mut stream| async move {
# // The count should be 3 after all increments
# let result = stream.next().await.unwrap();
# assert!(result >= 0); // Count is at least 0
# }));
```
...
| Atomic | `use::atomic(collection, nondet!(...))` | Snapshot reflects all updates from completed atomic blocks |
...
Hydro's built-in atomicity has important limitations:
...
3. **Performance implications**: Atomic streams introduce synchronization points that can affect throughput. Use them only when consistency is required.
...
1. **Use atomic streams sparingly**: Only mark streams as atomic when you need consistency guarantees. Unnecessary atomicity can hurt performance.

2. **Document consistency requirements**: Make it clear in your code and documentation what consistency guarantees your service provides.

3. **Test with simulation**: Always test atomic code with exhaustive simulation to catch subtle ordering bugs.

4. **Consider alternatives**: Sometimes you can avoid atomicity by restructuring your protocol. For example, including a sequence number in responses lets clients detect stale reads without server-side atomicity.
