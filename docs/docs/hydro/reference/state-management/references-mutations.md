...
Within each slice, the deposits are applied before the reads because the write closure is declared first, so reads always observe a balance that reflects every deposit in the same batch. Across slices, the balance persists via the [state hook](./slices.mdx#state-hooks).
