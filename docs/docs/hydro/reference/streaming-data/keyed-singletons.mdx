...
Inside a slice, batched keyed singletons also unlock relational APIs like `get`, `join_keyed_stream`, `join_keyed_singleton`, and `lookup_keyed_singleton` for combining keyed data. See [Slice Blocks](../state-management/slices.mdx) for the full semantics of slicing each collection type.
