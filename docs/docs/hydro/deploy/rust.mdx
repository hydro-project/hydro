...
The `TracingOptions` builder has several options, such as sampling frequence, output file names, tracing options, and
optionally a setup command to run before the profiling starts. The `setup_command` can be used to install the profiling,
in this case we use the provided `DEBIAN_PERF_SETUP_COMMAND` which installs `perf` and sets some kernel paramters to
enable tracing.
