...
In order to opt into decoupling and partitioning, Hydro programs must be deployed with the `deploy_and_analyze` function and run with the `--features ilp` flag.
