    ...
    pub async fn connect(&self) -> ClientConnection {
        match self {
            ...
            ServerPort::Demux(bindings) => {
                ClientConnection::Demux(bindings.iter().map(|(k, v)| async move {
                    (*k, v.connect().await)
                }).collect::<FuturesUnordered<_>>().collect::<Vec<_>>().await.into_iter().collect())
            }
            ServerPort::Merge(ports) => {
                ClientConnection::Merge(ports.iter().map(|p| p.connect()).collect::<FuturesUnordered<_>>().collect::<Vec<_>>().await.into_iter().collect())
            }
...
pub async fn accept_bound(bound: BoundServer) -> AcceptedServer {
    match bound {
        ...
        BoundServer::Demux(bindings) => {
            AcceptedServer::Demux(bindings.into_iter().map(|(k, b)| async move {
                (k, accept_bound(b).await)
            }).collect::<FuturesUnordered<_>>().collect::<Vec<_>>().await.into_iter().collect())
        }
        BoundServer::Merge(merge) => {
            AcceptedServer::Merge(merge.into_iter().map(|b| async move {
                accept_bound(b).await
    ...
    fn from_defn(pipe: Connection) -> Self {
        match pipe {
            ...
            Connection::AsClient(ClientConnection::Merge(merge)) => {
                let sources = merge.into_iter().map(|port| {
