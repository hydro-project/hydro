...
pub struct ConnectedMultiConnection<I, O, C: Decoder<Item = I> + Encoder<O>> {
    source: Pin<Box<dyn Stream<Item = Result<(u64, I), <C as Decoder>::Error>> + Send + Sync>>,
    sink: Pin<Box<dyn Sink<(u64, O), Error = <C as Encoder<O>>::Error> + Send + Sync>>,
}
    ...
    fn from_defn(pipe: Connection) -> Self {
        match pipe {
            Connection::AsServer(AcceptedServer::MultiConnection(bound_server)) => {
                let (source, sink) = MultiConnectionStream::create::<I, O, C>(*bound_server);
...
> ConnectedSourceSink for ConnectedMultiConnection<I, O, C>
{
    ...
    type Stream =
        Pin<Box<dyn Stream<Item = Result<Self::Output, <C as Decoder>::Error>> + Send + Sync>>;

    type Input = (u64, O);
    type InError = <C as Encoder<O>>::Error;
    type Sink = Pin<Box<dyn Sink<Self::Input, Error = <C as Encoder<O>>::Error> + Send + Sync>>;
...
struct MultiConnectionSource<I, O, C: Decoder<Item = I> + Encoder<O>> {
    #[cfg(unix)]
    ...
    #[expect(clippy::type_complexity, reason = "Dynamic generic stream handling")]
    active_connections:
        HashMap<u64, Pin<Box<dyn Stream<Item = Result<I, <C as Decoder>::Error>> + Send + Sync>>>,
    #[expect(clippy::type_complexity, reason = "Dynamic generic stream handling")]
    new_sink_sender: mpsc::UnboundedSender<(
        u64,
        Pin<Box<dyn Sink<O, Error = <C as Encoder<O>>::Error> + Send + Sync>>,
    )>,
    _phantom: PhantomData<(Box<O>, Box<C>)>,
}

struct MultiConnectionSink<O, C: Encoder<O>> {
    #[expect(clippy::type_complexity, reason = "Dynamic generic stream handling")]
    connection_sinks:
        HashMap<u64, Pin<Box<dyn Sink<O, Error = <C as Encoder<O>>::Error> + Send + Sync>>>,
    #[expect(clippy::type_complexity, reason = "Dynamic generic stream handling")]
    new_sink_receiver: mpsc::UnboundedReceiver<(
        u64,
        Pin<Box<dyn Sink<O, Error = <C as Encoder<O>>::Error> + Send + Sync>>,
    )>,
    _phantom: PhantomData<(Box<O>, Box<C>)>,
...
impl MultiConnectionStream {
    ...
    ) -> (MultiConnectionSource<I, O, C>, MultiConnectionSink<O, C>) {
