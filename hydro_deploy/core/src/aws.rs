        ...
        let cloudwatch_agent_config = cloudwatch_log_group.map(|cwlg| {
            json!({
                ...
                "metrics": {
                    ...
                    // TODO(mingwei): this doesn't work, for unknown reasons. Maybe bad `$` escaping.
                    // "append_dimensions": {
                    //     "InstanceId": "$${aws:InstanceId}" // `$$` to escape for terraform, converts to `$` for AWS CWA to interpret.
                    // }
                }
            })
            .to_string()
