    ...
    Si1: Sink<Item1, Error = Si0::Error>, // TODO(mingwei): Convert to `Either` error?
{
    ...
    fn poll_ready(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        ...
        ready_both!(this.sink0.poll_ready(cx)?, this.sink1.poll_ready(cx)?);
        Poll::Ready(Ok(()))
    }
    ...
    fn poll_flush(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        let this = self.project();
        ready_both!(this.sink0.poll_flush(cx)?, this.sink1.poll_flush(cx)?);
        Poll::Ready(Ok(()))
    }
    fn poll_close(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        let this = self.project();
        ready_both!(this.sink0.poll_close(cx)?, this.sink1.poll_close(cx)?);
        Poll::Ready(Ok(()))
    }
}
