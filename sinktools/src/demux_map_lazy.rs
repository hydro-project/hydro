    ...
    Func: FnMut(Key) -> Si + Unpin,
{
    ...
    fn start_send(self: Pin<&mut Self>, item: (Key, Item)) -> Result<(), Self::Error> {
        ...
        let sink = this
            .sinks
            .entry(item.0.clone())
            .or_insert_with(|| (this.func)(item.0));
...
mod test {
    ...
    async fn test_lazy_demux_sink() {
        let outputs: Rc<RefCell<HashMap<&str, Vec<u8>>>> = Rc::new(RefCell::new(HashMap::new()));
        let outputs_clone = outputs.clone();

        let mut sink = demux_map_lazy(move |key: &str| {
        ...
        sink.send(("a", b"test1".as_slice())).await.unwrap();
        sink.send(("b", b"test2".as_slice())).await.unwrap();
        sink.send(("a", b"test3".as_slice())).await.unwrap();
    ...
    fn test_lazy_demux_sink_good() {
        ...
        let mut sink = pin!(demux_map_lazy(move |key: &str| {
            let outputs = outputs_clone.clone();
            ForEach::new(move |item: &[u8]| {
                outputs
                    .borrow_mut()
        ...
        assert_eq!(Ok(()), sink.as_mut().start_send(("a", b"test1".as_slice())));
        assert_eq!(Ok(()), sink.as_mut().start_send(("b", b"test2".as_slice())));
        assert_eq!(Ok(()), sink.as_mut().start_send(("a", b"test3".as_slice())));
