//! [`LazySinkSource`], and related items.

use core::future::Future;
use core::marker::PhantomData;
use core::pin::Pin;
use core::task::{Context, Poll, Waker};
use std::sync::{Arc, Mutex};
use std::task::Wake;

use futures_util::{ready, Sink, Stream};
use pin_project_lite::pin_project;

struct MultiWaker {
    wakers: Mutex<Vec<Waker>>,
}

impl MultiWaker {
    fn new(waker: &Waker) -> Self {
        MultiWaker {
            wakers: Mutex::new(vec![waker.clone()]),
        }
    }

    fn push(&self, waker: &Waker) {
        let mut guard = self.wakers.lock().unwrap();
        guard.push(waker.clone());
    }
}

impl Wake for MultiWaker {
    fn wake(self: Arc<Self>) {
        let mut wakers = Vec::new();

        {
            let mut guard = self.wakers.lock().unwrap();
            std::mem::swap(&mut wakers, &mut *guard);
        }

        for waker in wakers {
            waker.wake();
        }
    }
}

pin_project! {
    #[project = SharedStateProj]
    #[project_replace = SharedStateProjReplace]
    enum SharedState<Fut, St, Si, Item> {
        Uninit {
            future: Pin<Box<Fut>>,
        },
        Thunkulating {
            future: Pin<Box<Fut>>,
            item: Option<Item>,
            multi_waker: Option<Arc<MultiWaker>>,
        },
        Done {
            stream: Pin<Box<St>>,
            sink: Pin<Box<Si>>,
            buf: Option<Item>,
        },
        Taken,
    }
}

pin_project! {
    /// A lazy sink-source that implements both Sink and Stream traits. The internal state is initialized when the first item is attempted to be pulled from the stream, or when the first item is sent to the sink.
    pub struct LazySinkSource<Fut, St, Si, Item, Error> {
        #[pin]
        state: SharedState<Fut, St, Si, Item>,
        _phantom: PhantomData<Error>,
    }
}

impl<Fut, St, Si, Item, Error> LazySinkSource<Fut, St, Si, Item, Error> {
    /// Creates a new `LazySinkSource` with the given initialization future.
    pub fn new(future: Fut) -> Self {
        Self {
            state: SharedState::Uninit {
                future: Box::pin(future),
            },
            _phantom: PhantomData,
        }
    }
}

impl<Fut, St, Si, Item, Error> Sink<Item> for LazySinkSource<Fut, St, Si, Item, Error>
where
    Fut: Future<Output = Result<(St, Si), Error>>,
    St: Stream,
    Si: Sink<Item>,
    Error: From<Si::Error>,
{
    type Error = Error;

    fn poll_ready(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        let mut this = self.project();

        match this.state.as_mut().project() {
            SharedStateProj::Uninit { .. } => {
                return Poll::Ready(Ok(()));
            }
            SharedStateProj::Thunkulating {
                future,
                item,
                multi_waker,
            } => {
                let waker = if let Some(waker) = multi_waker {
                    waker.push(cx.waker());
                    Waker::from(waker.clone())
                } else {
                    let waker = Arc::new(MultiWaker::new(cx.waker()));
                    *multi_waker = Some(waker.clone());
                    Waker::from(waker)
                };

                let mut new_context = Context::from_waker(&waker);

                match future.as_mut().poll(&mut new_context) {
                    Poll::Ready(Ok((stream, sink))) => {
                        let buf = item.take();
                        this.state.set(SharedState::Done {
                            stream: Box::pin(stream),
                            sink: Box::pin(sink),
                            buf,
                        });
                    }
                    Poll::Ready(Err(e)) => {
                        return Poll::Ready(Err(e));
                    }
                    Poll::Pending => {
                        return Poll::Pending;
                    }
                }
            }
            _ => {}
        }

        match this.state.project() {
            SharedStateProj::Done {sink, buf, .. } => {
                if buf.is_some() {
                    ready!(sink.as_mut().poll_ready(cx).map_err(From::from)?);
                    sink.as_mut().start_send(buf.take().unwrap())?;
                }
                let result = sink.as_mut().poll_ready(cx).map_err(From::from);
                return result;
            }
            _ => panic!("LazySinkSource in invalid state."),
        }
    }

    fn start_send(self: Pin<&mut Self>, item: Item) -> Result<(), Self::Error> {
        let mut this = self.project();

        match this.state.as_mut().project() {
            SharedStateProj::Uninit { .. } => {
                // Need to transition to Thunkulating
                let old_state = this.state.as_mut().project_replace(SharedState::Taken);
                match old_state {
                    SharedStateProjReplace::Uninit { future } => {
                        this.state.set(SharedState::Thunkulating {
                            future,
                            item: Some(item),
                            multi_waker: None,
                        });
                        return Ok(());
                    }
                    _ => unreachable!(),
                }
            }
            SharedStateProj::Thunkulating { .. } => {
                panic!("LazySinkSource not ready.");
            }
            SharedStateProj::Done {sink, buf, .. } => {
                debug_assert!(buf.is_none());
                return sink.as_mut().start_send(item).map_err(From::from);
            }
            SharedStateProj::Taken => {
                panic!("LazySinkSource not ready.");
            }
        }
    }

    fn poll_flush(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        let mut this = self.project();

        match this.state.as_mut().project() {
            SharedStateProj::Uninit { .. } => {
                return Poll::Ready(Ok(()));
            }
            SharedStateProj::Thunkulating {
                future,
                item,
                multi_waker,
            } => {
                let waker = if let Some(waker) = multi_waker {
                    waker.push(cx.waker());
                    Waker::from(waker.clone())
                } else {
                    let waker = Arc::new(MultiWaker::new(cx.waker()));
                    *multi_waker = Some(waker.clone());
                    Waker::from(waker)
                };

                let mut new_context = Context::from_waker(&waker);

                match future.as_mut().poll(&mut new_context) {
                    Poll::Ready(Ok((stream, sink))) => {
                        let buf = item.take();
                        this.state.set(SharedState::Done {
                            stream: Box::pin(stream),
                            sink: Box::pin(sink),
                            buf,
                        });
                    }
                    Poll::Ready(Err(e)) => {
                        return Poll::Ready(Err(e));
                    }
                    Poll::Pending => {
                        return Poll::Pending;
                    }
                }
            }
            _ => {}
        }

        match this.state.project() {
            SharedStateProj::Done {sink, buf, .. } => {
                if buf.is_some() {
                    ready!(sink.as_mut().poll_ready(cx).map_err(From::from)?);
                    sink.as_mut().start_send(buf.take().unwrap())?;
                }
                let result = sink.as_mut().poll_flush(cx).map_err(From::from);
                return result;
            }
            _ => panic!("LazySinkSource in invalid state."),
        }
    }

    fn poll_close(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        let mut this = self.project();

        match this.state.as_mut().project() {
            SharedStateProj::Uninit { .. } => {
                return Poll::Ready(Ok(()));
            }
            SharedStateProj::Thunkulating {
                future,
                item,
                multi_waker,
            } => {
                let waker = if let Some(waker) = multi_waker {
                    waker.push(cx.waker());
                    Waker::from(waker.clone())
                } else {
                    let waker = Arc::new(MultiWaker::new(cx.waker()));
                    *multi_waker = Some(waker.clone());
                    Waker::from(waker)
                };

                let mut new_context = Context::from_waker(&waker);

                match future.as_mut().poll(&mut new_context) {
                    Poll::Ready(Ok((stream, sink))) => {
                        let buf = item.take();
                        this.state.set(SharedState::Done {
                            stream: Box::pin(stream),
                            sink: Box::pin(sink),
                            buf,
                        });
                    }
                    Poll::Ready(Err(e)) => {
                        return Poll::Ready(Err(e));
                    }
                    Poll::Pending => {
                        return Poll::Pending;
                    }
                }
            }
            _ => {}
        }

        match this.state.project() {
            SharedStateProj::Done {sink, buf, .. } => {
                if buf.is_some() {
                    ready!(sink.as_mut().poll_ready(cx).map_err(From::from)?);
                    sink.as_mut().start_send(buf.take().unwrap())?;
                }
                let result = sink.as_mut().poll_close(cx).map_err(From::from);
                return result;
            }
            _ => panic!("LazySinkSource in invalid state."),
        }
    }
}

impl<Fut, St, Si, Item, Error> Stream for LazySinkSource<Fut, St, Si, Item, Error>
where
    Fut: Future<Output = Result<(St, Si), Error>>,
    St: Stream,
    Si: Sink<Item>,
{
    type Item = St::Item;

    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        let mut this = self.project();

        match this.state.as_mut().project() {
            SharedStateProj::Uninit { .. } => {
                // Need to transition to Thunkulating
                let old_state = this.state.as_mut().project_replace(SharedState::Taken);
                match old_state {
                    SharedStateProjReplace::Uninit { future } => {
                        this.state.set(SharedState::Thunkulating {
                            future,
                            item: None,
                            multi_waker: None,
                        });
                    }
                    _ => unreachable!(),
                }
            }
            _ => {}
        }

        match this.state.as_mut().project() {
            SharedStateProj::Thunkulating {
                future,
                item,
                multi_waker,
            } => {
                let waker = if let Some(waker) = multi_waker {
                    waker.push(cx.waker());
                    Waker::from(waker.clone())
                } else {
                    let waker = Arc::new(MultiWaker::new(cx.waker()));
                    *multi_waker = Some(waker.clone());
                    Waker::from(waker)
                };

                let mut new_context = Context::from_waker(&waker);

                match future.as_mut().poll(&mut new_context) {
                    Poll::Ready(Ok((stream, sink))) => {
                        let buf = item.take();
                        this.state.set(SharedState::Done {
                            stream: Box::pin(stream),
                            sink: Box::pin(sink),
                            buf,
                        });
                    }

                    Poll::Ready(Err(_)) => {
                        return Poll::Ready(None);
                    }

                    Poll::Pending => {
                        return Poll::Pending;
                    }
                }
            }
            _ => {}
        }

        match this.state.project() {
            SharedStateProj::Done { stream, .. } => {
                let result = stream.as_mut().poll_next(cx);
                match &result {
                    Poll::Ready(Some(_)) => {}
                    Poll::Ready(None) => {}
                    Poll::Pending => {}
                }
                return result;
            }
            _ => panic!("LazySinkSource in invalid state."),
        }
    }
}

#[cfg(test)]
mod test {
    use futures_util::{SinkExt, StreamExt};

    use super::*;

    #[tokio::test(flavor = "current_thread")]
    async fn tcp_stream_drives_initialization() {
        use tokio::net::{TcpListener, TcpStream};
        use tokio_util::codec::{FramedRead, FramedWrite, LengthDelimitedCodec};

        let (initialization_tx, initialization_rx) = tokio::sync::oneshot::channel::<()>();

        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
                let addr = listener.local_addr().unwrap();

                let mut sink_source = LazySinkSource::new(async move {
                    // initialization is at least partially started now.
                    initialization_tx.send(()).unwrap();

                    let (stream, _) = listener.accept().await.unwrap();
                    let (rx, tx) = stream.into_split();
                    let fr = FramedRead::new(rx, LengthDelimitedCodec::new());
                    let fw = FramedWrite::new(tx, LengthDelimitedCodec::new());
                    Ok::<_, std::io::Error>((fr, fw))
                });

                let stream_task = tokio::task::spawn_local(async move { 
                    StreamExt::next(&mut sink_source).await
                });

                initialization_rx.await.unwrap(); // ensure that the runtime starts driving initialization via the stream.next() call.

                // Note: Since we consumed sink_source in the stream_task, we cannot use it for sending.
                // This test now only verifies stream initialization behavior.

                // trigger further initialization of the future.
                let mut socket = TcpStream::connect(addr).await.unwrap();
                let (client_rx, client_tx) = socket.split();
                let mut client_tx = FramedWrite::new(client_tx, LengthDelimitedCodec::new());
                let _client_rx = FramedRead::new(client_rx, LengthDelimitedCodec::new());

                // try to be really sure that the effects of the above initialization completing are propagated.
                for _ in 0..20 {
                    tokio::task::yield_now().await
                }

                assert!(!stream_task.is_finished()); // We haven't sent anything yet, so the stream should definitely not be resolved now.

                // Now actually send an item so that the stream will wake up and have an item ready to pull from it.
                SinkExt::send(&mut client_tx, bytes::Bytes::from("test"))
                    .await
                    .unwrap();

                assert_eq!(&stream_task.await.unwrap().unwrap().unwrap()[..], b"test");
            })
            .await;
    }

    #[tokio::test(flavor = "current_thread")]
    async fn tcp_sink_drives_initialization() {
        use tokio::net::{TcpListener, TcpStream};
        use tokio_util::codec::{FramedRead, FramedWrite, LengthDelimitedCodec};

        let (initialization_tx, initialization_rx) = tokio::sync::oneshot::channel::<()>();

        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
                let addr = listener.local_addr().unwrap();

                let mut sink_source = LazySinkSource::new(async move {
                    // initialization is at least partially started now.
                    initialization_tx.send(()).unwrap();

                    let (stream, _) = listener.accept().await.unwrap();
                    let (rx, tx) = stream.into_split();
                    let fr = FramedRead::new(rx, LengthDelimitedCodec::new());
                    let fw = FramedWrite::new(tx, LengthDelimitedCodec::new());
                    Ok::<_, std::io::Error>((fr, fw))
                });

                let sink_task = tokio::task::spawn_local(async move {
                    SinkExt::send(&mut sink_source, bytes::Bytes::from("test2"))
                        .await
                        .unwrap();
                });

                initialization_rx.await.unwrap(); // ensure that the runtime starts driving initialization via the send call.

                // try to be really sure that the above sink_task is waiting on the same future to be resolved.
                for _ in 0..20 {
                    tokio::task::yield_now().await
                }

                assert!(!sink_task.is_finished()); // Initialization should not be complete yet.

                // trigger further initialization of the future.
                let mut socket = TcpStream::connect(addr).await.unwrap();
                let (client_rx, client_tx) = socket.split();
                let _client_tx = FramedWrite::new(client_tx, LengthDelimitedCodec::new());
                let mut client_rx = FramedRead::new(client_rx, LengthDelimitedCodec::new());

                // try to be really sure that the effects of the above initialization completing are propagated.
                for _ in 0..20 {
                    tokio::task::yield_now().await
                }

                assert!(sink_task.is_finished());

                assert_eq!(&client_rx.next().await.unwrap().unwrap()[..], b"test2");

                sink_task.await.unwrap();
            })
            .await;
    }
}
