...
pub struct ValueBounded<KeyBound: Boundedness>(PhantomData<KeyBound>);

impl<KeyBound: Boundedness> KeyedSingletonBound for ValueBounded<KeyBound> {
...
impl<'a, K, V, L: Location<'a>, B: Boundedness> KeyedSingleton<K, V, L, ValueBounded<B>> {
...
impl<'a, K, V, L, B: Boundedness> KeyedSingleton<K, V, L, ValueBounded<B>>
where
    L: Location<'a> + NoTick + NoAtomic,
{
...
impl<'a, K, V, L, B: Boundedness> KeyedSingleton<K, V, Atomic<L>, ValueBounded<B>>
where
    L: Location<'a> + NoTick + NoAtomic,
{
    /// Returns a keyed singleton with entries consisting of _new_ key-value pairs that have
