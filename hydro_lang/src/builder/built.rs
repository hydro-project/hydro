...
impl<'a> BuiltFlow<'a> {
    ...
    fn into_deploy<D: LocalDeploy<'a>>(mut self) -> DeployFlow<'a, D> {
        ...
        DeployFlow {
            ...
            processes,
            process_id_name: self.process_id_name.drain(..).collect(),
            clusters,
            cluster_id_name: self.cluster_id_name.drain(..).collect(),
            externals,
            external_id_name: self.external_id_name.drain(..).collect(),
            used: false,
            _phantom: PhantomData,
        }
    }
