...
pub fn deploy_o2o(
    ...
    (
        {
            q!({ env.port(p1_port).connect::<ConnectedDirect>().into_sink() })
                .splice_untyped_ctx(&())
        },
        {
            q!({ env.port(p2_port).connect::<ConnectedDirect>().into_source() })
                .splice_untyped_ctx(&())
        },
    )
}
...
pub fn deploy_o2m(
    ...
    (
        ...
        {
            q!({ env.port(c2_port).connect::<ConnectedDirect>().into_source() })
                .splice_untyped_ctx(&())
        },
    )
}
...
pub fn deploy_m2o(
    ...
    (
        {
            q!({ env.port(c1_port).connect::<ConnectedDirect>().into_sink() })
                .splice_untyped_ctx(&())
        },
        {
            q!({
