...
impl ScriptCtx {
    /// Resolves a hook handle's scripted hook instance: `member` selects the cluster
    /// member's instance (via `.on(member_id)` on the handle) and must be `None` for a
    /// hook bound to an operator on a process. Panics with a precise message if the
    /// handle was never bound, if a cluster-bound handle is scripted without selecting a
    /// member, if `.on(..)` is used on a process-bound handle, or if the member does not
    /// exist.
    #[track_caller]
    ...
    /// know the matching type) for `hook_id` (on cluster member `member`, if the hook
    /// runs on a cluster) under the group protocol: join the current group if this
    /// decision belongs to it, open a new group if the previous one has been consumed,
    /// or hand the decision back to be retried once the previous group's tick execution
    /// has happened. Decisions for different members of the same tick never share a
    /// group: each member's tick is its own scheduler action, so they form consecutive
