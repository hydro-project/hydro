...
impl CompiledSim {
    ...
    /// performed.
    pub fn fuzz<'a>(&'a self, mut thunk: impl AsyncFn() + RefUnwindSafe) {
    ...
    pub fn exhaustive<'a>(&'a self, mut thunk: impl AsyncFnMut() + RefUnwindSafe) -> usize {
        if std::env::var("BOLERO_FUZZER").is_ok() {
            eprintln!(
...
impl<T: Serialize + DeserializeOwned, O: Ordering, R: Retries> Clone for SimReceiver<T, O, R> {
    fn clone(&self) -> Self {
        SimReceiver(self.0, PhantomData)
    }
}
