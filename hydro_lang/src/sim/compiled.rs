...
impl<W: std::io::Write> LogKind<W> {
    fn as_mut(&mut self) -> LogKind<&mut W> {
        match self {
            LogKind::Null => LogKind::Null,
            LogKind::Stderr => LogKind::Stderr,
            LogKind::Custom(w) => LogKind::Custom(w),
        }
    }
}

// via https://www.reddit.com/r/rust/comments/t69sld/is_there_a_way_to_allow_either_stdfmtwrite_or/
struct FmtWriter<W: std::io::Write>(LogKind<W>);
        ...
        loop {
            ...
            if self.async_dfir.run_available().await {
                ...
                if self.possibly_ready_ticks.is_empty() {
                    ...
                    let mut fmt_writer = FmtWriter(self.log.as_mut());
