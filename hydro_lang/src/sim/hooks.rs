    ...
    T: Serialize + DeserializeOwned,
{
    ...
    /// number of deliveries (`release(v, 1)` observes `v` exactly once, with no
    ...
    pub fn release(&self, value: T, copies: usize) -> DecisionFuture {
        assert_copies_at_least_one(copies);
        DecisionFuture::new(self.id, &OrderedRetriesDecision::Release(value, copies))
    }

    /// Scripts a top-level `assume_retries` action to expand the **front** of the
    /// buffered queue, whatever it is, into `copies` adjacent copies, consuming the
    /// slot (waiting for one to arrive if the queue is empty). Unlike [`Self::release`],
    /// this does not assert the released value; prefer `release(value, copies)`, which
