    ...
    T: Serialize + DeserializeOwned + PartialEq,
{
    ...
    pub fn release_empty(&self) -> DecisionFuture {
        DecisionFuture::new(self.id, &BatchDecision::<T>::Empty)
    }
}
    ...
    T: Serialize + DeserializeOwned + PartialEq,
{
    ...
    /// Scripts the next batch to be empty, holding everything buffered.
    pub fn release_empty(&self) -> DecisionFuture {
        DecisionFuture::new(self.id, &UnorderedBatchDecision::<T>::Empty)
    }
}
...
impl<T> SnapshotHook<T> {
        ...
        T: Serialize,
    {
        DecisionFuture::new(
            self.id,
            &SnapshotDecision::Reveal(bincode::serialize(&value).unwrap()),
        )
    }

    /// Scripts the next tick execution to observe the next buffered version.
    pub fn reveal_next(&self) -> DecisionFuture {
        DecisionFuture::new(self.id, &SnapshotDecision::RevealNext)
    }
