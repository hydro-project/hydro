...
#[derive(Clone)]
pub struct SimNode {
    pub port_counter: Rc<RefCell<usize>>,
}
...
impl<'a> Deploy<'a> for SimDeploy {
    ...
    fn allocate_process_port(process: &Self::Process) -> Self::Port {
        let mut borrowed = process.port_counter.borrow_mut();
        let port_id = *borrowed;
        *borrowed += 1;
        port_id
    }

    fn allocate_cluster_port(cluster: &Self::Cluster) -> Self::Port {
        let mut borrowed = cluster.port_counter.borrow_mut();
        let port_id = *borrowed;
        *borrowed += 1;
        port_id
    }
...
fn compile_sim_graph_trybuild(
    ...
    let cluster_dfir_exprs = cluster_graphs
        .into_iter()
        .map(|(lid, g)| {
            let mut dfir_expr: syn::Expr = syn::parse2(g.as_code(
                &quote! { __root_dfir_rs },
            ...
            let ser_lid = serde_json::to_string(&lid).unwrap();
            let extra_stmts_per_cluster = extra_stmts_cluster.get(&lid).cloned().unwrap_or_default();
            syn::parse_quote!((#ser_lid, (0..4).into_iter().map(|__current_cluster_id| {
                #(#extra_stmts_per_cluster)*
                #dfir_expr
            }).collect::<Vec<_>>())) // TODO UNHARDCODE
        })
        .collect::<Vec<syn::Expr>>();
    ...
    let source_ast: syn::File = syn::parse_quote! {
        ...
        fn __hydro_runtime_core<'a>(
            ...
            Vec<(&'static str, Vec<__root_dfir_rs::scheduled::graph::Dfir<'a>>)>,
            Vec<(&'static str, __root_dfir_rs::scheduled::graph::Dfir<'a>)>,
            ...
            #(#extra_stmts)*
            (vec![#(#process_dfir_exprs),*], vec![#(#cluster_dfir_exprs),*], vec![#(#tick_dfir_exprs),*], __hydro_hooks)
        }
        ...
        unsafe extern "Rust" fn __hydro_runtime(
            ...
            Vec<(&'static str, Vec<__root_dfir_rs::scheduled::graph::Dfir<'static>>)>,
            Vec<(&'static str, __root_dfir_rs::scheduled::graph::Dfir<'static>)>,
            ::std::collections::HashMap<&'static str, ::std::vec::Vec<Box<dyn hydro_lang::sim::runtime::SimHook>>>,
        ) {
