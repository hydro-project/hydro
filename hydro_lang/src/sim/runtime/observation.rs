...
/// consuming it. The front may be named ([`Self::Release`], asserting its value) or
/// released positionally ([`Self::ReleaseNext`], whatever it is).
#[derive(Clone, Debug, serde::Serialize, serde::Deserialize)]
    ...
    T: serde::Serialize + serde::de::DeserializeOwned,
{
    fn describe(&self) -> String {
        match self {
            OrderedRetriesDecision::Release(_, copies) => format!("release(value, {})", copies),
...
fn check_copies_at_least_one(copies: usize) -> Result<(), String> {
    if copies == 0 {
        Err("release: the number of copies must be at least 1".to_owned())
    } else {
        Ok(())
    }
}
    ...
    T: Clone + serde::Serialize + serde::de::DeserializeOwned + PartialEq,
{
    ...
    fn is_honorable(&self, decision: &Self::Decision) -> Result<bool, String> {
        let (expected, copies) = match decision {
            OrderedRetriesDecision::Release(expected, copies) => (Some(expected), *copies),
            OrderedRetriesDecision::ReleaseNext(copies) => (None, *copies),
        };
        check_copies_at_least_one(copies)?;
        match (self.input.borrow().front(), expected) {
            ...
            // ahead of the named value: the decision is permanently stuck.
            (Some(_), Some(_)) => Err(
                "release: the named value did not match the front of the buffered queue \
    ...
    fn apply(&mut self, decision: Self::Decision) {
        let copies = match decision {
            OrderedRetriesDecision::Release(_, copies) => copies,
            OrderedRetriesDecision::ReleaseNext(copies) => copies,
        };
