...
/// place: each `release` names the front of the queue and a cardinality, and the copies
...
fn scripted_top_level_ordered_retries_expands_slots() {
    ...
    flow.sim().deterministic(async || {
        in_send.send_many([1, 2]);
        retries.release(1, 2).await;
        retries.release(2, 3).await;
        output.assert_yields_only([1, 1, 2, 2, 2]).await;
    });
}

/// On a `TotalOrder` stream, `release` must name the front of the buffered queue: input
...
#[test]
#[should_panic(expected = "release: the named value did not match the front")]
...
fn scripted_order_then_retries_matches_retries_then_order() {
    ...
    flow.sim().deterministic(async || {
        ...
        retries.release(10, 1).await;
        ordering.emit(20).await; // slot 2: 20 (kept for re-emission)
        retries.release(20, 2).await; // ... expanded to two adjacent copies
