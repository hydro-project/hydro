...
/// A `pause_until` wait is a script barrier, like an output await: while an earlier
/// decision group is unconsumed it must not resolve, and when that group is stuck, the
/// wait reports the stuck group as the root cause instead of blaming its own predicate
/// (which only failed *because* the script is stuck).
#[test]
#[should_panic(expected = "a scripted decision can never be satisfied")]
fn scripted_pause_until_reports_stuck_prior_group() {
    let mut flow = FlowBuilder::new();
    let node = flow.process::<()>();
    let stuck_hook: BatchHook<i32> = flow.sim_hook();
    let pause_hook: BatchHook<i32> = flow.sim_hook();
    ...
    flow.sim().deterministic(async || {
        ...
        pause_send.send(10);
        stuck_hook.release(3).await; // only two elements will ever arrive
        // Both the barrier and the predicate are unsatisfiable here, but the stuck
        // `release(3)` is the root cause and must be the reported error.
        pause_hook.pause_until_count(2).await;
    });
}

/// The barrier is also load-bearing for soundness, not just error attribution: a
/// `pause_until` predicate reads the hook's *current* buffer, which still contains the
/// elements an unconsumed queued decision is about to consume. Here `release_values([1, 2])`
/// consumes both seeds and the cycle produces only a *single* element back, so
/// `pause_until_count(2)` can never be satisfied by any state the script actually
/// reaches — the wait must be reported as unsatisfiable. Without the barrier, the wait
/// would resolve against the two buffered seeds (the very state the queued decision is
/// about to consume), silently accepting a script whose synchronization belief is wrong.
#[test]
#[should_panic(expected = "pause_until_count(2) can never be satisfied")]
fn scripted_pause_until_cannot_be_satisfied_by_the_state_its_own_decision_consumes() {
    use crate::networking::TCP;
    ...
    let released = sliced! {
        let batch = use::batch(merged, nondet!(/** scripted */ hook = batch_hook));
        batch
    };
    ...
    let _out = released.sim_output();

    flow.sim().deterministic(async || {
        in_send.send_many_unordered([1, 2]);
        batch_hook.release_values([1, 2]).await; // consumes both seeds; cycles back only 101
        // The post-release buffer only ever holds {101}: this must fail as
        // unsatisfiable, not resolve against the pre-release seeds {1, 2}.
        batch_hook.pause_until_count(2).await;
    });
}
...
/// The only implicit behaviors are the *only-possibility* ones. A scripted hook with an
/// empty buffer contributes an empty batch when its tick runs for another hook —
/// requiring the test to write `release_empty()` for every such execution would be pure
/// noise.
#[test]
...
/// A group in which no decision can trigger its tick can never make the tick runnable.
/// The simulator reports the unconsumable group instead of executing an empty tick or
/// silently finishing.
#[test]
#[should_panic(expected = "none of the scripted decisions in this group can trigger their tick")]
fn scripted_group_with_no_triggering_decision_panics() {
    let mut flow = FlowBuilder::new();
    let node = flow.process::<()>();
    let first_hook: BatchHook<i32> = flow.sim_hook();
    let second_hook: BatchHook<i32> = flow.sim_hook();
...
/// When a tick mixes a scripted input with an unscripted one, a group of non-triggering
/// decisions cannot be reported eagerly: data could still arrive at the unscripted input
/// and trigger the tick. If no such data ever arrives, the simulation quiesces with the
/// group still pending, and the failure surfaces at the suspended output await with the
/// cannot-trigger error style (rather than the "can never be satisfied" style, which is
/// reserved for decisions waiting on input that can never arrive).
#[test]
#[should_panic(expected = "none of the tick's hooks can trigger it")]
fn scripted_empty_decision_with_idle_unscripted_sibling_reports_stuck_tick() {
    let mut flow = FlowBuilder::new();
    let node = flow.process::<()>();
    let scripted_hook: BatchHook<i32> = flow.sim_hook();

    let (scripted_send, scripted_input) = node.sim_input();
    let (_unscripted_send, unscripted_input) = node.sim_input();
    let out = sliced! {
        let scripted_batch = use::batch(scripted_input, nondet!(/** scripted */ hook = scripted_hook));
        let unscripted_batch = use::batch(unscripted_input, nondet!(/** never fed */));
        scripted_batch.chain(unscripted_batch)
    }
    .sim_output();

    flow.sim().deterministic(async || {
        scripted_send.send(1);
        scripted_hook.release_empty().await;
        // The empty release cannot trigger the tick, and the unscripted input never
        // receives data, so this await can only end in the stuck-group error.
        out.next().await;
    });
}
...
/// A top-level `assume_retries` on a `NoOrder` stream is scripted with `retry(value,
...
/// consumed.
#[test]
fn scripted_top_level_retries_retry() {
    let mut flow = FlowBuilder::new();
    let node = flow.process::<()>();
    ...
    flow.sim().deterministic(async || {
        ...
        retries.retry(2, 2).await;
        retries.retry(1, 1).await;
        output.assert_yields_only_unordered([2, 2, 1]).await;
    });
}

/// A top-level `assume_retries` on a `TotalOrder` stream expands the front slot in
/// place: each `retry` names the front of the queue and a cardinality, and the copies
/// are adjacent in the output.
#[test]
fn scripted_top_level_ordered_retries_expands_slots() {
    ...
    flow.sim().deterministic(async || {
        in_send.send_many([1, 2]);
        retries.retry(1, 2).await;
        retries.retry(2, 3).await;
        output.assert_yields_only([1, 1, 2, 2, 2]).await;
    });
}

/// On a `TotalOrder` stream, `retry` must name the front of the buffered queue: input
/// order is preserved, so a mismatching front can never be released ahead of the named
/// value and the decision is permanently stuck.
#[test]
#[should_panic(expected = "retry: the named value did not match the front")]
fn scripted_top_level_ordered_retries_front_mismatch_panics() {
    ...
    flow.sim().deterministic(async || {
        in_send.send_many([1, 2]);
        retries.retry(2, 1).await;
    });
}

/// `retry_next(copies)` is the positional form of `retry` on a `TotalOrder` stream:
/// it expands the front slot without asserting its value.
#[test]
fn scripted_top_level_ordered_retries_retry_next() {
    let mut flow = FlowBuilder::new();
    let node = flow.process::<()>();
    let retries: RetriesHook<u32, TotalOrder> = flow.sim_hook();
    let (in_send, input) = node.sim_input::<u32, TotalOrder, ExactlyOnce>();
    ...
    flow.sim().deterministic(async || {
        in_send.send_many([1, 2]);
        retries.retry_next(2).await;
        retries.retry_next(1).await;
        output.assert_yields_only([1, 1, 2]).await;
    });
}

/// `retry` with zero copies is rejected at the scripting call site: releasing an
...
fn scripted_retries_retry_zero_copies_panics() {
    let mut flow = FlowBuilder::new();
    let node = flow.process::<()>();
    let retries: RetriesHook<u32, NoOrder> = flow.sim_hook();
    let (in_send, input) = node.sim_input::<u32, NoOrder, ExactlyOnce>();
...
#[test]
#[should_panic(expected = "adjacent duplicate slots of the same element are redundant")]
...
#[test]
#[should_panic(expected = "every buffered element must occupy at least one slot")]
...
fn scripted_order_then_retries_matches_retries_then_order() {
    ...
    flow.sim().deterministic(async || {
        ...
        retries.retry(10, 2).await;
        retries.retry(20, 3).await;
        for value in target {
...
#[test]
#[should_panic(expected = "must release every pending input value at least once")]
...
#[test]
#[should_panic(expected = "expanded adjacently in place")]
...
#[test]
#[should_panic(expected = "in adjacent slots, which is redundant")]
