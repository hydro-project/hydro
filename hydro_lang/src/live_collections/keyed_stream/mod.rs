...
/// be deterministic.
///
/// Although keyed streams are conceptually grouped by keys, keys are not immediately
/// hashed or compared when constructing a keyed stream. Instead, keyed streams lazily
/// hash their keys, deferring until an operator such as [`KeyedStream::fold`] is called.
/// Keyed streams are useful even when no such operators are used, since they can capture
/// total ordering _within_ each group (which is not possible with [`Stream`]).
///
/// Type Parameters:
...
{
    /// Shifts this keyed stream into an atomic context, which guarantees that any [`Clone`]ed branches
...
where
    L: Location<'a>,
{
    /// Produces a new keyed that combines the groups of the inputs by first emitting the elements
    /// of the `self` stream, and then emits the elements of the `other` stream (if a key is only
    /// present in one of the inputs, its values are passed through as-is). The output has a
    /// [`TotalOrder`] guarantee if and only if both inputs have a [`TotalOrder`] guarantee.
    ///
