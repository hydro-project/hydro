...
/// operator such as [`KeyedStream::fold`] is called, which require `K: Hash + Eq`. Keyed streams
/// are useful even when no such operators are used, since they can capture total ordering _within_
/// each group (which is not possible with [`Stream`]).
///
/// Type Parameters:
