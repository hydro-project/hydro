    ...
    L: Location<'a>,
{
    ...
    /// # }
    /// ```
    pub fn filter_if_true(self, signal: Singleton<bool, L, Bounded>) -> Stream<T, L, B, O, R> {
        self.cross_singleton(signal.filter(q!(|b| *b)))
            .map(q!(|(d, _)| d))
    }

    /// Passes this stream through if the boolean signal is `false`, otherwise the output is empty.
    ///
    /// # Example
    /// ```rust
    /// # #[cfg(feature = "deploy")] {
    ...
    /// # }
    /// ```
    pub fn filter_if_false(self, signal: Singleton<bool, L, Bounded>) -> Stream<T, L, B, O, R> {
