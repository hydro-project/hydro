...
impl<'a, K, V, L: Location<'a>, B: Boundedness, O, R> Stream<(K, V), L, B, O, R> {
    ...
    ///
    /// Because [`KeyedStream`] lazily hashes keys, this operator has zero computational cost and
    /// _does not_ require that the key type is hashable. Keyed streams are useful for performing
    /// grouped aggregations, but also for more precise ordering guarantees such as total ordering
    /// _within_ each group but no ordering _across_ groups.
    ///
    /// # Example
    /// ```rust
    /// # use hydro_lang::prelude::*;
...
{
    /// Shifts this stream into an atomic context, which guarantees that any [`Clone`]ed branches
