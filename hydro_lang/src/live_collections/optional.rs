...
use crate::compile::ir::{
    HydroIrOpMetadata, HydroNode, HydroRoot, HydroSource, CollectionKind, TeeNode,
};
    ...
    fn from(singleton: Singleton<T, L, B>) -> Self {
        Optional::new(
            ...
            HydroNode::Cast {
                ...
                metadata: singleton.location.new_node_metadata(Self::collection_kind()),
            },
        )
    }
}
    ...
    L: Location<'a>,
{
    ...
    pub fn snapshot(self, tick: &Tick<L>, _nondet: NonDet) -> Optional<T, Tick<L>, Bounded> {
        ...
        Optional::new(
            ...
            HydroNode::Batch {
                inner: Box::new(self.ir_node.into_inner()),
                metadata: tick.new_node_metadata(Optional::<T, Tick<L>, Bounded>::collection_kind()),
            },
        )
    }
