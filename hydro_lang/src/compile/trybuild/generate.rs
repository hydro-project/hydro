    ...
    let source_ast: syn::File = match deploy_mode {
        ...
        DeployMode::Containerized => {
            syn::parse_quote! {
                ...
                #[allow(unused)]
                async fn __hydro_runtime<'a>() -> #root::runtime_support::dfir_rs::scheduled::context::InlineFlow<impl ::std::ops::AsyncFnMut() + 'a> {
                    /// extra_stmts
                    #( #extra_stmts )*
        ...
        DeployMode::HydroDeploy => {
            syn::parse_quote! {
                ...
                fn __hydro_runtime<'a>(
                    ...
                    -> #root::runtime_support::dfir_rs::scheduled::context::InlineFlow<impl ::std::ops::AsyncFnMut() + 'a>
                {
                    #( #extra_stmts )*
