...
pub fn compile_graph_trybuild(
    ...
    let source_ast: syn::File = if is_containerized {
        syn::parse_quote! {
            ...
            async fn main() {
                let mut flow = __hydro_runtime().await;

                hydro_lang::runtime_support::resource_measurement::run_containerized(flow).await; // this calls launch_flow
            }
        }
    } else {
        syn::parse_quote! {
            ...
            #[allow(unused)]
            fn __hydro_runtime<'a>(__hydro_lang_trybuild_cli: &'a hydro_lang::runtime_support::dfir_rs::util::deploy::DeployPorts<u32, hydro_lang::__staged::deploy::deploy_runtime::HydroMeta>) -> hydro_lang::runtime_support::dfir_rs::scheduled::graph::Dfir<'a> {
                /// extra_stmts
                #(#extra_stmts)*
