...
fn analyze_overheads_node(node: &mut HydroNode, _next_stmt_id: &mut usize, max_send_overhead: &mut f64, max_recv_overhead: &mut f64, location: &LocationId) {
    match node {
        HydroNode::Network { metadata, to_location, .. } => {
...
pub fn analyze_send_recv_overheads(ir: &mut [HydroLeaf], location: &LocationId) -> (f64, f64) {
    ...
    traverse_dfir(ir, |_, _| {}, |node, next_stmt_id| {
        analyze_overheads_node(node, next_stmt_id, &mut max_send_overhead, &mut max_recv_overhead, location);
    });
