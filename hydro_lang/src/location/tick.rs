...
///
/// An `Atomic` context ensures that all reads and writes within a single tick
/// are serialized, preventing interleaving of operations from different sources.
/// This is useful for implementing linearizable read/write protocols where
/// consistency between concurrent operations is required.
///
/// Created via [`Stream::atomic`](crate::live_collections::stream::Stream::atomic)
/// on a stream that has been batched into a tick.
#[derive(Clone)]
