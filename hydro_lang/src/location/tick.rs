...
use crate::{Bounded, Optional, Singleton, Stream, TotalOrder, local_nondet};
    ...
    L: Location<'a>,
{
        ...
        L: NoTick + NoAtomic,
    {
        ...
        out.batch(self, local_nondet!(
                "at runtime, `spin` produces a single value per tick, so each batch is guaranteed to be the same size."
            ))
    }
    ...
    where
        ...
        L: NoTick + NoAtomic,
    {
        self.outer().singleton(e).latest_tick(
            self,
            local_nondet!("a top-level singleton produces the same value each tick"),
        )
    }
