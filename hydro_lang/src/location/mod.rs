...
use crate::{NoOrder, Singleton, Stream, TotalOrder, Unbounded, local_nondet};
            ...
            },
            {
                raw_stream
                    ...
                    .assume_ordering::<TotalOrder>(
                        local_nondet!("order of messages is deterministic within each key due to TCP")
                    )
            },
            {
                raw_membership_stream
                    ...
                    .assume_ordering::<TotalOrder>(local_nondet!(
                        "membership events are ordered within each key"
                    ))
            }
            .map(q!(|join| {
                if join {
        ...
        (
            ...
            },
            {
                raw_stream
                    .into_keyed()
                    .assume_ordering::<TotalOrder>(local_nondet!(
                        "order of messages is deterministic within each key due to TCP"
                    ))
            },
            {
                raw_membership_stream
