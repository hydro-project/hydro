...
pub trait Location<'a>: Clone {
    ...
    fn bidi_external_many_bytes<L, Codec: Encoder<Bytes> + Decoder<Item = BytesMut>, OutOrder>(
        &self,
        from: &External<L>,
    ) -> (
        ExternalBytesPort<Many>,
        Stream<std::io::Result<(u64, BytesMut)>, Self, Unbounded, NoOrder, ExactlyOnce>,
        ForwardRef<'a, Stream<(u64, Bytes), Self, Unbounded, OutOrder, ExactlyOnce>>,
    )
    where
        Self: Sized + NoTick,
    {
        ...
        let (fwd_ref, to_sink) =
            self.forward_ref::<Stream<(u64, Bytes), Self, Unbounded, OutOrder, ExactlyOnce>>();
    ...
    #[expect(clippy::type_complexity, reason = "Dynamic generic stream handling")]
    fn bidi_external_many_bincode<L, InT: DeserializeOwned, OutT: Serialize, OutOrder>(
        &self,
        from: &External<L>,
    ) -> (
        ExternalBincodeBidi<InT, OutT, Many>,
        Stream<(u64, InT), Self, Unbounded, NoOrder, ExactlyOnce>,
        ForwardRef<'a, Stream<(u64, OutT), Self, Unbounded, OutOrder, ExactlyOnce>>,
    )
    where
        Self: Sized + NoTick,
    {
        ...
        let (fwd_ref, to_sink) =
            self.forward_ref::<Stream<(u64, OutT), Self, Unbounded, OutOrder, ExactlyOnce>>();
        let mut flow_state_borrow = self.flow_state().borrow_mut();
...
mod tests {
    ...
    async fn multi_external_bytes() {
        ...
        let (in_port, input, complete_sink) =
            first_node.bidi_external_many_bytes::<_, LengthDelimitedCodec, _>(&external);
        let out = input
            .map(q!(|r| r.unwrap()))
            .send_bincode_external(&external);
    ...
    async fn echo_external_bytes() {
        ...
        let (port, input, complete_sink) =
            first_node.bidi_external_many_bytes::<_, LengthDelimitedCodec, _>(&external);
        complete_sink.complete(input.map(q!(|r| r.unwrap())).map(q!(|(id, bytes)| {
        ...
        assert_eq!(external_out_1.next().await.unwrap(), vec![2, 3, 4]);
        assert_eq!(external_out_2.next().await.unwrap(), vec![5, 6]);
    }
    ...
    async fn echo_external_bincode() {
        ...
        complete_sink.complete(input.map(q!(|(id, text): (u64, String)| {
