...
pub trait Location<'a>: Clone {
    ...
    fn bidi_external_many_bytes<L, T, Codec: Encoder<T> + Decoder, OutOrder>(
        &self,
        from: &External<L>,
    ) -> (
        ...
        Stream<
            ...
            ExactlyOnce,
        >,
        ForwardRef<'a, Stream<(u64, T), Self, Unbounded, OutOrder, ExactlyOnce>>,
    )
    where
        Self: Sized + NoTick,
    {
        ...
        let (fwd_ref, to_sink) =
            self.forward_ref::<Stream<(u64, T), Self, Unbounded, OutOrder, ExactlyOnce>>();
...
mod tests {
    ...
    async fn multi_external_bytes() {
        ...
        let (in_port, input, complete_sink) =
            first_node.bidi_external_many_bytes::<_, _, LengthDelimitedCodec, _>(&external);
        let out = input
            .map(q!(|r| r.unwrap()))
            .send_bincode_external(&external);
