...
pub trait Location<'a>: Clone {
    ...
    fn bidi_external_many_bytes<L, OutOrder>(
        &self,
        from: &External<L>,
    ) -> (
...
mod tests {
    ...
    async fn multi_external_bytes() {
        ...
        let (in_port, input, complete_sink) = first_node.bidi_external_many_bytes(&external);
        let out = input
            .map(q!(|r| r.unwrap()))
            .send_bincode_external(&external);
    ...
    async fn echo_external_bytes() {
        ...
        let (port, input, complete_sink) = first_node.bidi_external_many_bytes(&external);
