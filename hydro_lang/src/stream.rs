    ...
    Order: MinOrder<NoOrder, Min = NoOrder>,
{
    ...
    /// let batch = unsafe {
    ///     numbers.timestamped(&tick).tick_batch()
    /// };
    /// batch
...
impl<'a, T, L: Location<'a>, B> Stream<T, L, B, TotalOrder> {
    ...
    /// let words = process.source_iter(q!(vec!["HELLO", "WORLD"]));
    /// let batch = unsafe {
    ///     words.timestamped(&tick).tick_batch()
    /// };
    /// batch.fold(q!(|| String::new()), q!(|acc, x| acc.push_str(x))).all_ticks().drop_timestamp()
    /// # }, |mut stream| async move {
    /// // "HELLOWORLD"
...
impl<'a, T, L: Location<'a>> Stream<T, L, Bounded, TotalOrder> {
    ...
    /// a [`TotalOrder`] guarantee because the order of the elements is preserved.
    /// 
    ...
    ///     numbers.timestamped(&tick).tick_batch()
    /// };
    /// batch.clone().map(q!(|x| x + 1)).chain(batch).all_ticks().drop_timestamp()
    /// # }, |mut stream| async move {
    /// // 2, 3, 4, 5, 1, 2, 3, 4
    ...
    pub fn union<O2>(
        ...
        unsafe {
            ...
            self.timestamped(&tick)
                .tick_batch()
                .assume_ordering()
                .chain(other.timestamped(&tick).tick_batch().assume_ordering())
                .all_ticks()
                .drop_timestamp()
                .assume_ordering()
        }
...
impl<'a, K: Eq + Hash, V, L: Location<'a>> Stream<(K, V), Tick<L>, Bounded> {
    ...
    ///     numbers.timestamped(&tick).tick_batch()
    /// };
    /// batch.fold_keyed(q!(|| 0), q!(|acc, x| *acc += x)).all_ticks().drop_timestamp()
    /// # }, |mut stream| async move {
    /// // (1, 5), (2, 7)
    ...
    ///     numbers.timestamped(&tick).tick_batch()
    /// };
    /// batch.reduce_keyed(q!(|acc, x| *acc += x)).all_ticks().drop_timestamp()
    /// # }, |mut stream| async move {
    /// // (1, 5), (2, 7)
    /// assert_eq!(stream.next().await.unwrap(), (1, 5));
    /// assert_eq!(stream.next().await.unwrap(), (2, 7));
...
impl<'a, K: Eq + Hash, V, L: Location<'a>, Order> Stream<(K, V), Tick<L>, Bounded, Order> {
    ...
    ///     numbers.timestamped(&tick).tick_batch()
    /// };
    /// batch.fold_keyed_commutative(q!(|| 0), q!(|acc, x| *acc += x)).all_ticks().drop_timestamp()
    /// # }, |mut stream| async move {
    /// // (1, 5), (2, 7)
    /// assert_eq!(stream.next().await.unwrap(), (1, 5));
    /// assert_eq!(stream.next().await.unwrap(), (2, 7));
    ...
    ///     numbers.timestamped(&tick).tick_batch()
    /// };
    /// batch.reduce_keyed_commutative(q!(|acc, x| *acc += x)).all_ticks().drop_timestamp()
    /// # }, |mut stream| async move {
    /// // (1, 5), (2, 7)
    /// assert_eq!(stream.next().await.unwrap(), (1, 5));
    /// assert_eq!(stream.next().await.unwrap(), (2, 7));
