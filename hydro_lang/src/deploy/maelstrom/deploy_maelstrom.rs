...
pub struct MaelstromCluster {
    ...
    compiled_binary: Rc<RefCell<Option<PathBuf>>>,
}
...
impl Node for MaelstromExternal {
    ...
    fn next_port(&self) -> Self::Port {
        let next_port = *self.next_port.borrow();
        *self.next_port.borrow_mut() += 1;
        format!("external_port_{}", next_port)
    }
    ...
    fn instantiate(
        ...
        // External nodes don't need instantiation in Maelstrom
    }
}
...
impl<'a> RegisterPort<'a, MaelstromDeploy> for MaelstromExternal {
    ...
    > + 'a {
        async move {
            panic!("Maelstrom external ports are handled via stdin/stdout, not direct connections")
        }
    }
...
impl MaelstromClusterSpec {
    /// Create a new Maelstrom cluster spec with the given number of nodes.
...
impl<'a> ClusterSpec<'a, MaelstromDeploy> for MaelstromClusterSpec {
    fn build(self, key: LocationKey, name_hint: &str) -> MaelstromCluster {
        MaelstromCluster {
            ...
            compiled_binary: Rc::new(RefCell::new(None)),
        }
    }
}

/// Specification for building a Maelstrom external client.
#[derive(Clone)]
pub struct MaelstromExternalSpec;

impl<'a> ExternalSpec<'a, MaelstromDeploy> for MaelstromExternalSpec {
    fn build(self, key: LocationKey, _name_hint: &str) -> MaelstromExternal {
        MaelstromExternal {
            key,
            next_port: Rc::new(RefCell::new(0)),
...
/// Dummy process spec that panics (processes not supported in Maelstrom).
pub struct MaelstromProcessSpec;

impl<'a> ProcessSpec<'a, MaelstromDeploy> for MaelstromProcessSpec {
    fn build(self, _key: LocationKey, _name_hint: &str) -> MaelstromProcess {
        panic!("Maelstrom deployment does not support processes, only clusters")
    }
}
...
impl MaelstromDeployment {
    ...
    /// Get the cluster spec for this deployment.
    pub fn cluster_spec(&self) -> MaelstromClusterSpec {
        MaelstromClusterSpec::new(self.node_count)
    }

    /// Get the external spec for this deployment.
    pub fn external_spec(&self) -> MaelstromExternalSpec {
        MaelstromExternalSpec
    }
    ...
    /// Run Maelstrom with the compiled binary.
