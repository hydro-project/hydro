...
impl<'a> ClusterSpec<'a, MaelstromDeploy> for MaelstromClusterSpec {
    fn build(self, _key: LocationKey, name_hint: &str) -> MaelstromCluster {
        MaelstromCluster {
...
impl<'a> ExternalSpec<'a, MaelstromDeploy> for MaelstromExternalSpec {
    fn build(self, _key: LocationKey, _name_hint: &str) -> MaelstromExternal {
        panic!("Maelstrom external ports are handled via `maelstrom_bidi_clients`")
    }
}
