...
use stageleft::{QuotedWithContext, RuntimeData, quote_type};
use syn::parse_quote;
...
impl<'a> Deploy<'a> for HydroDeploy {
    ...
    fn e2o_many_source(
        ...
        let codec = quote_type::<LengthDelimitedCodec>();
        let connect_expr: syn::Expr = parse_quote!(
            #root::runtime_support::dfir_rs::util::deploy::ConnectedSourceSink::into_source_sink(
                __hydro_lang_trybuild_cli
                    .port(#p2_port)
                    .connect::<#root::runtime_support::dfir_rs::util::deploy::multi_connection::ConnectedMultiConnection<_, _, #codec>>()
            )
        );
...
impl<'a> RegisterPort<'a, HydroDeploy> for DeployExternal {
    ...
    > + 'a {
        ...
        async move {
            ...
            (
                Box::pin(source.map(|item| item.unwrap().into()))
                    as Pin<Box<dyn Stream<Item = Bytes>>>,
