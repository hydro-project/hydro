...
pub fn deploy_containerized_m2m<'a, Tag: Unpin>(
    port: u16,
    bind_addr: &'a str,
) -> (syn::Expr, syn::Expr) {
    (
...
pub fn cluster_ids<'a>() -> impl QuotedWithContext<'a, &'a [MemberId<()>], ()> + Clone {
    // This is a dummy piece of code, since clusters are dynamic when containerized.
    q!(Box::leak(Box::new([MemberId::<()>::from_container_name(
        "INVALID CONTAINER NAME cluster_ids"
    )]))
    .as_slice())
}

pub fn cluster_self_id<'a>() -> impl QuotedWithContext<'a, MemberId<()>, ()> + Clone + 'a {
    // q!(MemberId::from_container_name(
    //     std::env::var("CONTAINER_NAME").unwrap()
    // ))

    q!(MemberId::<()>::from_container_name("fake container name"))
}
