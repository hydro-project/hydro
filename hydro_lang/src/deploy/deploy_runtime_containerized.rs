...
fn docker_membership_stream(
    ...
    let snapshot_stream = futures::stream::once(async move {
        ...
        docker
            ...
            .flat_map(|names| names.first())
            .map(|name| name.trim_start_matches('/'))
            .filter_map(|name| {
                let mut seen_joined_snapshot = seen_joined_snapshot.lock().unwrap();
                if seen_joined_snapshot.contains(name) {
                    seen_joined_snapshot.insert(name.to_owned());
                    Some((name.to_owned(), MembershipEvent::Joined))
                } else {
                    None
                }
            })
