...
pub struct DockerDeployProcess {
    ...
    exposed_ports: Rc<RefCell<Vec<u16>>>, // TODO: almost certainly don't need this.
    external_targets: Rc<RefCell<Vec<String>>>, // TODO: almost certainly don't need this.
...
pub struct DockerDeployCluster {
    ...
    exposed_ports: Rc<RefCell<Vec<u16>>>, // TODO: almost certainly do not need this.
    external_targets: Rc<RefCell<Vec<String>>>, // TODO: almost certainly do not need this.
...
pub struct DockerDeployExternal {
    ...
    reverse_connection_info:
        Rc<RefCell<HashMap<u16, (Rc<RefCell<Option<String>>>, u16, DockerNetwork)>>>,
}
...
impl<'a> RegisterPort<'a, DockerDeploy> for DockerDeployExternal {
    fn register(&self, key: usize, port: <DockerDeploy as Deploy>::Port) {
        eprintln!(
            "DockerDeployExternal register. self: {}, key: {key} port: {port}",
            self.name
        );
    ...
    {
        eprintln!("as_bincode_sink: key: {key}, {:?}", self.ports.borrow());
        ...
        async move {
            ...
            Box::pin(
                LazySink::new(move || {
                    Box::pin(async move {
                        eprintln!(
                            "as_bincode_sink: connecting to {remote_ip_address}:{remote_port}"
                        );
                        ...
                        eprintln!(
                            "as_bincode_sink: connected to {remote_ip_address}:{remote_port}"
                        );
    ...
    {
        eprintln!("as_bincode_source: key: {key}, {:?}", self.ports.borrow());
        ...
        async move {
            ...
            eprintln!("as_bincode_source: connecting to {remote_ip_address}:{remote_port}");

            let stream = TcpStream::connect(format!("{remote_ip_address}:{remote_port}"))
                .await
                .unwrap();

            eprintln!("as_bincode_source: connected to {remote_ip_address}:{remote_port}");
        ...
        for (_, _, process) in nodes.get_all_processes() {
            ...
            *process.docker_container_name.borrow_mut() = Some(docker_container_name.clone());
            ...
            let config = Config {
                ...
                env: Some(vec![
                    format!("CONTAINER_NAME={docker_container_name}"),
...
impl<'a> Deploy<'a> for DockerDeploy {
    ...
    fn m2m_connect(
        ...
        Box::new(move || {
            eprintln!("m2m_connect thunk: {serialized}");
        })
    }
    ...
    // ) -> syn::Expr {
    //     eprintln!(
    //         "DockerDeploy: e2o_source: {}:{p1_port:?} -> {}:{p2_port:?}. connection_info: {:?}, reverse_connection_info: {:?}",
    //         p1.name,
    //         p2.name,
    //         p1.connection_info.borrow_mut(),
    //         p1.reverse_connection_info.borrow_mut()
    //     );
    ...
    //             *p2_port,
    //             p2.network.clone(), // TODO: definitely don't need to include the network here.
    //         ),
    //     );

    //     p2.exposed_ports.borrow_mut().push(*p2_port);

    //     deploy_containerized_e2o(*p2_port)
    // }
    ...
    //     eprintln!(
    //         "DockerDeploy: o2e_sink: {}:{p1_port:?} -> {}:{p2_port:?}. connection_info: {:?}, reverse_connection_info: {:?}",
    //         p1.name,
    //         p2.name,
    //         p2.connection_info.borrow_mut(),
    //         p2.reverse_connection_info.borrow_mut()
    //     );
    //     p1.exposed_ports.borrow_mut().push(*p1_port);
    //     p2.reverse_connection_info.borrow_mut().insert(
    //         *p2_port,
    //         (
    //             p1.docker_container_name.clone(),
    //             *p1_port,
    //             p1.network.clone(),
    //         ),
    //     );

    //     deploy_containerized_o2e(*p1_port)
    // }
    ...
    fn e2o_source(
        ...
        let create_expr =
            deploy_containerized_external_sink_source_ident(socket_ident.clone(), shared_handle);
    ...
    {
        cluster_membership_stream(env.to_owned(), location_id)
    }
}
...
fn get_docker_image_name(
    ...
    format!("hy-{image_unique_tag}-{deployment_instance}-{location}")
}
...
impl<'a> ExternalSpec<'a, DockerDeploy> for DockerDeployExternalSpec {
    fn build(self, _id: usize, _name_hint: &str) -> <DockerDeploy as Deploy<'a>>::External {
        DockerDeployExternal {
            ...
            reverse_connection_info: Rc::new(RefCell::new(HashMap::new())),
        }
    }
}
