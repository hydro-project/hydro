...
use crate::compile::deploy_provider::{Deploy, ExternalSpec, Node, ProcessSpec, RegisterPort};
use crate::deploy::trybuild::create_graph_trybuild;
...
impl DockerDeploy {
    ...
    pub async fn provision(&self) -> Result<(), anyhow::Error> {
        let processes = self.docker_processes.borrow().clone();

        for docker_process in processes {
...
---"#
    ...
    pub async fn start(&self) -> Result<(), anyhow::Error> {
        ...
        let processess = self.docker_processes.borrow().clone();
        ...
        for process in processess {
            ...
            let container_name = process.docker_container_name.borrow().clone().unwrap();
            docker
    ...
    pub async fn stop(&self) -> Result<(), anyhow::Error> {
        ...
        let container_names: Vec<_> = self
            .docker_processes
            .borrow()
            .iter()
            .filter_map(|process| process.docker_container_name.borrow().clone())
            .collect();
