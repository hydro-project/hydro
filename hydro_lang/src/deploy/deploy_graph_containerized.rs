    ...
    pub async fn provision(&self, nodes: &DeployResult<'_, Self>) -> Result<(), anyhow::Error> {
        for (_, _, process) in nodes.get_all_processes() {
            ...
            let build_output = match build_crate_memoized(
                rust_crate
                    .get_build_params(hydro_deploy::HostTargetType::Linux(LinuxCompileType::Musl)),
            )
            .await
            {
...
impl<'a> Deploy<'a> for DockerDeploy {
    ...
    fn m2m_sink_source(
        ...
        let serialized = format!(
            "m2m_sink_source {}:{c1_port:?} -> {}:{c2_port:?}",
            c1.name, c2.name
        );
        eprintln!("{serialized}");
        ...
        deploy_containerized_m2m::<Self::Cluster>(*c2_port, bind_addr.as_str())
    }
