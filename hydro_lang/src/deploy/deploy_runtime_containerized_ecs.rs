            ...
            async move {
                ...
                let stream = loop {
                    match TcpStream::connect(target).await {
                        Ok(s) => break s,
                        Err(e) => {
                            debug!(name: "connect_retry", %target, error = %e);
            ...
            q!(sinktools::demux_map_lazy::<_, _, _, _>(
                move |key: &TaglessMemberId| {
                    ...
                    LazySink::<_, _, _, bytes::Bytes>::new(move || {
                        Box::pin(async move {
                            let port = port;
                            let target = format!("{}:{}", key.get_container_name(), port);
                            debug!(name: "connecting", %target);
...
pub fn cluster_self_id<'a>() -> impl QuotedWithContext<'a, TaglessMemberId, ()> + Clone + 'a {
    q!(TaglessMemberId::from_container_name(format!(
        "{}.hydro-{}.local",
            ...
            async move {
                ...
                if !task_arns.is_empty() {
                    ...
                    for task in task_details.tasks() {
                        ...
                        // Construct full Cloud Map DNS name
                        let dns_name = format!("{}.hydro-{}.local", container_id, deployment_instance);
                        trace!(name: "running_task", %container_id, %dns_name);
                        current_tasks.insert(dns_name.clone());
                        if !known_tasks.contains(&dns_name) {
                            trace!(name: "container_joined", %container_id, %dns_name);
                            events.push((dns_name, MembershipEvent::Joined));
                        }
                    }
                }

                for dns_name in known_tasks.iter() {
                    if !current_tasks.contains(dns_name) {
                        trace!(name: "container_left", %dns_name);
                        events.push((dns_name.to_owned(), MembershipEvent::Left));
                    }
                }
