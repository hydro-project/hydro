...
use hydro_lang::{ir::{traverse_dfir, HydroLeaf, HydroNode}, location::LocationId, stream::{deserialize_bincode_with_type, serialize_bincode_with_type}};
use serde::{Deserialize, Serialize};
use syn::visit_mut::{self, VisitMut};

use crate::{partition_syn_analysis::StructOrTupleIndex, repair::inject_id, rewrites::{get_network_type, ClusterSelfIdReplace, NetworkType}};
                            ...
                            for segment in path_expr.path.segments.iter_mut() {
                                ...
                                if ident.starts_with(&prefix) {
                                    ...
                                    *expr = syn::parse_quote!({
                                    ...
                                    println!("Partitioning: Replaced cluster members at node {}", self.op_id);
    ...
    if let Some(indices) = nodes_to_partition.get(&next_stmt_id) {
        ...
        let f: syn::Expr = if new_cluster_id.is_some() {
            ...
            let new_output_type: syn::Type = syn::parse_quote! {
                (ClusterId<()>, #original_output_type)
            };
            ...
            syn::parse_quote!(
                |struct_or_tuple| {
                    ...
                    (
                        ClusterId::<()>::from_raw((partition_val % #num_partitions as u32) as u32),
                        struct_or_tuple
                    )
                }
            )
...
fn replace_receiver_src_id(node: &mut HydroNode, partitioner: &Partitioner, op_id: usize) {
    ...
    } = node
    {
        if input.metadata().location_kind.root().raw_id() == *location_id {
            println!("Rewriting network on op {} so the sender's ID is mapped from the partition to the original sender", op_id);
    ...
    if let Some(network_type) = get_network_type(node, new_cluster_id.unwrap()) {
        ...
        match network_type {
            NetworkType::Send | NetworkType::SendRecv => {
                println!("Replacing deserialize function for op {} to include partitioned cluster ID", op_id);
                let unit_type: syn::Type = syn::parse_quote! { () }; // Assume we are a Cluster<()>
                let new_deserialize_fn = deserialize_bincode_with_type(
                    Some(&unit_type),
                    &output_type,
                );
        ...
        match network_type {
            NetworkType::Recv | NetworkType::SendRecv => {
                println!("Replacing serialize function for op {} to include partitioned cluster ID", op_id);
        ...
        // Update the output type to include the ClusterId
        let new_output_type: syn::Type = syn::parse_quote! {
            (ClusterId<_>, #output_type)
        };
        metadata.output_type = Some(new_output_type.into());
    }
}

fn replace_process_location_id(location: &mut LocationId, process_location: usize, cluster_location: usize) {
    if location.root().raw_id() == process_location {
        location.swap_root(LocationId::Cluster(cluster_location));
    }
}

fn replace_process_input_persist_location_id(input: &mut Box<HydroNode>, process_location: usize, cluster_location: usize) {
    if let HydroNode::Persist {
        metadata: persist_metadata,
        ..
    } = input.as_mut()
    {
        replace_process_location_id(&mut persist_metadata.location_kind, process_location, cluster_location);
    }
}
...
fn replace_process_node_location(node: &mut HydroNode, partitioner: &Partitioner) {
    ...
    if let Some(new_id) = new_cluster_id {
        ...
        // Modify the metadata
        replace_process_location_id(&mut node.metadata_mut().location_kind, *location_id, *new_id);
    }
}
...
fn replace_process_leaf_location(leaf: &mut HydroLeaf, partitioner: &Partitioner) {
    ...
    if let Some(new_id) = new_cluster_id {
        ...
        // Modify the metadata
        replace_process_location_id(&mut leaf.metadata_mut().location_kind, *location_id, *new_id);
    }
}
