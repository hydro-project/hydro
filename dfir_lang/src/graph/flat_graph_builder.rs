...
mod test {
    ...
    /// Test that `spin()` (a `FloType::Source`) is rejected inside a `loop { ... }` context — like
    /// all sources, it must live at the root level and be windowed into the loop.
    #[test]
    fn test_spin_source_in_loop_banned() {
        let mut builder = FlatGraphBuilder::new();
        let loop_id = builder.insert_loop(None);
        builder.add_dfir(
            parse_quote! {
                spin() -> for_each(std::mem::drop);
            },
            Some(loop_id),
            None,
        );
        ...
        let Err(diagnostics) = builder.build() else {
            panic!("Should fail to build: `spin()` is a source and cannot be inside a loop.");
        };
        assert!(
            diagnostics.iter().any(|diagnostic| {
                Level::Error == diagnostic.level && diagnostic.message.contains("root level")
            }),
            "Expected a source-must-be-root-level error, got: {:?}",
            diagnostics,
        );
    }

    /// Test that `iter_ref()` (a `FloType::Source`) is rejected inside a `loop { ... }` context.
    #[test]
    fn test_iter_ref_source_in_loop_banned() {
        let mut builder = FlatGraphBuilder::new();
        builder.add_dfir(
            parse_quote! {
                my_buf = source_iter([1, 2, 3]) -> handoff();
                my_buf -> for_each(std::mem::drop);
            },
            None,
            None,
        );
        let loop_id = builder.insert_loop(None);
        // Parse from a string so the `#my_buf` handoff reference isn't misread as `quote`
        // interpolation (as it would be inside `parse_quote!`).
        builder.add_dfir(
            syn::parse_str::<DfirCode>("iter_ref(#my_buf) -> for_each(std::mem::drop);").unwrap(),
            Some(loop_id),
            None,
        );

        let Err(diagnostics) = builder.build() else {
            panic!("Should fail to build: `iter_ref()` is a source and cannot be inside a loop.");
        };
        assert!(
            diagnostics.iter().any(|diagnostic| {
                Level::Error == diagnostic.level && diagnostic.message.contains("root level")
