...
///
/// # Safety
/// This function uses non-deterministic time-based samples, and also reports histogram data
...
) -> Singleton<(Rc<RefCell<Histogram<u64>>>, Arithmetic<f64>), Cluster<'a, Client>, Unbounded> {
...
pub fn print_bench_results<'a, L: Location<'a> + NoTick + NoAtomic>(
    results: Singleton<(Rc<RefCell<Histogram<u64>>>, Arithmetic<f64>), L, Unbounded>,
) {
    unsafe {
