...
a) better matches how existing distributed systems reason about concurrent execution
b) simplifies the type system for locations and associated cognitive overhead
c) reduces the use of "ticks" across several different patterns
...
We also need to send out acknowledgements for each increment request. A naive (and natural) approach is to `clone()` the stream of increment requests so that each request is used to both update the counter and generate and acknowledgement. The updated code might look like this:
...
1. We introduced one new use of `batch()` for the increment acks. Each `nondet!()` comes with a massive congnitive burden for the developer and weakens Hydro's safety story.
...
1) To ensure atomic processing of elements with `.atomic(...)` as above
...
let current_count: Singleton<usize, Atomic<Process>, Unbounded> =
    increment_atomic.values().fold_commutative(q!(|| 0), q!(|c, _| *c += 1));
let increment_ack = increment.end_atomic();
```
...
We can do this by first declaring an `AlignPoint`:
```rust
let consistent_count_align = process.align_point();
```
...
And finally, we need to make sure that the acknowledgement is aligned with the count. We do this with a `release_aligned` API on streams that only releases elements downstream the other alignments are up-to-date (i.e. all future snapshots of `current_count` incorporate the increment request we are ack-ing):
```rust
...
1. Find all IR nodes that are upstream (transitive inputs) of at least two instances of the alignment point
