...
HydroLean/
  ...
  Programs/          -- verified Hydro programs (Rust ports; every program is its
    ...
    Paxos/           -- paxos.rs port (goal c): one file per Rust function over the
                     -- decisions-as-inputs surface; the headline is paxos_core's
                     -- TYPE (SlotFunctional bundle, PaxosCore.lean);
...
5. ✅ (c) Paxos: 1:1 port, the headline carried on `paxos_core`'s output
   type (the `SlotFunctional` bundle, `Programs/Paxos/PaxosCore.lean` —
   `#print axioms paxos_core`: standard axioms only), and the executable
