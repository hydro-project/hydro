...
1. The assembly layer computed over run views reconstructing
   ...
   `SPWireDiscipline` carriers), and the final assembly is one induction
...
`paxos_core` returns its replicas output at
`{out : Fin nP → Stream (Nat × Option P) //
  variant = .guarded → nA ≤ 2 * f + 1 → SlotFunctional out}` —
the safety guarantee is a **field of the return value**, so the D23
...
separate headline theorem left** (consumers project `.2.property`;
`#print axioms paxos_core` audits the face because the definition contains
the proof). Three ingredients make it work:
...
1. **The `fold_monotonic` pattern at whole-program scale**: the value
   ...
   byte-identical output through `.val`.
