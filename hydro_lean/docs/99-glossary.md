...
| write-state-then-ack (`yield_atomic`) | write-before-ack `acceptor_p2` contract (`AP2Ensures.ok_spec`), transported through the `a_log` knot (`alog_succ`, `PaxosCore.lean`) | §4.5.2 "atomic processing" |
...
| `nondet!(/** causal justification */)` | a **derived guarantee** when the comment claims one (e.g. leader-ballot stability → `leader_election.ensures.stable`, paxos.rs:186–189, FINDINGS D21), else a **named proof input** of the verified face | — |
