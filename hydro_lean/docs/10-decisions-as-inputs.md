...
below is fully executed**: the headline is carried on `paxos_core`'s output
type (the `SlotFunctional` bundle, `Programs/Paxos/PaxosCore.lean`; axiom
audit in `Paxos/AxCheck.lean` — `#print axioms paxos_core`).
...
- **Monotonicity is type-derived** (`Hydro/Growth.lean`, Step 0): the
  ...
  written with the reader-lifted wire combinators (inline `let`-chains in
  the `Verified` definitions; the named cycle bodies
  `leader_election_bodyM`/`paxos_core_bodyM`), cycles close with
  `MonoMap.fix` (generic
...
- `AcceptorP1.lean`/`AcceptorP2.lean`: the `Verified` artifacts
  `acceptor_p1_ticks`/`acceptor_p2_ticks` (`AP1Ensures`:
  `ok_spec`/`reply_echo`/`reply_dst`/`decode_cap`; `AP2Ensures`:
  `ok_spec` — write-before-ack on the published log output —
  /`log_entry`/`decode_cap`).
...
- `SequencePayload.lean`: `sp_send_step_guarded_eq` (the guarded step via
  ...
  `leader_election`'s contract field `stable`, FINDINGS D21 — no longer a
  proof input). The module's contract is `sequence_payload.ensures`
  (`SPEnsures`: `commit_spec`/`emission_spec`/`log_entry_spec`/
  `emission_functional`, each guarded by the named per-guarantee
  requirement `SPRequires` — exactly `leader_election`'s ensures fields —
  plus the public statement vocabulary `SPEmission`/`SPChosen`).
- `PP1b.lean` exports `p_p1b.ensures` (`PP1bEnsures`, stated over the
  artifact's actual output legs): `leader_len`/`view_promise`/
  `leader_providers` (given the named `PP1bReplyCap`)/`qlogs_pinned`/
  `ballot_stable` — the last given the named `PP1bRequires`, whose
  `solicited_at_follower` field is the caller's solicitation fact fed
  back through the election cycle.
- `PaxosCore.lean` (wiring + contract projection only): the
  `sequence_payload` carrier at each cycle (`pcG`/`spInputs`), `pcReq`
  (the `SPRequires` discharge — one projection literal from
  `leader_election.ensures`), `run_emission_zero`, **`alog_succ`** (the
  `a_log` knot at the signature), wire growth along the chain (`pcG_le`),
  and **`run_promise_covers` (K1)** — zero event induction.

## Remaining plan — **ALL COMPLETED** (see `Programs/Paxos/PaxosCore.lean`)
...
0. ✅ **Type-level monotonicity**: `Hydro/Growth.lean` (`Growth` class,
   ...
   is a `→ₘ` composition `rfl`-equal to its transcription (the `Verified`
   bodies; named cycle bodies `leader_election_bodyM`,
   `paxos_core_bodyM`); the
...
1. ✅ **Run-level chain instantiation**: `SPChainQ` (SPChain + frozen-bucket
   ...
   from the `SPRequires` hypotheses, which `PaxosCore.pcReq` discharges
   from `leader_election.ensures` (`own`, `stable` — the trigger's
   `!p_is_leader` gate crosses the `forward_ref` cycle, FINDINGS D21 —,
   `view_promise`, `pinned`); the pinned-view map (`spPin`, classical
   choice) is SP-internal, well-defined by the requirement's pin clause ←
   `p_p1b.ensures.qlogs_pinned` ← `foldEarlyStop_full_pin` through
   `snapshotC_getElem_prefix`.
2. ✅ **K2/K3 counting**: the guarded-fan-in nodup ghost (B1 + ownership
   disjointness, inside `leader_election`) → `acceptor_p1`'s `decode_cap`
   → `PP1bReplyCap`; the P2a-key nodup ghost (inside `sequence_payload`)
   → `acceptor_p2`'s `decode_cap`; `SPChosen` via `SPEnsures.commit_spec`
   (`collect_quorum_distinct_members` + `AP2Ensures.ok_spec`) and
   `LEEnsures.providers`
...
3. ✅ **K4**: `emission_chosen_agree` — the ONE `Nat`-induction over the
   ...
   regress `k+1 → k` through `alog_succ`/`AP2Ensures.log_entry`/
   `batchC_mem`. Base case `run_emission_zero` (the empty cycle realizes
   no acceptor tick).
4. ✅ **Agreement**: `SPEnsures.commit_spec` (through the unconditional
   `joinResponses_zip_elim`) + the type-carried face (`fix_slot_functional`
   paid at `paxos_core`'s definition, `Paxos/PaxosCore.lean`);
