...
running with the bugfix flag and intersecting quorums implies
`SlotFunctional`: cross-proposer slot-functionality of `p_to_replicas`
over the **full**
decision space — every batching/snapshot/shuffle decision, every client
payload timing, every cycle-unfolding depth. There is no separate headline
theorem: consumers project `.2.property`, the value components stay
definitionally the 1:1 transcription (the `fold_monotonic` pattern —
the proof is attached at the boundary), and `#print axioms paxos_core`
audits the face itself. The named proof inputs of the
