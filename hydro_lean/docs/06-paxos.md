...
## The headline (proven; it IS `paxos_core`'s type — `Programs/Paxos/PaxosCore.lean`)
...
`paxos_core` is one `Verified` artifact — the map (`.f`, the 1:1 dataflow
transcription; `.mono`, prefix-monotonicity in the client callback) plus
its contract (`.ensures`): running with the bugfix flag and intersecting
quorums implies `SlotFunctional`, cross-proposer slot-functionality of
`p_to_replicas` over the **full** decision space — every
batching/snapshot/shuffle decision, every client payload timing, every
cycle-unfolding depth. There is no separate headline theorem: consumers
project `(paxos_core …).ensures cb rfl hnA`, the map stays definitionally
the 1:1 transcription (the `fold_monotonic` pattern — the proof is
attached at the boundary), and `#print axioms paxos_core` audits the face
itself. The named proof inputs of the
...
(`leader_election.ensures.stable` — the election trigger reads `!p_is_leader`
...
- **K2/K3** (`PaxosCore.lean` handoffs): consumed-P1a nodup (guarded send-once
  ...
  vote caps; `SPChosen` / `LEEnsures.providers` instantiate the
  `collect_quorum` faces to get `f+1` **distinct** members on both sides;
  frozen buckets (`p_p1b.ensures.qlogs_pinned`) pin a single quorum view
  per ballot (`LEEnsures.pinned`).
- **K4** (`emission_chosen_agree`): the ONE `Nat`-induction over the unfolding
  ...
  `k+1 → k` through `alog_succ`/`AP2Ensures.log_entry`/`batchC_mem`. Ballots may not
...
- **`guarded`** applies the minimal fixes (P1a once per ballot; recommit
  once per ballot acquisition) and satisfies the type-carried face; on the same
