...
> is **derived** in the unmodified model (`leader_election.ensures.stable`,
> `Programs/Paxos/LeaderElection.lean`) — the type-carried face's only
> remaining input is `nA ≤ 2f + 1`. Artifacts: `Hydro/CausalAvail.lean` (the shelved
...
the safety face. It materializes the paxos.rs:185–188 `nondet_commit`
...
- Leader-ballot stability is a **theorem of the current model** —
  `leader_election.ensures.stable`, exported as a module contract field
  (`LeaderElection.lean`); `hstab` is gone from the safety face (final
  form: `paxos_core`'s output type, hypothesis `nA ≤ 2f+1` only). The "causality from the fixpoint" instinct is
...
- The derivation is protocol-specific: it leans on the trigger gate, view
  ...
  2. `le_trigger_gate` + `p_leader_heartbeat.ensures.trigger_gate`
     (`PLeaderHeartbeat.lean`): a trigger-true tick reads a `false` leader
     flag off the `forward_ref` cycle input (paxos.rs:449).
  3. the `hbpromise`/`hpersists`/`hmask` ghost `have`s +
     `p1bMaxQuorumBallot_ge` (`PP1b.lean`): full buckets are genuinely
     promised, persist along the accumulated cuts, and can only be masked
     by a strictly larger full own bucket (`p_has_largest_ballot ≡ true`
     at realized ticks — `p_ballot_calc.ensures.hasLargest_true`, now
     consumed).
  4. the `hflag_prefix`/`hsol` ghost `have`s (`LeaderElection.lean`):
     the cycle's flag input is a prefix of the run's flag output (realized
     ticks are final across the unfolding); every `Ok`-promised ballot was
     solicited at a flag-false tick (`PP1bRequires.solicited_at_follower`,
     the module's named solicitation requirement).
  5. the `hnff` regress ghost + `p_p1b.ensures.ballot_stable`
     (`PP1b.lean`): the mask regress — strong induction on the remaining
     ticks; fabrication levels strictly ascend on the finite run —
     exported as a `p_p1b` contract field over the named `PP1bRequires`.
...
1. **C not adopted.** The protocol-level derivation of `LeaderBallotStable`
   shipped instead (`leader_election.ensures.stable`; the safety face carries
