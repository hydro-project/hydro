> **Historical note.** These docs describe the retired V1 generation
> (`HydroLean/Hydro` + `HydroLean/Programs`, decisions-as-inputs over core
> Lean). That code has been ported to **`HydroV2/`** (tagless-final
> signature, graded quotient carriers, multiple interpretations incl. the
> executable `Eager`); start at `HydroV2/README.md`. The metatheory docs
> (`01-metatheory.md`, Flo/Gyatso) remain current.
...
- **(c) Paxos — safety proven.** The full `paxos_core` port
  ...
  decisions-as-inputs surface, docs/10), the headline **carried on
  `paxos_core`'s output type**
  (`Programs/Paxos/PaxosCore.lean`: the replicas stream returns at the
  `SlotFunctional` bundle — cross-proposer slot-functionality of
...
2. [06-paxos.md](06-paxos.md) — the capstone: the Paxos port, the
   type-carried safety face, and the executable falsifications.
3. [11-causal-availability.md](11-causal-availability.md) — the causality
   ...
   fixpoint (`leader_election.ensures.stable` derived); the shelved
   `causalTickLoop`
