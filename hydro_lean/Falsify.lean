...
wiring, `sequence_payload` — applied at the boundary via `.f`) and forces
...
`paxos_core_bodyM`/`leader_election_bodyM` line by line.
-/
...
def paxosStepIO (variant : PaxosVariant) (r : PaxosRef Nat 2 3) :
  ...
  let sp := (sequence_payload variant 1 spN).f
