> **Status update (V1 retirement).** The `HydroLean/Hydro` + `HydroLean/Programs`
> generation this bar was ratified against has been **retired and ported to
> `HydroV2/`** (the tagless-final generation): the B1/B2 falsifiers live in
> `HydroV2/Paxos/Falsification.lean` (`lake exe falsify`, at the `Eager`
> interpretation), the D21 exploration in `HydroV2/Paxos/Exploration.lean`
> (`lake exe explore`), 2PC in `HydroV2/TwoPC.lean` (`lake exe v2twopc`),
> and the Paxos headline in `HydroV2/Paxos/PaxosCore.lean` +
> `Paxos/CoupleWf.lean` (`paxos_safe_sched'`; the earlier square-coupling
> home `SquareSafety.lean` is retired — FINDINGS D42). `Flo/` and
> `Gyatso/` (dissertation ch. 2–3) remain.
> File references below to `Programs/…` are historical (jj history).
...
- **(c) Paxos** ✅ — 1:1 port of `hydro_test/src/cluster/paxos.rs`
  ...
  which **is `paxos_core`'s type** (`Programs/Paxos/PaxosCore.lean`):
  ...
  `paxos_core` is one `Verified` artifact — dataflow (`.f`), monotonicity
  in the client callback (`.mono`), and the guarantee (`.ensures`: the
  bugfix flag ⇒ quorum intersection ⇒ slot-functionality): there is no
  separate headline theorem — consumers project
  `(paxos_core …).ensures cb rfl hnA`, the map stays definitionally the
  1:1 transcription, and `#print axioms paxos_core` audits the face
  itself. `SlotFunctional out` =
  ...
  statement (§5).
- Axiom audits: every deliverable named above is `#print axioms`-audited in
  `Programs/AxCheck.lean`, `Programs/Paxos/AxCheck.lean`, `Flo/AxiomCheck.lean`
  (for (c), auditing the `paxos_core` definition audits the proof it
  carries).
...
- ~~**Deriving `LeaderBallotStable`**~~ — RESOLVED: derived as
  `leader_election.ensures.stable` (LeaderElection's module contract; no time-indexed
