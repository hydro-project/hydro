...
- **(c) Paxos** ✅ — 1:1 port of `hydro_test/src/cluster/paxos.rs`
  ...
  (the bugfix flag ⇒ quorum intersection ⇒ slot-functionality): there is no
  separate headline theorem — consumers project `.2.property`, the value
  components stay definitionally the 1:1 transcription, and
  `#print axioms paxos_core` audits the face itself. `SlotFunctional out` =
