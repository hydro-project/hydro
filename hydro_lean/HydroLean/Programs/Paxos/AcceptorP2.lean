...
establishes as a system invariant (`sp_emission_functional`, consumed by
the assembly in `Paxos/PaxosCore.lean`); the Rust TODO is
...
- outputs: coverage-monotonicity of the `a_log` wire is **carried by
  `acceptor_p2_ticks`'s output type** (`MonoSing covVOc` — the manual
...
`acceptor_p2_ticks` is a `Verified` map: the `→ₘ` stage together with its
contract (`AP2Ensures`). No separate unverified version, no separate
contract theorem. -/

/-- What `acceptor_p2` **ensures**, on its ticks signature: the output
pair (coverage-`Monotonic` published log, ack batches) against the inputs
`(p2a, am)`. -/
structure AP2Ensures (p2a : TStream (P2a P nP))
    (am : TSing (Option (Ballot nP)))
    (out : MonoSing (covVOc (P := P) (nP := nP))
      × TStream (Fin nP × P2b nP)) : Prop where
  /-- `Ok`-ack (write-before-ack): an `Ok` ack pins a tick — the
  ...
  ok_spec : ∀ {dm : Fin nP × P2b nP}, dm ∈ out.2.flatten →
    dm.2.res = .ok () →
    ∃ (t : Nat) (hta : t < am.length) (htl : t < out.1.vals.length),
      am[t]'hta = some dm.2.ballot ∧
      LogCovers ((out.1.vals[t]'htl).2) dm.2.slot dm.2.ballot
  /-- Log-entry echo: every entry of the published log quotes a consumed
  P2a. -/
  log_entry : ∀ {t : Nat} (htl : t < out.1.vals.length) {slot : Nat}
    {e : LogValue P nP},
    (slot, e) ∈ ((out.1.vals[t]'htl).2 : LogMap P nP) →
    ∃ m ∈ p2a.flatten, slot = (m : P2a P nP).slot ∧ e = ⟨m.ballot, m.value⟩
  /-- Decode cap: with duplicate-free `(slot, ballot)` keys on the P2a
  input (the B2 once-per-ballot fan-in requirement, named), the demuxed
  ack slice carries at most one `Ok` vote per key. -/
  decode_cap : ((p2a.flatten).map
...
/-- **paxos.rs:826–884 `acceptor_p2`, ticks-lifted**: the single verified
artifact. -/
def acceptor_p2_ticks :
    Verified (TStream (P2a P nP) × TSing (Option (Ballot nP)))
      (MonoSing (covVOc (P := P) (nP := nP)) × TStream (Fin nP × P2b nP))
      (fun x out => AP2Ensures x.1 x.2 out) :=
  Verified.ofMono
    (let p2a := MonoMap.fst
    ...
    { ok_spec := fun {dm} hdm hok => by
        ...
        have hsl : t < ((acceptorP2Loop P nP).states
          ...
          exact ht
        have htl : t < (((acceptorP2Loop P nP).states
            ((TSing.zip p2a am).map fun bm => AP2In.mk bm.1 bm.2 none)).map
              (fun log => ((none : Option Nat), log))).length := by
        ...
        · -- write-before-ack at the ack's own tick
          ...
          have hval : (((acceptorP2Loop P nP).states
            ((TSing.zip p2a am).map fun bm => AP2In.mk bm.1 bm.2 none)).map
              (fun log => ((none : Option Nat), log)))[t]'htl
            ...
            rfl
          show LogCovers (((((acceptorP2Loop P nP).states
            ((TSing.zip p2a am).map fun bm => AP2In.mk bm.1 bm.2 none)).map
              (fun log => ((none : Option Nat), log)))[t]'(by
              rw [List.length_map]; exact hsl)).2) dm.2.slot dm.2.ballot
          rw [hval, hdms, hdmb]
          exact hcov
      log_entry := fun {t} htl {slot} {e} hE => by
        obtain ⟨p2a, am⟩ := x
        have htl' : t < (((acceptorP2Loop P nP).states
            ((TSing.zip p2a am).map fun bm => AP2In.mk bm.1 bm.2 none)).map
              (fun log => ((none : Option Nat), log))).length := htl
        have hsl : t < ((acceptorP2Loop P nP).states
            ((TSing.zip p2a am).map fun bm => AP2In.mk bm.1 bm.2 none)).length := by
          have := htl'
          rwa [List.length_map] at this
        have hval : (((acceptorP2Loop P nP).states
          ...
          rw [List.getElem_map,
            (acceptorP2Loop P nP).states_getElem _ t hsl]
          rfl
        have hE' : (slot, e) ∈ (((((acceptorP2Loop P nP).states
            ((TSing.zip p2a am).map fun bm => AP2In.mk bm.1 bm.2 none)).map
              (fun log => ((none : Option Nat), log)))[t]'htl').2
            : LogMap P nP) := hE
        rw [hval] at hE'
      ...
      decode_cap := fun hnd i slot b => by
        obtain ⟨p2a, am⟩ := x
