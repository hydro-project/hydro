...
was needed for the headline — the type-carried face dropped `hstab` via the
protocol-level derivation (now `le_ballot_stable`, `LeaderElection.lean`:
the well-founded regress over fabricated-ballot levels
