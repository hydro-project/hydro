...
instantiations of the leader-view contracts, the base-case emptiness, the
K1 composition (`run_promise_covers` — promise payloads cover chosen
keys of lower ballots, through the knot), and the final assembly
(`emission_chosen_agree`, `fix_slot_functional` — the proof `paxos_core`
carries on its output type). Every step consumes a module contract at the
signature or the modules' abstract witnesses (`SPEmission`, `SPChosen`) —
...
theorem fix_out_eq (i : Fin nP) :
    (MonoMap.fix₀ nd.fuel PaxosRef.init
        (paxos_core_bodyM G f cbM nd)).2 i
...
theorem fix_slot_functional (hnA : nA ≤ 2 * f + 1) :
    SlotFunctional
      (MonoMap.fix₀ nd.fuel PaxosRef.init
        (paxos_core_bodyM G f cbM nd)).2 := by
...
of the typed body, returning `(new-leader ballots, p_to_replicas)` per
proposer **with the verified face on the output type** — THE HEADLINE
(deliverable (c)): like the inner modules' contracts and the `Monotonic`
wires, the replicas output carries its guarantee as part of its type —
...
(`.2.property rfl hnA`), and `#print axioms paxos_core` audits the face
...
paxos.rs:186–189 leader-ballot stability (`le_ballot_stable`) are all
**derived** (they are the guarded variant plus the election protocol's own
trigger gate), not assumed; the client callback's prefix-monotonicity is
carried by its `→ₘ` type. The faithful variant falsifies the same
statement (`Paxos/Falsification.lean`).

The value components stay definitionally the 1:1 transcription (the proof
is attached at the boundary — the `fold_monotonic` pattern — never
...
def paxos_core (variant : PaxosVariant) (f : Nat)
    ...
    (Fin nP → Stream (Ballot nP))
      × {out : Fin nP → Stream (Nat × Option P) //
          variant = .guarded → nA ≤ 2 * f + 1 → SlotFunctional out} :=
  ((MonoMap.fix₀ nondet.fuel PaxosRef.init
      (paxos_core_bodyM variant f c_to_proposers nondet)).1,
   ⟨(MonoMap.fix₀ nondet.fuel PaxosRef.init
      (paxos_core_bodyM variant f c_to_proposers nondet)).2,
    fun hv hnA => by
      subst hv
      exact fix_slot_functional f c_to_proposers nondet hnA⟩)
