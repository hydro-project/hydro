...
- the max ballot only grows — **carried by the output type** of
  `acceptor_p1_ticks` (`MonoSing obtVO`, the `across_ticks(max)` fold with
...
`acceptor_p1_ticks` is a `Verified` map: the `→ₘ` stage (prefix-monotone by
inheritance, usable in fixpoint bodies via `.toMonoMap`) together with its
contract (`AP1Ensures`, reached by `.ensures`). There is no separate
unverified version and no separate contract theorem. -/

/-- What `acceptor_p1` **ensures**, on its ticks signature: the output
pair (`Monotonic` max wire, reply batches) against the inputs
`(p1a, al)`. -/
structure AP1Ensures (p1a : TStream (Ballot nP))
    (al : TSing (Option Nat × LogMap P nP))
    (out : MonoSing (obtVO (nP := nP)) × TStream (Fin nP × P1b P nP)) :
    Prop where
  /-- `Ok`-promise: an `Ok` reply pins a tick — the carried payload is the
  `a_log` **input** value at that tick, and the max wire carries exactly
  the promised ballot there. -/
  ok_spec : ∀ {dm : Fin nP × P1b P nP}, dm ∈ out.2.flatten →
    ∀ {pay : Option Nat × LogMap P nP}, dm.2.res = .ok pay →
    ∃ (t : Nat) (htl : t < al.length), pay = al[t]'htl ∧
      ∃ hm : t < out.1.vals.length,
        out.1.vals[t]'hm = some dm.2.ballot
  /-- Reply echo: every reply quotes a consumed P1a. -/
  reply_echo : ∀ {dm : Fin nP × P1b P nP}, dm ∈ out.2.flatten →
    dm.2.ballot ∈ p1a.flatten
  /-- Reply routing: every reply is addressed to its ballot's owner
  (paxos.rs:329). -/
  reply_dst : ∀ {dm : Fin nP × P1b P nP}, dm ∈ out.2.flatten →
    dm.1 = dm.2.ballot.proposerId
  /-- Decode cap: with duplicate-free P1a input (the B1 send-once fan-in
  requirement, named), the demuxed reply slice carries at most one `Ok`
  promise per ballot. -/
  decode_cap : (p1a.flatten).Nodup → ∀ (i : Fin nP) (b : Ballot nP),
    ((out.2.flatten).filterMap
      (fun dm => if dm.1 = i then
        some (dm.2.ballot, dm.2.res) else none)).countP
      (fun e => decide (e.1 = b) && e.2.isOk) ≤ 1

/-- **paxos.rs:485–524 `acceptor_p1`, ticks-lifted**: the single verified
artifact — `.f`/`.mono` the transcription stage, `.ensures` the contract,
paid here once. -/
def acceptor_p1_ticks :
    Verified (TStream (Ballot nP) × TSing (Option Nat × LogMap P nP))
      (MonoSing (obtVO (nP := nP)) × TStream (Fin nP × P1b P nP))
      (fun x out => AP1Ensures x.1 x.2 out) :=
  Verified.ofMono
    (let p1a := MonoMap.fst
    ...
    { ok_spec := fun {dm} hdm {pay} hok => by
        ...
        have hsc : t < (scanSt (fun s b => aMaxBallot s b) none p1a).length := by
          rw [scanSt_length]
          exact htb
        have hm : t < (scanSt (fun s b => aMaxBallot s b) none p1a).length := hsc
        refine ⟨t, htl, hpay.trans hlog, hm, ?_⟩
        show (scanSt (fun s b => aMaxBallot s b) none p1a)[t]'hm
          = some dm.2.ballot
        have hval : (scanSt (fun s b => aMaxBallot s b) none p1a)[t]'hm
      ...
      reply_dst := fun {dm} hdm => by
        obtain ⟨p1a, al⟩ := x
        have hdm' : dm ∈ ((acceptorP1Loop P nP).outputs
            ((TSing.zip p1a al).map fun bl => AP1In.mk bl.1 bl.2)).flatten := hdm
        obtain ⟨t, ht, b', hb', rfl⟩ := acceptorP1_out_elim hdm'
        rfl
      decode_cap := fun hnd i b => by
        obtain ⟨p1a, al⟩ := x
