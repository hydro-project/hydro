...
The `.2.val` projection is the replicas stream (the value component of the
`SlotFunctional`-bundled output — for `runFaithful` the bundle's guarantee
is vacuous, its hypothesis `variant = .guarded` being false).
Verified output (recorded in FINDINGS.md B1):
  runFaithful.2.val 0 = [(0, some 20)]   -- proposer 0 commits y
  runFaithful.2.val 1 = [(0, some 10)]   -- proposer 1 commits x — DISAGREEMENT
  runGuarded.2.val 1  = []               -- B1 fix blocks the fake quorum
-/
