...
/-- The faithful run. -/
def runFaithful := (paxos_core PaxosVariant.faithful 1 nd).f clients
/-- The guarded run, same script. -/
def runGuarded := (paxos_core PaxosVariant.guarded 1 nd).f clients
...
The `.2` projection is the replicas stream; the guarantee is the
`Verified` map's `.ensures` (for the faithful variant its hypothesis
`variant = .guarded` is false — the type claims nothing).
