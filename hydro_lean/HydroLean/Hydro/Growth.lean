...
/-! ## Verified wires: the type carries the guarantees (Verus-style)

A `Verified α β Ens` is a prefix-monotone map whose **type carries its
guarantees**: `ensures : ∀ a, Ens a (f a)`. `Ens` is typically a
`Prop`-structure with one field per guarantee, each field carrying its own
named preconditions — variadic guarantees, per-guarantee requirements.
Inside a function body, a `let`-bound `Verified` wire transports its facts
to every use site (no external addressing, no device defs); at module
boundaries, the callee's `ensures` rides the call. -/

/-- A prefix-monotone map whose type carries its guarantees. -/
structure Verified (α : Type u) (β : Type v) [Growth α] [Growth β]
    (Ens : α → β → Prop) extends MonoMap α β where
  ensures : ∀ a, Ens a (f a)

namespace Verified

variable {α : Type u} {β : Type v} [Growth α] [Growth β]

/-- Pay the obligation once, at the binding site. -/
def ofMono {Ens : α → β → Prop} (m : MonoMap α β)
    (h : ∀ a, Ens a (m.f a)) : Verified α β Ens :=
  { m with ensures := h }

/-- Weaken the guarantee (consequence). -/
def weaken {Ens Ens' : α → β → Prop} (v : Verified α β Ens)
    (h : ∀ a b, Ens a b → Ens' a b) : Verified α β Ens' :=
  { v with ensures := fun a => h a _ (v.ensures a) }

end Verified

/-! Monotone maps form a growth carrier themselves (pointwise), so a
monotone **callback** can be an input wire: `MonoMap.evalM` applies it,
monotone in both the callback and the argument. This is what lets
`paxos_core` take its client callback as a first-class monotone input and
close the loop with the `Γ`-ful `MonoMap.fix`. -/

/-- Pointwise growth on monotone maps. -/
instance {α : Type u} {β : Type v} [Growth α] [Growth β] :
    Growth (MonoMap α β) where
  le m m' := ∀ a, m.f a ⊑ m'.f a
  le_refl m a := Growth.le_refl (m.f a)
  le_trans h1 h2 a := Growth.le_trans (h1 a) (h2 a)

/-- Application, as a stage: monotone in the callback and the argument. -/
def MonoMap.evalM {α : Type u} {β : Type v} [Growth α] [Growth β] :
    MonoMap (MonoMap α β × α) β where
  f x := x.1.f x.2
  mono h := Growth.le_trans (h.1 _) (MonoMap.mono _ h.2)
