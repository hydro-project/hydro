...
def MonoRel (L : Type) (mem : L → Nat) : HydroSem L mem where
  ...
  scan_batches_unordered bs g init :=
    ⟨(fun i => scanAcrossTicksTrace (g i) init (bs.val.1 i),
      fun i => scanAcrossTicksTrace (g i) init (bs.val.2 i)),
     fun i => scanAcrossTicksTrace_prefix _ init (bs.property i)⟩
  scan_batches_unordered₂ bs cs g init :=
    ⟨(fun i => scanAcrossTicksTrace (fun s bt => g i s bt.1 bt.2) init
        (Trace.zip (bs.val.1 i) (cs.val.1 i)),
      fun i => scanAcrossTicksTrace (fun s bt => g i s bt.1 bt.2) init
        (Trace.zip (bs.val.2 i) (cs.val.2 i))),
     fun i => scanAcrossTicksTrace_prefix _ init
       (zip_prefix (bs.property i) (cs.property i))⟩
