...
/-- An additive multiset map distributes over list sums (the shape
`scan_bound` contributions take at instantiation sites). -/
theorem map_sum_additive {α δ : Type _} (f : Multiset α → Multiset δ)
    (hf0 : f 0 = 0) (hfadd : ∀ x y, f (x + y) = f x + f y) :
    ∀ (bs : List (Multiset α)), (bs.map f).sum = f bs.sum
  | [] => by rw [List.map_nil, List.sum_nil, List.sum_nil, hf0]
  | b :: bs => by
    rw [List.map_cons, List.sum_cons, List.sum_cons, hfadd,
      map_sum_additive f hf0 hfadd bs]
...
theorem map_snd_zip_prefix {α β : Type _} :
    ∀ (a : List α) (b : List β), (List.zip a b).map Prod.snd <+: b
  | [], _ => List.nil_prefix
  | _ :: _, [] => List.nil_prefix
  | x :: xs, y :: ys => by
    rw [List.zip_cons_cons, List.map_cons]
    exact List.cons_prefix_cons.mpr ⟨rfl, map_snd_zip_prefix xs ys⟩
