...
def Values (L : Type) (mem : L → Nat) : HydroSem L mem where
  ...
  scan_batches_unordered_across_ticks bs t g init := fun i =>
    scanAcrossTicksTrace (fun s bt => g i s bt.1 bt.2) init (Trace.zip (bs i) (t i))
  scan_batches_unordered bs g init := fun i =>
    scanAcrossTicksTrace (g i) init (bs i)
  scan_batches_unordered₂ bs cs g init := fun i =>
    scanAcrossTicksTrace (fun s bt => g i s bt.1 bt.2) init (Trace.zip (bs i) (cs i))
  ...
  emitMultisetBatches t _d := fun i => t i
