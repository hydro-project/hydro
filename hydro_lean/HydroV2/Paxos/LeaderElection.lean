...
structure LEDec (H : HydroSem L mem) (nP nA : Nat) (P : Type)
  ...
  hb : PLHDec H nP
  /-- `p_to_acceptors_p1a.broadcast(&acceptors, …)`: P1a transport
  (proposers → acceptors). -/
  ...
  /-- `a_to_proposers_p1b.demux(&proposers, …)`: P1b transport
  (acceptors → proposers). -/
...
a face: it is the generated free theorem `leader_election_param…`
(`monoC`/`monoLaws`, `MonoHRel.lean`), consumed at the
`PaxosCoreLemmas.lean` stage chain. Output tuple: (`p_ballot`,
...
hydro [p1bPairDecEq] def leader_election (H : HydroSem L mem)
  ...
  fix (p1b_fail : H.Stream prop (Ballot (mem prop)) .noOrder
    ...
    let p_to_acceptors_p1a := H.values (H.broadcast dec.p1aCh
    ...
    let ap1S := acceptor_p1 H acc prop
      (H.batch p_to_acceptors_p1a dec.p1aBatch) a_log dec.p1bCh
    let ap1 := ap1S.val
    ...
    -- zip projections at a release tick (shared by the solicitation
    -- chain and the send-once counting)
    ...
    ghost let fails := _root_.HydroV2.leader_election.p1b_fail
      ...
      num_participants dec p_received_p2b_ballots a_log
    ...
    ghost let pass := fun fl ia f =>
      _root_.HydroV2.leader_election.body (P := P) (Values L mem)
        variant prop acc quorum_size num_participants dec
