...
hydro def acceptor_p1 (H : HydroSem L mem) (acc prop : L)
    ...
    (chP1b : H.TransportDec (mem prop) (mem acc)) :
      ...
      ∃ (t : Nat) (hb : t < (p_to_acceptors_p1a j).length)
        ...
        ∧ m.res = (if some m.ballot = (foldAcrossTicksTrace aMaxStep none
            ...
            then Except.ok ((a_log j)[t]'hl)
              ...
              (p_to_acceptors_p1a j))[t]'hm)) := fun j r m hm1 => by
  ...
  (a_max_ballot,
    H.values (H.demux chP1b (H.allTicks replies) (fun r => r.val)))
  prove
    max_face := fun i => rfl,
    reply_decomp := fun r => rfl,
...
theorem acceptor_p1_ok_pins (acc prop : L)
    (bs : Fin (mem acc) → Trace (Multiset (Ballot (mem prop))))
    (alog : TickV (mem acc) (ALog P (mem prop)) .unbounded) (ch : Unit)
...
#guard ((acceptor_p1 (Values PaxLoc (paxMem 1 1)) .acc .prop
    ...
    (fun _ => [(none, []), (none, [(0, ⟨Ballot.mk 1 0, some 42⟩)])]
      : TickV 1 (ALog Nat 1) .unbounded) ()).val.1 0).vals
  = [some (Ballot.mk 1 0), some (Ballot.mk 1 0)]

#guard (acceptor_p1 (Values PaxLoc (paxMem 1 1)) .acc .prop
    (fun _ => [{Ballot.mk 1 0}, {Ballot.mk 0 0}])
    (fun _ => [(none, []), (none, [(0, ⟨Ballot.mk 1 0, some 42⟩)])]
      : TickV 1 (ALog Nat 1) .unbounded) ()).val.2 0
