...
variable (ledec : LEDec (Values L mem) (mem prop) (mem acc) P)
variable (spdec : SPDec (Values L mem) (mem prop) (mem acc) P)
...
def pcAlogStep (al : TickV (mem acc) (ALog P (mem prop)) .unbounded) :
  ...
  (sequence_payload (Values L mem) .guarded prop acc cp ck
    ((Values L mem).forgetBound (leader_election (Values L mem) .guarded
      prop acc (f + 1) (2 * f + 1) ledec sm al).val.1)
    (leader_election (Values L mem) .guarded prop acc (f + 1)
      (2 * f + 1) ledec sm al).val.2.1
    (leader_election (Values L mem) .guarded prop acc (f + 1)
      (2 * f + 1) ledec sm al).val.2.2.1 f
    ((Values L mem).forgetBound (leader_election (Values L mem) .guarded
      prop acc (f + 1) (2 * f + 1) ledec sm al).val.2.2.2)
    spdec).val.2.1
...
def pcLE (m : Nat) :=
  (leader_election (Values L mem) .guarded prop acc (f + 1)
    (2 * f + 1) ledec sm
    (pcAlogW prop acc f cp ck ledec spdec sm m)).val
...
theorem paxos_core_agree (hnA : mem acc ≤ 2 * f + 1) :
  ...
  have hle : ∀ m, LEEnsures .guarded prop acc (f + 1)
    ...
    (leader_election (Values L mem) .guarded prop acc (f + 1)
      (2 * f + 1) ledec sm
      (pcAlogW prop acc f cp ck ledec spdec sm m)).property rfl
    ...
    { own := (hle m).own
      ...
      pinned := fun i {t t'} hpl hpl' hpr hpr' hpb hpb' h1 h2 h3 =>
        (hle m).pinned (by omega) i hpl hpl' hpr hpr' hpb hpb'
          h1 h2 h3 }
  -- Flo monotonicity of the election run in the cycle wires: the
  -- generated free theorem at the Flo instance (`monoC`/`monoLaws`),
  -- one leg per output — the retired hand-written mono face
  have leMono : ∀ (al al' : TickV (mem acc)
      (ALog P (mem prop)) .unbounded), (∀ j, al j <+: al' j) →
      (∀ i, ((leader_election (Values L mem) .guarded prop acc (f + 1)
          (2 * f + 1) ledec sm al).val.1 i).vals
        <+: ((leader_election (Values L mem) .guarded prop acc (f + 1)
          (2 * f + 1) ledec sm al').val.1 i).vals)
      ∧ (∀ i, (leader_election (Values L mem) .guarded prop acc (f + 1)
          (2 * f + 1) ledec sm al).val.2.1 i
        <+: (leader_election (Values L mem) .guarded prop acc (f + 1)
          (2 * f + 1) ledec sm al').val.2.1 i)
      ∧ (∀ i, (leader_election (Values L mem) .guarded prop acc (f + 1)
          (2 * f + 1) ledec sm al).val.2.2.1 i
        <+: (leader_election (Values L mem) .guarded prop acc (f + 1)
          (2 * f + 1) ledec sm al').val.2.2.1 i)
      ∧ (∀ j, ((leader_election (Values L mem) .guarded prop acc (f + 1)
          (2 * f + 1) ledec sm al).val.2.2.2 j).vals
        <+: ((leader_election (Values L mem) .guarded prop acc (f + 1)
          (2 * f + 1) ledec sm al').val.2.2.2 j).vals) := by
    intro al al' hal
    exact ⟨leader_election_param₁ (monoC L mem) monoLaws () .guarded
        prop acc (f + 1) (2 * f + 1) ledec ledec
        (fun _ _ => rfl) (fun _ _ => rfl) (fun _ _ => rfl)
        (fun _ _ => rfl) (fun _ _ => rfl) (fun _ _ => rfl)
        (fun _ _ => rfl) (fun _ _ => rfl) (fun _ _ => rfl)
        (fun _ _ => rfl) (fun _ _ => rfl) (fun _ _ => rfl)
        (fun _ _ => rfl) (fun _ _ => rfl) (fun _ _ => rfl)
        sm sm (fun _ _ _ => le_refl _)
        al al' (fun _ _ => hal) () le_rfl,
      leader_election_param₂ (monoC L mem) monoLaws () .guarded
        prop acc (f + 1) (2 * f + 1) ledec ledec
        (fun _ _ => rfl) (fun _ _ => rfl) (fun _ _ => rfl)
        (fun _ _ => rfl) (fun _ _ => rfl) (fun _ _ => rfl)
        (fun _ _ => rfl) (fun _ _ => rfl) (fun _ _ => rfl)
  ...
  have halog_step : ∀ (al al' : TickV (mem acc)
    ...
    intro al al' hal
    have hLE := leMono al al' hal
    exact (sequence_payload_mono .guarded prop acc _ _ _ _ _ _ _ _
      _ _ f _ _ spdec (fun _ => List.prefix_refl _)
  ...
  have hleM : ∀ {m m' : Nat}, m ≤ m' →
    ...
    fun {m m'} hmm' =>
    leMono (pcAlogW prop acc f cp ck ledec spdec sm m)
      (pcAlogW prop acc f cp ck ledec spdec sm m')
      (halog hmm')
  ...
  have hagree : ∀ (K : Nat) (slot₀ : Nat) (i₁ : Fin (mem prop))
    ...
    | succ k ih =>
      ...
      have hcklen : t₁ < (ck j).length := by
        have h2 : ((pcSP prop acc f cp ck ledec spdec sm k).val.2.1
              j).length ≤ (ck j).length :=
          (hsp k).log_len_le_ck j
        omega
