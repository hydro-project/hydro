...
structure PP1bDec (H : HydroSem L mem) (nP : Nat) (P : Type)
  ...
  /-- `collect_quorum_with_response`'s quorum emission order (the
  freshly-quorum'd pairs entering the wire). -/
  cqwrEmit : H.EmitDec nP (Ballot nP × ALog P nP)
...
structure PP1bEnsures (prop : L) (quorumSize : Nat)
    ...
    (okPool : Fin (mem prop)
      → Multiset (Ballot (mem prop) × ALog P (mem prop)))
  ...
  /-- **The realized success pool** (the witness data): the shared
  quorum stage's emissions embed with multiplicity in the reply pool's
  `Ok` projection — `CQWREnsures.emit_pool_le`, bridged through the
  consumed batches. -/
  ok_pool_le : ∀ (i : Fin (mem prop)),
    okPool i ≤ Multiset.filterMap p1bOkPair (pool i)
  ...
  leader_gate : ∀ (i : Fin (mem prop)) {t : Nat}
    ...
    ∃ (hv : t < (pP1bViews (mem prop) quorumSize
        (okPool i) (dOrd i) (dSnap i)).length)
      (hb : t < (pb i).length) (hl : t < (phl i).length)
      (qlogs : List (ALog P (mem prop))),
      pP1bQuorum quorumSize
          ((pP1bViews (mem prop) quorumSize
            (okPool i) (dOrd i) (dSnap i))[t]'hv) ((pb i)[t]'hb)
        = some qlogs
  ...
  flags_eq : ∀ (i : Fin (mem prop)),
    out.1 i = pP1bFlags (mem prop) quorumSize (okPool i)
      (pb i) (phl i) (dOrd i) (dSnap i)
  ...
  accepted_eq : ∀ (i : Fin (mem prop)),
    out.2.1 i = List.map (fun l => Multiset.ofList l)
      ((((pP1bViews (mem prop) quorumSize (okPool i)
          (dOrd i) (dSnap i)).zip (pb i)).map
...
hydro [p1bPairDecEq] def p_p1b (H : HydroSem L mem) (prop : L)
  ...
  ensures out => ∃ okPool, PP1bEnsures prop quorum_size
    a_to_proposers_p1b p_ballot p_has_largest_ballot okPool
    dec.order dec.snap out :=
  ...
  let cqwrS := collect_quorum_with_response H prop p1b_pairs
    quorum_size num_participants dec.cqwr dec.cqwrEmit
  -- the realized success pool embeds in the reply pool's `Ok`
  -- projection: the shared stage's `emit_pool_le`, bridged through the
  -- consumed batches
  ghost have hok_le : ∀ i, cqwrS.val.1 i
      ≤ Multiset.filterMap p1bOkPair (a_to_proposers_p1b i) := fun i =>
    le_trans ((cqwrS.property rfl).emit_pool_le i)
      (consumed_okProj_le (a_to_proposers_p1b i) (dec.cqwr i))
  -- .into_keyed().assume_ordering::<TotalOrder>(nondet!(…))
  let quorum_outs := H.assume_ordering cqwrS.val.1 dec.order
  ...
  -- the flag face, at the flag wire
  ghost have hflags : ∀ i, p_is_leader i
      = pP1bFlags (mem prop) quorum_size (cqwrS.val.1 i) (p_ballot i)
          (p_has_largest_ballot i) (dec.order i) (dec.snap i) := fun i => by
    unfold pP1bFlags pP1bViews
    rfl
  -- .flatten_unordered(): the selection order cannot leak
  let accepted_logs := H.emitBatchesUnordered
    (H.mapTick p_received_quorum_of_p1bs (fun _me ql => ql.getD []))
  -- the accepted-batch face, at the accepted wire
  ghost have haccept : ∀ i, accepted_logs i
      = List.map (fun l => Multiset.ofList l)
          ((((pP1bViews (mem prop) quorum_size
              (cqwrS.val.1 i) (dec.order i)
              (dec.snap i)).zip (p_ballot i)).map
            (fun vb => pP1bQuorum quorum_size vb.1 vb.2)).map
            (fun ql => ql.getD [])) := fun i => by
    unfold pP1bViews
    rfl
  -- witness: the realized success pool is the shared stage's success
  -- wire
  ghost witness (fun i => cqwrS.val.1 i)
  -- fails.flat_map_ordered(q!(|(_, ballot)| ballot))
  let fail_ballots := H.filterMap cqwrS.val.2 (fun _me ke => ke.2)
  (p_is_leader, accepted_logs, fail_ballots)
  prove
    ok_pool_le := hok_le,
    flags_eq := hflags,
    accepted_eq := haccept,
    leader_gate := (fun i {t} => by
      dsimp only
      rw [hflags i]
      intro ht htrue
      -- the flag reads (quorum-at-my-ballot, has-largest) at one tick
      have ht2 : t < ((Trace.zip
          ((Trace.zip (pP1bViews (mem prop) quorum_size (cqwrS.val.1 i)
              (dec.order i) (dec.snap i)) (p_ballot i)).map
            (fun vb => pP1bQuorum quorum_size vb.1 vb.2))
          (p_has_largest_ballot i)).map
      ...
      have h1 : ((Trace.zip
          ((Trace.zip (pP1bViews (mem prop) quorum_size (cqwrS.val.1 i)
            ...
            (fun vb => pP1bQuorum quorum_size vb.1 vb.2))
          (p_has_largest_ballot i)).map
          (fun ql => ql.1.isSome && ql.2))[t]'ht2 = true := htrue
      obtain ⟨hq2, hl, hread⟩ := Trace.zip_map_getElem ht2
      rw [hread] at h1
      obtain ⟨hv, hb, hread'⟩ := Trace.zip_map_getElem hq2
      rw [hread'] at h1
      ...
      cases hmax : p1bMaxQuorumBallot quorum_size
          ((pP1bViews (mem prop) quorum_size (cqwrS.val.1 i) (dec.order i)
      ...
      | some r =>
        ...
        · rw [if_neg hrb] at hq
          cases hq),
    accepted_src := (fun i {t} => by
      dsimp only
      rw [haccept i]
      intro ht lg hlg
      -- the batch is the gated bucket of the tick's (view, ballot) read
      have hin : t < (((pP1bViews (mem prop) quorum_size (cqwrS.val.1 i)
          (dec.order i) (dec.snap i)).zip (p_ballot i)).map
          (fun vb => pP1bQuorum quorum_size vb.1 vb.2)).length := by
        have hlen := ht
        simp only [Trace.zip, List.length_map, List.length_zip] at hlen ⊢
        omega
      obtain ⟨hv, hb, hread⟩ := Trace.zip_map_getElem hin
      refine ⟨hb, ?_⟩
      have h0 : lg ∈ (List.map (fun l => Multiset.ofList l)
          ((((pP1bViews (mem prop) quorum_size (cqwrS.val.1 i)
              (dec.order i) (dec.snap i)).zip (p_ballot i)).map
            (fun vb => pP1bQuorum quorum_size vb.1 vb.2)).map
            (fun ql => ql.getD [])))[t]'ht := hlg
      rw [List.getElem_map, List.getElem_map, hread] at h0
      have hlg2 : lg ∈ (pP1bQuorum quorum_size
          ((pP1bViews (mem prop) quorum_size (cqwrS.val.1 i) (dec.order i)
            (dec.snap i))[t]'hv) ((p_ballot i)[t]'hb)).getD [] :=
        Multiset.mem_coe.mp h0
      ...
      | some qlogs =>
        ...
        | some r =>
          ...
          · rw [if_pos hrb] at hq
            ...
            have hview : (pP1bViews (mem prop) quorum_size (cqwrS.val.1 i)
                (dec.order i) (dec.snap i))[t]'hv
                ∈ (prefixCuts (selectOrder (cqwrS.val.1 i)
                    (dec.order i)) 0 (dec.snap i)).map
            ...
            rw [hk] at hpair
            have hsel2 : ((rb, lg) : Ballot (mem prop)
                × ALog P (mem prop))
                ∈ selectOrder (cqwrS.val.1 i)
                  (dec.order i) :=
              (List.take_sublist ..).subset hpair
            have hpool := Multiset.mem_of_le (hok_le i)
              (selectOrder_mem _ hsel2)
    ...
    fails_src := fun i b hb => by
      ...
      have hb' : b ∈ Multiset.filterMap
          (fun ke : Ballot (mem prop) × Option (Ballot (mem prop)) =>
            ke.2)
          (Multiset.filterMap cqErrProj
            (Multiset.map
              (fun m : P1b P (mem prop) => (m.ballot, m.res))
      ...
      obtain ⟨m, hm, hpm⟩ := Multiset.mem_map.mp hpr
      refine ⟨m, hm, ?_⟩
      cases hr : m.res with
      | ok pl =>
        rw [← hpm, hr] at hprE
      ...
      | error e =>
        rw [← hpm, hr] at hprE
        have hkee : ke = (m.ballot, e) := by
          have hproj : cqErrProj ((m.ballot, Except.error e)
              : Ballot (mem prop) × Except (Option (Ballot (mem prop)))
                (ALog P (mem prop)))
              = some (m.ballot, e) := rfl
          rw [hproj] at hprE
          ...
          exact hprE'.symm
        rw [hkee] at hke2
        rw [show ((m.ballot, e) : Ballot (mem prop)
          × Option (Ballot (mem prop))).2 = e from rfl] at hke2
        rw [hke2]
