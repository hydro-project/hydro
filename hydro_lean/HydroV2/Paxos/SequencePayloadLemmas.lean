...
The bridges from the shared verified quorum layer (`Std/Quorum`,
`Std/RequestResponse`) into this module's pools, the sent-trace
vocabulary (the leader's
...
/-! ## Bridges into the shared quorum layer (`Std/Quorum`,
`Std/RequestResponse`) -/

/-- The generated-metadata batches (`emitBatchesUnordered`), flattened:
membership in the per-tick multiset sum is membership in the flattened
emission trace. -/
theorem mem_ofList_sum {β : Type _} [DecidableEq β]
    {t : List (List β)} {x : β} :
    x ∈ (t.map (fun l => Multiset.ofList l)).sum ↔ x ∈ t.flatten := by
  induction t with
  | nil =>
    simp only [List.map_nil, List.sum_nil, List.flatten_nil]
    exact ⟨fun h => absurd h (Multiset.notMem_zero x),
      fun h => absurd h (List.not_mem_nil)⟩
  | cons l ls ih =>
    rw [List.map_cons, List.sum_cons, List.flatten_cons]
    constructor
    · intro h
      rcases Multiset.mem_add.mp h with h | h
      · exact List.mem_append.mpr (Or.inl h)
      · exact List.mem_append.mpr (Or.inr (ih.mp h))
    · intro h
      rcases List.mem_append.mp h with h | h
      · exact Multiset.mem_add.mpr (Or.inl h)
      · exact Multiset.mem_add.mpr (Or.inr (ih.mpr h))

/-- `collect_quorum`'s ok-count over the keyed P2b pairs **is** the P2b
pool's vote count at the key. -/
theorem cqOkCount_p2b_pairs {nP : Nat} (pool : Multiset (P2b nP))
    (k : Nat × Ballot nP) :
    cqOkCount (pool.map (fun m => ((m.slot, m.ballot), m.res))) k
