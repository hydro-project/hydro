...
/-- The `Ok` projection of the keyed replies is `p1bOkPair`. -/
theorem okProj_pairs {nP : Nat} (pool : Multiset (P1b P nP)) :
    Multiset.filterMap cqOkProj
        (pool.map (fun m : P1b P nP => (m.ballot, m.res)))
      = Multiset.filterMap p1bOkPair pool := by
  induction pool using Multiset.induction with
  | empty => rfl
  | cons m rest ih =>
    ...
    | ok pl =>
      rw [Multiset.filterMap_cons_some _ _ _
          (show cqOkProj (m.ballot, Except.ok pl) = some (m.ballot, pl)
            from rfl),
        Multiset.filterMap_cons_some _ _ _
          (show p1bOkPair m = some (m.ballot, pl) by
            unfold p1bOkPair; rw [hres]),
        ih]
    | error e =>
      rw [Multiset.filterMap_cons_none _ _
          (show cqOkProj (m.ballot, Except.error e) = none from rfl),
        Multiset.filterMap_cons_none _ _
          (show p1bOkPair m = none by unfold p1bOkPair; rw [hres]),
        ih]

/-- **Consumed-pool projection bound** (surface vocabulary): the `Ok`
projection of any consumed keyed-reply pool embeds in the reply pool's
`p1bOkPair` projection — the bridge from `CQWREnsures.emit_pool_le` to
the regress's reply-pool vocabulary. -/
theorem consumed_okProj_le {nP : Nat}
    (pool : Multiset (P1b P nP))
    (dcq : List (Multiset (Ballot nP
      × Except (Option (Ballot nP)) (ALog P nP)))) :
    (cqConsumed (pool.map (fun m : P1b P nP => (m.ballot, m.res)))
        dcq).filterMap cqOkProj
      ≤ Multiset.filterMap p1bOkPair pool := by
  have hconsume : cqConsumed (pool.map (fun m : P1b P nP => (m.ballot, m.res)))
        dcq
      ≤ pool.map (fun m : P1b P nP => (m.ballot, m.res)) :=
    cqConsumed_le _ _
  exact le_trans (Multiset.filterMap_le_filterMap _ hconsume)
    (le_of_eq (okProj_pairs pool))
...
structure PP1bRequires (nP : Nat) (quorumSize : Nat)
  ...
  /-- **Success-pool soundness** (the shared quorum stage's
  `CQWREnsures.emit_pool_le`, bridged): the realized success pool embeds
  with multiplicity in the reply pool's `Ok` projection. -/
  ok_pool_le : okPool ≤ Multiset.filterMap p1bOkPair pool
  ...
  solicited_at_follower : ∀ m ∈ pool, (∃ v, (m : P1b P nP).res = .ok v) →
    ∃ (u : Nat)
      (hu : u < (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap).length)
      (hb : u < pb.length),
      pb[u]'hb = m.ballot
      ∧ (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap)[u]'hu = false
...
theorem pP1bViews_full_carry {nP : Nat} (quorumSize : Nat) {okPool : Multiset (Ballot nP × ALog P nP)}
    (pool : Multiset (P1b P nP))
    (dOrd : List (Ballot nP × ALog P nP)) (dSnap : List Nat)
    {t t' : Nat} (h : t ≤ t')
    (ht' : t' < (pP1bViews nP quorumSize okPool dOrd dSnap).length)
    {qb : Ballot nP} {vs : List (ALog P nP)}
    (hmem : (qb, vs) ∈ (pP1bViews nP quorumSize okPool dOrd
      dSnap)[t]'(Nat.lt_of_le_of_lt h ht'))
    (hfull : vs.length = quorumSize) :
    (qb, vs) ∈ (pP1bViews nP quorumSize okPool dOrd dSnap)[t']'ht' := by
  unfold pP1bViews at *
  have ht0 : t < (prefixCuts (selectOrder
      okPool dOrd) 0 dSnap).length := by
  ...
  obtain ⟨ext, hext⟩ := prefixCuts_getElem_prefix (pool := selectOrder
    okPool dOrd) (d := dSnap) h ht0'
  rw [← hext]
  exact foldEarlyStop_full_stable hmem hfull ext

/-- Every payload of a realized bucket is an `Ok` reply in the pool at
the bucket's ballot. -/
theorem pP1bViews_entry_ok {nP : Nat} (quorumSize : Nat) {okPool : Multiset (Ballot nP × ALog P nP)}
    (pool : Multiset (P1b P nP))
    (hok : okPool ≤ Multiset.filterMap p1bOkPair pool)
    (dOrd : List (Ballot nP × ALog P nP)) (dSnap : List Nat)
    {t : Nat} (ht : t < (pP1bViews nP quorumSize okPool dOrd dSnap).length)
    {qb : Ballot nP} {vs : List (ALog P nP)}
    (hmem : (qb, vs) ∈ (pP1bViews nP quorumSize okPool dOrd dSnap)[t]'ht) :
  ...
  have hsel : ((qb, v) : Ballot nP × ALog P nP)
      ∈ selectOrder okPool dOrd :=
    (List.take_sublist ..).subset hpair
  obtain ⟨m, hm, hok⟩ :=
    (Multiset.mem_filterMap _ _).mp
      (Multiset.mem_of_le hok
        (selectOrder_mem _ hsel))
  refine ⟨m, hm, ?_, ?_⟩
...
theorem pP1bFlags_getElem {nP : Nat} (quorumSize : Nat) {okPool : Multiset (Ballot nP × ALog P nP)}
    (pool : Multiset (P1b P nP)) (pb : Trace (Ballot nP))
    (phl : Trace Bool) (dOrd : List (Ballot nP × ALog P nP))
    (dSnap : List Nat) {u : Nat}
    (hu : u < (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap).length) :
    ∃ (hv : u < (pP1bViews nP quorumSize okPool dOrd dSnap).length)
      (hb : u < pb.length) (hl : u < phl.length),
      (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap)[u]'hu
        = ((pP1bQuorum quorumSize
            ((pP1bViews nP quorumSize okPool dOrd dSnap)[u]'hv)
            (pb[u]'hb)).isSome && phl[u]'hl) := by
  have hlen := hu
  unfold pP1bFlags at hlen
  simp only [Trace.zip, List.length_map, List.length_zip] at hlen
  have hv : u < (pP1bViews nP quorumSize okPool dOrd dSnap).length := by
    omega
  have hb : u < pb.length := by omega
  have hl : u < phl.length := by omega
  refine ⟨hv, hb, hl, ?_⟩
  have hu' : u < ((Trace.zip
      ((Trace.zip (pP1bViews nP quorumSize okPool dOrd dSnap) pb).map
        (fun vb => pP1bQuorum quorumSize vb.1 vb.2)) phl).map
      (fun ql => ql.1.isSome && ql.2)).length := hu
  show ((Trace.zip
      ((Trace.zip (pP1bViews nP quorumSize okPool dOrd dSnap) pb).map
        (fun vb => pP1bQuorum quorumSize vb.1 vb.2)) phl).map
      (fun ql => ql.1.isSome && ql.2))[u]'hu' = _
  obtain ⟨hq2, hl2, hread⟩ := Trace.zip_map_getElem hu'
  rw [hread]
  obtain ⟨hv2, hb2, hread2⟩ := Trace.zip_map_getElem hq2
  rw [hread2]

/-- All realized buckets hold at most `quorum_size` logs. -/
theorem pP1bViews_length_le {nP : Nat} (quorumSize : Nat) {okPool : Multiset (Ballot nP × ALog P nP)}
    (hq1 : 1 ≤ quorumSize) (pool : Multiset (P1b P nP))
    (dOrd : List (Ballot nP × ALog P nP)) (dSnap : List Nat)
    {t : Nat} (ht : t < (pP1bViews nP quorumSize okPool dOrd dSnap).length)
    {e : Ballot nP × List (ALog P nP)}
    (hmem : e ∈ (pP1bViews nP quorumSize okPool dOrd dSnap)[t]'ht) :
    e.2.length ≤ quorumSize := by
  unfold pP1bViews at ht hmem
  rw [List.getElem_map] at hmem
...
theorem pP1bViews_bucket_sub_oks {nP : Nat} (quorumSize : Nat) {okPool : Multiset (Ballot nP × ALog P nP)}
    (pool : Multiset (P1b P nP))
    (hok : okPool ≤ Multiset.filterMap p1bOkPair pool)
    (dOrd : List (Ballot nP × ALog P nP)) (dSnap : List Nat)
    {t : Nat} (ht : t < (pP1bViews nP quorumSize okPool dOrd dSnap).length)
  ...
  · subst hbx
    ...
    have hle := Multiset.count_le_of_le
      ...
      (selectOrder_subpool (pool := okPool)
        (d := dOrd))
    have hle2 := Multiset.count_le_of_le
      ((bx, vx) : Ballot nP × ALog P nP)
      hok
    rw [hpair] at hsel
    omega
...
exhausts the trace. -/
theorem pP1b_no_false_full {nP : Nat} (quorumSize : Nat) {okPool : Multiset (Ballot nP × ALog P nP)}
    (hq1 : 1 ≤ quorumSize) (pool : Multiset (P1b P nP))
    (pb : Trace (Ballot nP)) (phl : Trace Bool)
    (dOrd : List (Ballot nP × ALog P nP)) (dSnap : List Nat) (me : Fin nP)
    (req : PP1bRequires nP quorumSize pool okPool pb phl dOrd dSnap me) :
    ∀ (n u : Nat)
      (hu : u < (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap).length),
      (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap).length - u ≤ n →
      (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap)[u]'hu = false →
      ∀ (hv : u < (pP1bViews nP quorumSize okPool dOrd dSnap).length)
        (hb : u < pb.length) (vs : List (ALog P nP)),
        (pb[u]'hb, vs) ∈ (pP1bViews nP quorumSize okPool dOrd dSnap)[u]'hv →
  ...
  | succ n ih =>
    ...
    have hrmem : ((rb, rlogs) : Ballot nP × List (ALog P nP))
        ∈ (pP1bViews nP quorumSize okPool dOrd dSnap)[u]'hv := by
    ...
    obtain ⟨m, hm, hmb, hmres⟩ := pP1bViews_entry_ok quorumSize pool req.ok_pool_le dOrd
...
theorem pP1b_ballot_stable {nP : Nat} (quorumSize : Nat) {okPool : Multiset (Ballot nP × ALog P nP)}
    (hq1 : 1 ≤ quorumSize) (pool : Multiset (P1b P nP))
    (pb : Trace (Ballot nP)) (phl : Trace Bool)
    (dOrd : List (Ballot nP × ALog P nP)) (dSnap : List Nat) (me : Fin nP)
    (req : PP1bRequires nP quorumSize pool okPool pb phl dOrd dSnap me)
    {t : Nat}
    (ht1 : t + 1 < (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap).length)
    (h1 : (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap)[t + 1]'ht1
      = true)
    (h0 : (pP1bFlags nP quorumSize okPool pb phl dOrd
  ...
  have hqm : p1bMaxQuorumBallot quorumSize
      ((pP1bViews nP quorumSize okPool dOrd dSnap)[t + 1]'hv1)
      = some (pb[t + 1]'hb1, qlogs) := by
    unfold pP1bQuorum at hq
    cases hmax : p1bMaxQuorumBallot quorumSize
        ((pP1bViews nP quorumSize okPool dOrd dSnap)[t + 1]'hv1) with
  ...
  have hut' : t + 1 < u := by
    ...
    · exfalso
      subst heqe
      have hfl : (pP1bFlags nP quorumSize okPool pb phl dOrd
  ...
  refine pP1b_no_false_full quorumSize hq1 pool pb phl dOrd dSnap me req
    (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap).length u hu
...
per tick. -/
theorem pP1bViews_bucket_unique {nP : Nat} {quorumSize : Nat} {okPool : Multiset (Ballot nP × ALog P nP)}
    {dOrd : List (Ballot nP × ALog P nP)} {dSnap : List Nat} {t : Nat}
    (ht : t < (pP1bViews nP quorumSize okPool dOrd dSnap).length)
    {qb : Ballot nP} {vs vs' : List (ALog P nP)}
    (h : (qb, vs) ∈ (pP1bViews nP quorumSize okPool dOrd dSnap)[t]'ht)
    (h' : (qb, vs') ∈ (pP1bViews nP quorumSize okPool dOrd dSnap)[t]'ht) :
    vs = vs' := by
...
theorem pP1b_leader_bucket {nP : Nat} (quorumSize : Nat) {okPool : Multiset (Ballot nP × ALog P nP)}
    (hq1 : 1 ≤ quorumSize) (pool : Multiset (P1b P nP))
    (pb : Trace (Ballot nP)) (phl : Trace Bool)
    (dOrd : List (Ballot nP × ALog P nP)) (dSnap : List Nat) {u : Nat}
    (hu : u < (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap).length)
    (htrue : (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap)[u]'hu
      = true) :
    ∃ (hv : u < (pP1bViews nP quorumSize okPool dOrd dSnap).length)
      (hb : u < pb.length) (hl : u < phl.length)
      (qlogs : List (ALog P nP)),
      pP1bQuorum quorumSize
          ((pP1bViews nP quorumSize okPool dOrd dSnap)[u]'hv) (pb[u]'hb)
        = some qlogs
      ∧ (pb[u]'hb, qlogs)
          ∈ (pP1bViews nP quorumSize okPool dOrd dSnap)[u]'hv
  ...
  unfold pP1bQuorum at hq
  cases hmax : p1bMaxQuorumBallot quorumSize
      ((pP1bViews nP quorumSize okPool dOrd dSnap)[u]'hv) with
  | none =>
    rw [hmax] at hq
    cases hq
  | some r =>
...
theorem pP1b_quorum_pinned {nP : Nat} (quorumSize : Nat) {okPool : Multiset (Ballot nP × ALog P nP)}
    (hq1 : 1 ≤ quorumSize) (pool : Multiset (P1b P nP))
    (pb : Trace (Ballot nP)) (phl : Trace Bool)
    (dOrd : List (Ballot nP × ALog P nP)) (dSnap : List Nat)
    {t t' : Nat} (h : t ≤ t')
    (ht : t < (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap).length)
    (ht' : t' < (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap).length)
    (htrue : (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap)[t]'ht
      = true)
    (htrue' : (pP1bFlags nP quorumSize okPool pb phl dOrd dSnap)[t']'ht'
      = true)
    {hb : t < pb.length} {hb' : t' < pb.length}
    (hpb : pb[t]'hb = pb[t']'hb') :
    ∃ (hv : t < (pP1bViews nP quorumSize okPool dOrd dSnap).length)
      (hv' : t' < (pP1bViews nP quorumSize okPool dOrd dSnap).length)
      (qlogs : List (ALog P nP)),
      pP1bQuorum quorumSize
          ((pP1bViews nP quorumSize okPool dOrd dSnap)[t]'hv) (pb[t]'hb)
        = some qlogs
      ∧ pP1bQuorum quorumSize
          ((pP1bViews nP quorumSize okPool dOrd dSnap)[t']'hv')
          (pb[t']'hb')
        = some qlogs := by
