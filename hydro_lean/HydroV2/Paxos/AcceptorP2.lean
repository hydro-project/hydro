...
hydro def acceptor_p2 (H : HydroSem L mem) (acc prop : L)
    (a_max_ballot :
    ...
    (nondet_batch : H.BatchDec (mem acc) (P2a P (mem prop)))
    (chP2b : H.TransportDec (mem prop) (mem acc)) :
  ...
  -- the log face, at the log wire (`ack_src`'s coverage and the
  -- contract's `log_face` share it)
  ghost have hface : ∀ i, a_log i
      = (Trace.zip (a_checkpoint i)
          (foldAcrossTicksTrace (· + ·) 0
            (ap2QualBatches (mem prop) (a_max_ballot i)
              (p_to_acceptors_p2a i) (nondet_batch i)))).map
        (fun ce => (ce.1, logView ce.2)) := fun i => by
    ...
    (fun _me p2a mb =>
      (p2a.sender.val,
        (⟨p2a.slot, p2a.ballot,
          ...
          : P2b (mem prop))))
  -- per-sender ack characterization WITH write-before-ack coverage, at
  -- the ack wire: `ack_src` and `from_src` both consume it (the merged
  -- version re-derived nothing)
  ghost have hfrom : ∀ (j : Fin (mem acc)) (r : Fin (mem prop)),
      ∀ m ∈ ap2From (fun j' => batchCuts (p_to_acceptors_p2a j') 0
          (nondet_batch j'))
        (fun j' => a_max_ballot j') j r,
      ∃ (t : Nat)
        (hb : t < (batchCuts (p_to_acceptors_p2a j) 0
          (nondet_batch j)).length)
        (hm : t < (a_max_ballot j).length),
        ∃ p2a ∈ (batchCuts (p_to_acceptors_p2a j) 0
            (nondet_batch j))[t]'hb,
          p2a.sender.val = r.val
          ∧ m.slot = p2a.slot ∧ m.ballot = p2a.ballot
          ∧ m.res = (if some p2a.ballot = (a_max_ballot j)[t]'hm
              then Except.ok () else Except.error ((a_max_ballot j)[t]'hm))
          ∧ (m.res = .ok () →
              (a_max_ballot j)[t]'hm = some m.ballot
              ∧ ∀ hl : t < (a_log j).length,
                LogCovers (((a_log j)[t]'hl).2) m.slot
                  m.ballot) := fun j r m hm1 => by
    ...
    · rw [if_pos hr] at haddr
      ...
      -- all_ticks: the ack's tick, and the tick's two reads
      obtain ⟨mt, hmt, hdx2⟩ := mem_list_sum.mp hdx
      obtain ⟨t, hb, hmx, rfl⟩ := Trace.mem_zip_map hmt
        ...
        · refine ⟨?_, ?_⟩
          ...
          · intro hl
            ...
            have hqb : t < (ap2QualBatches (mem prop) (a_max_ballot j)
              ...
              simp only [Trace.zip, List.length_map, List.length_zip]
              exact Nat.lt_min.mpr ⟨hb, hmx⟩
            have hqel : (ap2QualBatches (mem prop) (a_max_ballot j)
              ...
              obtain ⟨hb2, hmx2, hqread⟩ := Trace.zip_map_getElem hqb
              rw [hqread]
            have hent : ((a.slot,
  ...
  (a_log,
    H.values (H.demux chP2b (H.allTicks acks) (fun r => r.val)))
  prove
    log_face := hface,
    ack_src := (fun r m hm => by
      have hm0 : m ∈ ((List.finRange (mem acc)).map
          (fun j => ap2From
            (fun j' => batchCuts (p_to_acceptors_p2a j') 0
              (nondet_batch j'))
            (fun j' => a_max_ballot j') j r)).sum := hm
      obtain ⟨mm, hmm, hm1⟩ := mem_list_sum.mp hm0
      obtain ⟨j, -, rfl⟩ := List.mem_map.mp hmm
      obtain ⟨t, hb, hmx, p2a, hp2a, hsend, hslot, hballot, hres, -⟩ :=
        hfrom j r m hm1
      exact ⟨j, t, hb, hmx, p2a, hp2a, hsend, hslot, hballot, hres⟩),
    ack_decomp := fun r => rfl,
    from_key_cap := (fun j r sl b => by
      unfold ap2From
...
theorem acceptor_p2_ok_max (acc prop : L)
    ...
    (pool : Fin (mem acc) → Multiset (P2a P (mem prop)))
    (ck : TickV (mem acc) (Option Nat) .unbounded)
    (dB : BatchCuts (mem acc) (P2a P (mem prop))) (ch : Unit)
