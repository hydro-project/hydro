...
fn recursively_set_span(token: &mut proc_macro2::TokenTree, span: proc_macro2::Span) {
    match token {
        proc_macro2::TokenTree::Group(group) => {
            ...
            for mut inner_token in group.stream() {
                recursively_set_span(&mut inner_token, span);
                new_stream.extend(std::iter::once(inner_token));
            }
            let new_group = proc_macro2::Group::new(group.delimiter(), new_stream);
            *group = new_group;
            group.set_span(span);
        }
        _ => {
...
pub fn copy_span(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let output = target.into_iter().fold(
